...
url = "http://elanpy.com:39111/?key=123&method=scrapy"
...
def addWhite(res):
    ...
    ip_res = requests.get(ip_url).json()["origin"]
    whiteList = requests.post(url, data={"secret":res["proxyPass"], "ip":ip_res, "order_id":res["proxyUser"]}).json()
    if str(whiteList["code"])=="200":
        return True
    else:
        ezfuc.sleep(2)
        ezfuc.color_print("添加白名单失败")
        addWhite(res)

def ezproxy(req, add_white=True):
    res = getIP()
    if add_white:
        addWhite(res)
