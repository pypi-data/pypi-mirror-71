...
class Search:
    ...
    def __init__(self,movie_name,api_key,movie_language,review_template,project_folder):
    ...
    def _imdb_search(self):
        ...
        return self.search_service.cse().list(
            q = f'{self.movie_name} {self.movie_language}',
            cx =self.cx,
        ).execute()
    ...
    def _image_download(self,image_url):
        ...
        with open(os.path.join(self.project_folder,'assets','images',f'{self.movie_name}.jpg'), 'wb') as handler:
    ...
    def data(self):
        ...
        movie_info,cast_info = self._crawl_imdb_info(movie_data['pagemap']['metatags'][0]['pageid'])
        if 'og:image' in movie_data['pagemap']['metatags'][0]:
            return self._image_download(image_url),movie_info,cast_info
        return False,movie_info,cast_info
