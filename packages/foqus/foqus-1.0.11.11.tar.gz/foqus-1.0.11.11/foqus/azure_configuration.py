...
def azure_move_directory(src, dest):
    SAS = AZURE_SAS
    logger.info("moving blobs from original to recovery folder on the same azure container... ")
    try:
        comm = 'azcopy cp "' + src + '?' + SAS + '" ' + '"' + dest + '?' + SAS + '" --recursive=true --overwrite=ifSourceNewer'
        print(comm)
        ...
        supp = 'azcopy rm "' + src + '?' + SAS + '" --recursive=true'
        print(supp)
