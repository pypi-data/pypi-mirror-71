...
class DownloadItemData(object):
    ...
    def _run_load(self):
        ...
        self._filter_items()
        self.itemStorageType = DataStorageType.get_dataStorageType(DownloadItemData.STORAGE_TYPE)
    ...
    def write_spreadsheet(self,fileName=None):
        ...
        self.df.to_excel(writer, sheet_name=DownloadItemData.SHEET_NAME,index=False)
...
class DownloadItemDataMstr(DownloadItemData):
    ...
    def __init__(self, spreadSheetName,  
        ...
        self.custId = None
        self.output_fields = [x for x in self.output_fields if x not in self.exc_output_fields]
        self.email = emailAddress
        self._client = FirestoreClient.getInstance()
        self.use_tmp_folder = use_tmp
        self.file_name = self.create_file_name(spreadSheetName)
...
class ProcessItemUpload(object):
    ...
    def _process_updates(self):
        ...
        update_fields = [x for x in update_fields if x in list(self.df.columns)]
    ...
    def _pre_process(self):
        itemStorageType = DataStorageType.get_dataStorageType(ProcessItemUpload.STORAGE_TYPE)
...
def process_uploaded_file(blob_uri,filename,email):
    ...
    if sBlob.get_fileType() == 'master':
        ...
        if sBlob.get_fileType() == 'customer':
            ...
            raise Exception("Unknown uploaded file... valid sheet name not found")
