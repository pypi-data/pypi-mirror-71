...
class GNCSocketClient(GNCSocketBase):
    ...
    def socket_task(self):
        ...
        with GNCSocket(mode='client', timeout=self.timeout, raise_timeout=self.raise_timeout,log_path=self.log_path) as gs:
            try:
                self.loop = loop
            ...
            async def activate_sock():
                try:
                    ...
                    self.logger.exception("Tiempo fuera en escritura %s, station %s" %(
                        te, self.station))
                    await asyncio.sleep(10)
                except (ConnectionResetError, ConnectionAbortedError) as conn_error:
                    self.logger.exception("Excepción por desconexión %s, station %s"%(
                        conn_error, self.station))
                    await asyncio.sleep(10)                                                           
