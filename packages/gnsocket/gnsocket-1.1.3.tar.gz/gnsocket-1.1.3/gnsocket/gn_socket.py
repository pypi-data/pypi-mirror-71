    ...
    * client
    ...
    async def readbytes(self, reader, n):
        ...
        try:
            ...
            print("Incomplete read, station %s" %self.mode, reader, n)
            self.logger.exception("Station %s, Tiempo fuera al no poder leer en readbytes %s bytes, %s"%(n, self.mode,ir))
            return None
        except (ConnectionResetError, ConnectionAbortedError) as conn_error:
    ...
    async def recv_msg(self, id_client):
        ...
        try:
            ...
            while t:
                ...
                n = 0
                while n < bs_1:
                    if await self.heart_beat(id_client):
                        # moment 2 -> get content message, check if there are more
                        try:
