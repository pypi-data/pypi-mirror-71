...
def create_datetime(*values: Expression) -> Expression:
    ...
    passed_values = [_ensure_expression(value) for value in values]   
    # Ensure 6 values are always passed. DataPrep native functions do not have variable argument counts.
    expanded_values = passed_values + [_ensure_expression(1)] * (3 - len(passed_values))
    expanded_values = expanded_values + [_ensure_expression(0)] * (6 - len(expanded_values))
    return InvokeExpression(IdentifierExpression('CreateDateTime'), expanded_values)
