...
def get_device_id_by_client_id (registered_devices):
    try:
    ...
    return registered_devices [d ['client_id']]

def register_device (device_model_id = None):
...
def use_device_sd (device, lang = 'ko-KR', output_audio_file = None, once = True, display = True, verbose = False, *args, **kwargs):
    if isinstance (device, (int, str)):
        audio_source = sources.SoundDeviceStream(
            sample_rate=16000,
            sample_width=2,
            block_size=6400,
            flush_size=25600,
        ...
        audio_source = device # virtusl device
    resp = assistant.send (audio_source, lang, output_audio_file, once, display, verbose, *args, **kwargs)
    audio_source.close ()
    return resp

def transport (resp, output_path = '', lang = None):
    dialog = AttrDict ()
    dialog ['question'] = resp.question
    dialog ['original_answer_audio'] = os.path.join (output_path, 'assistant.mp3')
    resp.save (dialog ['original_answer_audio'])

    detail = resp.detail
    if isinstance (detail, bytes):
        detail = detail.decode ()
    dialog ['detail'] = os.path.join (output_path, 'assistant.html')
    with open (dialog ['detail'], 'w') as f:
        f.write (detail)
    dialog ['supplemental_display_text'] = resp.supplemental_display_text
    dialog ['modulated_answer_audio'] = os.path.join (output_path, 'assistant.wav')
    try:
        modulator.modulate (dialog ['original_answer_audio'], dialog ['modulated_answer_audio'], lang)
