...
class Table(metaclass=MetaTable):
                ...
                if 'array' in config and config['array']:
                    ...
                    if (config['type'] == 'int' or
                        ...
                        if 'to' in value:
                            ...
                            fields.append(cls.name+'."'+column+"\"<=%s")
                        if 'in' in value and (isinstance(value['in'], list) or isinstance(value['in'], tuple)):
    ...
    @classmethod
    def save(cls, id, data, where={}):
        where = cls.where(where)
        ...
        try:
            ...
            cursor.execute(*debug(f"""UPDATE {cls}
                                    SET {update.fields()}
                                    WHERE {cls('id')}=%s AND {where.fields()}
                                    RETURNING {cls.select()}""",
                                update.values(id)+where.values()))
