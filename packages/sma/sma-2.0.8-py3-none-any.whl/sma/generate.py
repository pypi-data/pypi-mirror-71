...
def addRandomEdges(G : nx.Graph, 
    ...
    G.add_edges_from(itertools.islice(possibleSocSocEdges, 
                                      edgesCounts[sma.properties.EDGE_TYPE_SOC_SOC]))
    
    # add eco-soc edges
    possibleEcoSocEdges = list(itertools.product(socNodes, ecoNodes))
    numpy.random.shuffle(possibleEcoSocEdges)
    G.add_edges_from(itertools.islice(possibleEcoSocEdges, 
                                      edgesCounts[sma.properties.EDGE_TYPE_ECO_SOC]))
    
    # add eco-eco edges
    possibleEcoEcoEdges = list(itertools.combinations(ecoNodes, 2))
    numpy.random.shuffle(possibleEcoEcoEdges)
    G.add_edges_from(itertools.islice(possibleEcoEcoEdges, 
                                      edgesCounts[sma.properties.EDGE_TYPE_ECO_ECO]))
...
def _randomEdgesFixedDegreesIntraLevel(G : nx.Graph, degree_dict : dict):
    degree_dict = {k : v for k, v in degree_dict.items() if v > 0}
    print(degree_dict)
    while len(degree_dict) > 0:
        vertex, degree = degree_dict.popitem()
        candidates = list(degree_dict.keys())
        if len(candidates) < degree:
            print("no")
        numpy.random.shuffle(candidates)
        winners = candidates[:degree]
        G.add_edges_from(itertools.product([vertex], winners))
        for winner in winners:
            if degree_dict[winner] <= 1:
                degree_dict.pop(winner)
            else:
                degree_dict[winner] -= 1
        print(degree_dict)
        print(sum(degree_dict.values()))


def randomMultiSENsFixedDegrees(G : nx.Graph):
    degree_dist = sma.degreeDistribution(G)  
    nodes_by_type = sma.nodesByType(G)
    levels = nodes_by_type.keys()
    template = nx.Graph()
    template.add_nodes_from(G.nodes(data = True))
    while True:
        rand = template.copy()
        for level in levels:
            degree_dict = {node : degree_dist[node][level] for node in nodes_by_type[level]}
            _randomEdgesFixedDegreesIntraLevel(rand, degree_dict)
        yield rand
