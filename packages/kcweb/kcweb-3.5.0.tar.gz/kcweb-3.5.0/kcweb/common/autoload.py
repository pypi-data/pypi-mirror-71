# -*- coding: utf-8 -*-
import time,hashlib,json,re,os,platform,sys,shutil,zipfile,requests
...
class create:
    ...
    def packplug(self,plug):
        ...
        if os.path.exists(self.appname+"/"+self.modular+"/controller/"+plug):
            ...
            return False,"失败"
    def uploadplug(self,plug,username,password):
        "上传一个插件"
        timestamp=times()
        ...
        if arr and arr['code']==0:
            return True,arr['msg'],arr['data']
        elif arr:
            return False,arr['msg']
        else:
            return False,"上传失败，服务器暂时无法处理上传"
    def installplug(self,plug,edition=''):
        ...
        if os.path.exists(self.appname+"/"+self.modular+"/controller/"+plug):
            ...
            http=Http()
            http.openurl(config.domain['kcwebapi']+"/pub/plug","POST",data={"modular":self.modular,"name":str(plug),"edition":str(edition)})
            arr=json_decode(http.get_text)['data']
            del http
            if arr:
                r=requests.get(config.domain['kcwebfile']+"/"+arr['fileurl'])
    ...
    def packmodular(self):
        ...
        if os.path.exists(self.appname+"/"+self.modular):
            ...
            return False,"失败"
    def uploadmodular(self,username,password):
        "上传模块"
        timestamp=times()
        sign=md5(str(username)+str(timestamp)+md5(md5(password)))
        http=Http()
    ...
    def installmodular(self):
        ...
        if os.path.exists(self.appname+"/"+self.modular):
            ...
            http=Http()
            http.openurl(config.domain['kcwebapi']+"/pub/modular","POST",data={"name":self.modular})
            arr=json_decode(http.get_text)['data']
            print(config.domain['kcwebapi']+"/pub/modular")
            del http
            if arr:
                r=requests.get(config.domain['kcwebfile']+"/"+arr['fileurl'])
