...
#from .lib import generateColorByText
# import cv2
import numpy as np
...
class Canvas(QWidget):
    ...
    def mousePressEvent(self, ev):
        ...
        logging.debug(f"self.editing(): {self.editing()}")
        ...
        if ev.button() == Qt.LeftButton:
            ...
            logging.debug("RightButton to pop up something")
            self.selectShapePoint(pos)
            self.prevPoint = pos
            logging.debug(f"RightButton {pos}")
            self.repaint()
    ...
    def setShapesVisible(self, shapes, value):
        for shape in shapes:
            self.visible[shape] = value
        self.repaint()
