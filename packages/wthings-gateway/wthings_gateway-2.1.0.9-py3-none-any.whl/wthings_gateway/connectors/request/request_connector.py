...
from wthings_gateway.connectors.request.json_request_uplink_converter import JsonRequestUplinkConverter
from wthings_gateway.connectors.request.json_request_downlink_converter import JsonRequestDownlinkConverter
        ...
        for endpoint in self.__config["mapping"]:
            try:
                ...
                if endpoint["converter"]["type"] == "custom":
                    module = WTUtility.check_and_import(self.__connector_type, endpoint["converter"]["extension"])
    ...
    def __fill_attribute_updates(self):
        for attribute_request in self.__config["attributeUpdates"]:
            if attribute_request.get("converter") is not None:
                converter = WTUtility.check_and_import("request", attribute_request["converter"])(attribute_request)
            else:
    ...
    def __fill_rpc_requests(self):
        for rpc_request in self.__config["serverSideRpc"]:
            if rpc_request.get("converter") is not None:
                converter = WTUtility.check_and_import("request", rpc_request["converter"])(rpc_request)
            else:
