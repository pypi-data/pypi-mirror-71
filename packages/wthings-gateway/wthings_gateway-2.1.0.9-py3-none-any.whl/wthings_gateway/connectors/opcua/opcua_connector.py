...
class OpcUaConnector(Thread, Connector):
        ...
        try:
            ...
            while not self.__stopped:
                ...
                try:
                    ...
                    log.error("OSError on connection to OPC-UA server with url %s", self.__server_conf.get("url"))
                except Exception as e:
                    self.close()
                    log.exception(e)
        except Exception as e:
    ...
    def server_side_rpc_handler(self, content):
        try:
            self.__log.debug("server_side_rpc_handler content=%s" % (content))
                ...
                if rpc_method is not None and method.get(rpc_method) is not None:
                    ...
                    try:
                        if type(arguments) is list:
                            self.__log.debug("nodel=%s rpc_method=%s list:arguments=%s" % (method["node"], method[rpc_method], arguments))
                            result = method["node"].call_method(method[rpc_method], *arguments)
                        elif arguments is not None:
                            self.__log.debug("nodel=%s rpc_method=%s arguments=%s" % (method["node"], method[rpc_method], arguments))
                            result = method["node"].call_method(method[rpc_method], arguments)
                        else:
                            self.__log.debug("nodel=%s rpc_method=%s " % (method["node"], method[rpc_method]))
    ...
    def __scan_nodes_from_config(self):
        # log.info("__scan_nodes_from_config------------------------self.__interest_nodes=%s" % (self.__interest_nodes, ))
        try:
                ...
                for device_object in self.__interest_nodes:
                    for current_device in device_object:
                        try:
                            ...
                            log.info("device_configuration=%s" % (device_configuration, ))
                            devices_info_array = self.__search_general_info(device_configuration)
                            log.info("devices_info_array=%s" % (devices_info_array, ))
    ...
    def __search_nodes_and_subscribe(self, device_info):
        ...
        for information_type in information_types:
            for information in device_info["configuration"][information_type]:
                ...
                config_path = WTUtility.get_value(information["path"], get_tag=True)
                    ...
                    if information_node is not None:
                        ...
                        if device_info.get("uplink_converter") is None:
                            ...
                            if device_info["configuration"].get('converter') is None:
                                ...
                                converter = WTUtility.check_and_import(self._connector_type, configuration)
                        ...
                        log.info("information_value=%s" % (information_value, ))
                        converted_data = converter.convert((config_path, information_path), information_value)
                        log.info("converted_data=%s" % (converted_data, ))
                        ...
                        log.debug("Data to WThings: %s", converted_data)
                    else:
                ...
                for attribute_update in device_info["configuration"]["attributes_updates"]:
                    ...
                    for attribute_node in attribute_nodes:
                        if attribute_node is not None:
                            self.__available_object_resources[device_name]["variables"].append({attribute_update["attributeOnWThings"]: attribute_node})
                        else:
    ...
    def __search_general_info(self, device):
        result = []
        match_devices = []
        self.__search_node(self.__opcua_nodes["root"], WTUtility.get_value(device["deviceNodePattern"], get_tag=True), result=match_devices)
        for device_node in match_devices:
            if device_node is not None:
                ...
                name_expression = WTUtility.get_value(name_pattern_config, get_tag=True)
                if "${" in name_pattern_config and "}" in name_pattern_config:
                    ...
                    device_name_node = []
                    self.__log.debug("device_node=%s name_path=%s" % (device_node, name_path))
                    self.__search_node(device_node, name_path, result=device_name_node)
                    self.__log.debug("device_name_node=%s" % (device_name_node))
                ...
                if device.get("deviceTypePattern"):
                    device_type_expression = WTUtility.get_value(device["deviceTypePattern"],
                                                                 get_tag=True)
                ...
                log.error("Device node not found with expression: %s", WTUtility.get_value(device["deviceNodePattern"], get_tag=True))
        return result
...
class SubHandler(object):
    ...
    def datachange_notification(self, node, val, data):
        try:
            log.debug("datachange_notification node=%s val=%s data=" % (node, val, data))
            log.debug("SourceTimestamp=%s" % (data.monitored_item.Value.SourceTimestamp))
