...
log = logging.getLogger("wt_connection")


class WTTimeoutException(Exception):
    pass


class WTQoSException(Exception):
    pass
...
class WTPublishInfo:
    ...
    WT_ERR_INVAL = 3
    WT_ERR_NO_CONN = 4
    WT_ERR_CONN_REFUSED = 5
    WT_ERR_NOT_FOUND = 6
    WT_ERR_CONN_LOST = 7
    WT_ERR_TLS = 8
    WT_ERR_PAYLOAD_SIZE = 9
    WT_ERR_NOT_SUPPORTED = 10
    WT_ERR_AUTH = 11
    WT_ERR_ACL_DENIED = 12
    WT_ERR_UNKNOWN = 13
    WT_ERR_ERRNO = 14
    WT_ERR_QUEUE_SIZE = 15
...
class WTDeviceMqttClient:
    ...
    def _on_publish(self, client, userdata, result):
        # log.debug("Data published to WThings!")
        pass
    ...
    def disconnect(self):
        ...
        log.debug("Disconnecting from WThings")
        self.__is_connected = False
    ...
    def _on_message(self, client, userdata, message):
        content = WTUtility.decode(message)
    ...
    def send_rpc_reply(self, req_id, resp, quality_of_service=1, wait_for_publish=False):
        if quality_of_service != 0 and quality_of_service != 1:
            ...
            return
        log.info("mqtt rpc replytopic=%s data=%s" % (RPC_RESPONSE_TOPIC + req_id, resp))
    ...
    def send_rpc_call(self, method, params, callback):
        ...
        log.info("mqtt rc call topic=%s data=%s" % (RPC_REQUEST_TOPIC, payload))
    ...
    def publish_data(self, data, topic, qos):
        ...
        if qos != 0 and qos != 1:
            ...
            raise WTQoSException("Quality of service (qos) value must be 0 or 1")
        else:
            log.debug("mqtt publish topic=%s data=%s" % (topic, data))
            return WTPublishInfo(self._client.publish(topic, data, qos))
    ...
    def request_attributes(self, client_keys=None, shared_keys=None, callback=None):
        ...
        log.info("mqtt request_attributes topic=%s data=%s" % (ATTRIBUTES_TOPIC_REQUEST, msg))
                ...
                if not self.__timeout_queue.empty():
                    ...
                    if item is not None:
                        ...
                        if callback is not None:
                            callback(None, WTTimeoutException("Timeout while waiting for reply from WThings!"))
                else:
                    time.sleep(0.01)
