...
from wthings_gateway.wt_client.wt_device_mqtt import WTDeviceMqttClient, DEVICE_TS_KV_VALIDATOR, KV_VALIDATOR
from wthings_gateway.wt_utility.wt_utility import WTUtility

GATEWAY_ATTRIBUTES_TOPIC = "v1/gateway/attributes"
GATEWAY_ATTRIBUTES_REQUEST_TOPIC = "v1/gateway/attributes/request"  # 没有使用
...
class WTGatewayMqttClient(WTDeviceMqttClient):
    ...
    def _on_message(self, client, userdata, message):
        ...
        log.debug("_on_message topic=%s content=%s" % (message.topic, content))
        ...
        if message.topic.startswith(GATEWAY_ATTRIBUTES_RESPONSE_TOPIC):
            with self._lock:
                ...
                if self._attr_request_dict[req_id]:
                    ...
                    log.error("Unable to find callback to process attributes response from WT")
    ...
    def gw_connect_device(self, device_name, device_type):
        log.debug("topic=%s data=%s" % (GATEWAY_MAIN_TOPIC + "connect", {"device": device_name, "type": device_type}))
    ...
    def gw_disconnect_device(self, device_name):
        log.debug("topic=%s data=%s" % (GATEWAY_MAIN_TOPIC + "disconnect", {"device": device_name}))
    ...
    def gw_send_rpc_reply(self, device, req_id, resp, quality_of_service=1):
        if quality_of_service != 0 and quality_of_service != 1:
            ...
            return
        log.debug("gw_send_rpc_reply topic=%s data=%s" % (GATEWAY_RPC_TOPIC, {"device": device, "id": req_id, "data": resp}))
