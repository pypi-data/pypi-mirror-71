...
from wthings_gateway.wt_client.wt_gateway_mqtt import WTGatewayMqttClient
import threading

log = logging.getLogger("wt_connection")


class WTClient(threading.Thread):
    def __init__(self, config):
        ...
        self.client = WTGatewayMqttClient(self.__host, self.__port, self.__token, self)
    ...
    def _on_connect(self, client, userdata, flags, rc, *extra_params):
        log.debug('WT client %s connected to WThings', str(client))
        self.__is_connected = True
        self.client._on_connect(client, userdata, flags, rc, *extra_params)

    def _on_disconnect(self, client, userdata, rc):
        log.info("WT client %s has been disconnected.", str(client))
        self.unsubscribe('*')
        self.__is_connected = False
        ...
        try:
            while not self.client.is_connected() and not self.__stopped:
                if not self.__paused:
                    if self.__stopped:
                        break
                    log.debug("connecting to WThings")
                    try:
