...
from wthings_gateway.storage.file_event_storage import FileEventStorage
from wthings_gateway.gateway.wt_gateway_remote_configurator import RemoteConfigurator

log = logging.getLogger('service')
main_handler = logging.handlers.MemoryHandler(-1)


class WTGatewayService:
    def __init__(self, config_dir, log_dir, token):
        self.__lock = RLock()
        self.init_path(config_dir=config_dir, log_dir=log_dir, token=token)
        config_file = self._config_dir + 'wt_gateway.yaml'
        ...
        if config["wthings"].get("remoteConfiguration"):
            try:
        ...
        self._send_thread = Thread(target=self.__read_data_from_storage, daemon=True,
                                   name="Send data to WThings Thread")
        ...
        try:
            ...
            while True:
                ...
                if self.__rpc_requests_in_progress and self.wt_client.is_connected():
                    ...
                    new_rpc_request_in_progress = {key: value for key, value in self.__rpc_requests_in_progress.items()
                                                   if value != 'del'}
                ...
                if not self.__request_config_after_connect and \
                        self.wt_client.is_connected() and not self.wt_client.client.get_subscriptions_in_progress():
                    self.__request_config_after_connect = True
                    self.__check_shared_attributes()

                if cur_time - gateway_statistic_send > 60000 and self.wt_client.is_connected():
                    summary_messages = self.__form_statistics()
                    # with self.__lock:
                    # log.info("send_telemetry............")
                    self.wt_client.client.send_telemetry(summary_messages)
        ...
        try:
            ...
            if content is not None:
                ...
                new_configuration = shared_attributes.get(
                    "remote_configuration") if shared_attributes is not None and shared_attributes.get(
                    "remote_configuration", None) is not None else content.get("configuration")
                log.info("shared_attributes=%s client_attributes=%s" % (shared_attributes, client_attributes))
                log.info("new_configuration=%s %s" % (new_configuration, type(new_configuration)))
                # new_configuration 网关扩展配置 __remote_configurator 远程配置实例
                # new_configuration 样例
                # [{"type":"OPC UA","configuration":{
                # "servers":[{"applicationName":"WThings OPC-UA client","applicationUri":"","host":"localhost",
                # "port":49320,"scanPeriodInSeconds":10,"timeoutInMillis":5000,"security":"None","identity":{
                # "type":"anonymous"},"keystore":{"type":"JKS","location":"example.pfx","password":"secret",
                # "alias":"gateway","keyPassword":"secret"},"mapping":[{"deviceNodePattern":"Channel1\\.Device\\d+$",
                # "deviceNamePattern":"Device ${_System._DeviceId}","attributes":[{"key":"temperature",
                # "type":"string","value":"${TemperatureAndHumiditySensor\\\\.Temperature}"}],"timeseries":[{
                # "key":"humidity","type":"string","value":"${
                # Root\\\\.Objects\\\\.Device1\\\\.TemperatureAndHumiditySensor\\\\.Humidity}"}]}]}]},"id":"OPC-UA
                # 01"}]
                if new_configuration is not None and self.__remote_configurator is not None:
                    try:
                        ...
                        # self._send_thread.start()
                        self.__remote_configurator.send_current_configuration()
                        self.on_applied_configuration(new_configuration)
                    except Exception as e:
                        log.exception(e)
    ...
    def on_applied_configuration(self, new_configuration):
        log.info("on_applied_configuration")
        self._published_events.put(
            self.wt_client.client.send_attributes({"appliedConfiguration": new_configuration}))

    def get_config_path(self):
        return self._config_dir

    def init_path(self, config_dir=None, log_dir=None, token=None):
        # Check system
        if not config_dir:
            if platform.system() == 'Windows':
                config_dir = os.path.expanduser('~') + os.sep + "wthings-gateway"
            else:
                config_dir = "/etc/wthings-gateway".replace('/', os.path.sep)
        if not log_dir:
            if platform.system() == 'Windows':
                log_dir = os.path.expanduser('~') + os.sep + "wthings-gateway"
            else:
                log_dir = "/var/log/wthings-gateway".replace('/', os.path.sep)
        self._log_dir = log_dir + os.sep + "logs" + os.sep
        # Check log directory
        if not os.path.exists(self._log_dir):
            os.makedirs(self._log_dir)
        target_path = config_dir + os.sep + "config"
        source_path = path.dirname(path.dirname(path.abspath(__file__))) + '/config'
        ...
        self._config_dir = target_path + os.sep
        with open(self._config_dir + 'wt_gateway.yaml') as general_config:
            config = safe_load(general_config)
        if token:
            config["wthings"]["security"]["accessToken"] = token
            with open(self._config_dir + 'wt_gateway.yaml', 'w') as general_config:
                safe_dump(config, general_config)
        else:
            if config["wthings"]["security"]["accessToken"] == "PUT_YOUR_GW_ACCESS_TOKEN_HERE":
                raise RuntimeError("Set token the first time you run it 'wthings-gateway -t xxxxxxx'")

    def set_log_path(self):
        log_config = configparser.ConfigParser()
        log_config.read(self._config_dir + 'logs.conf')
        _handlers = log_config.get("handlers", "keys").split(",")
        handler_prefix = "handler_"
        for handler in _handlers:
            handler = handler.strip()
            if handler:
                handler_section_name = handler_prefix + handler
                if handler != "consoleHandler":
                    log_file = self._log_dir + handler_section_name.replace("handler_", "").replace("Handler", "") + ".log"
                    handler_args = eval(log_config.get(handler_section_name, "args"))
                    handler_args = list(handler_args)
                    handler_args[0] = log_file
                    log_config.set(handler_section_name, "args", str(tuple(handler_args)))
        # 保存到文件 a = 追加，w = 写入
        with open(self._config_dir + 'logs.conf', "w") as file:
            log_config.write(file)

    def subscribe_to_required_topics(self):
        self.wt_client.client.gw_set_server_side_rpc_request_handler(self._rpc_request_handler)
        self.wt_client.client.set_server_side_rpc_request_handler(self._rpc_request_handler)
        self.wt_client.client.subscribe_to_all_attributes(self._attribute_update_callback)
        self.wt_client.client.gw_subscribe_to_all_attributes(self._attribute_update_callback)

    def __check_shared_attributes(self):
        self.wt_client.client.request_attributes(callback=self._attributes_parse)

    def _load_connectors(self, main_config):
        """
        加载class类到_implemented_connectors
        :param main_config:
        :return:
        """
        self.connectors_configs = {}
        if main_config.get("connectors") is None:
            # raise Exception("Configuration for connectors not found, check your config file.")
            log.error("Configuration for connectors not found, check your config file.")
            return
        for connector in main_config['connectors']:
            try:
                connector_class = WTUtility.check_and_import(connector["type"],
            ...
            for connector_config in self.connectors_configs[connector_type]:
                for config in connector_config["config"]:
                    ...
                    try:
                        ...
                        # print("设置连接器名称 connector=", connector, "connector.get_name()=", connector.get_name())
    ...
    def send_to_storage(self, connector_name, data):
        if not connector_name == self.name:
            if not WTUtility.validate_converted_data(data):
    ...
    def __read_data_from_storage(self):
        ...
        while True:
            try:
                if self.wt_client.is_connected():
                    ...
                    if events:
                        for event in events:
                            ...
                            if current_event.get("telemetry"):
                                if type(current_event["telemetry"]) == list:
                                    ...
                                    if not self.wt_client.is_connected(): break
                        ...
                        if devices_data_in_event_pack:
                            if not self.wt_client.is_connected(): break
                            self.__send_data(devices_data_in_event_pack)
                        if self.wt_client.is_connected() and (
                                        self.__remote_configurator is None or not self.__remote_configurator.in_process):
                            success = True
                            while not self._published_events.empty():
                                if (
                                                self.__remote_configurator is not None and self.__remote_configurator.in_process) or not self.wt_client.is_connected() or self._published_events.empty():
                                    break
                                event = self._published_events.get(False, 10)
                                try:
                                    if self.wt_client.is_connected() and (
                                                    self.__remote_configurator is None or not self.__remote_configurator.in_process):
                                        success = event.get() == event.WT_ERR_SUCCESS
                                    else:
                                        break
                                except Exception as e:
                                    log.exception(e)
        ...
        try:
            for device in devices_data_in_event_pack:
                if devices_data_in_event_pack[device].get("attributes"):
                    if device == self.name:
                        self._published_events.put(
                            self.wt_client.client.send_attributes(devices_data_in_event_pack[device]["attributes"]))
                    else:
                        self._published_events.put(self.wt_client.client.gw_send_attributes(device,
                                                                                            devices_data_in_event_pack[
                                                                                                device][
                                                                                                "attributes"]))
                if devices_data_in_event_pack[device].get("telemetry"):
                    if device == self.name:
                        self._published_events.put(
                            self.wt_client.client.send_telemetry(devices_data_in_event_pack[device]["telemetry"]))
                    else:
                        self._published_events.put(self.wt_client.client.gw_send_telemetry(device,
                                                                                           devices_data_in_event_pack[
                                                                                               device][
                                                                                               "telemetry"]))
    ...
    def rpc_with_reply_processing(self, topic, content):
        self.__rpc_requests_in_progress[topic][2](topic)
    ...
    def send_rpc_reply(self, device=None, req_id=None, content=None, success_sent=None):
        try:
            ...
            if device is not None and success_sent is not None:
                self.wt_client.client.gw_send_rpc_reply(device, req_id, dumps(rpc_response))
            elif device is not None and req_id is not None and content is not None:
                self.wt_client.client.gw_send_rpc_reply(device, req_id, content)
            elif device is None and success_sent is not None:
                self.wt_client.client.send_rpc_reply(req_id, dumps(rpc_response), quality_of_service=1)
            elif device is None and content is not None:
                self.wt_client.client.send_rpc_reply(req_id, content, quality_of_service=1)
        except Exception as e:
            log.exception(e)
    ...
    def add_device(self, device_name, content, wait_for_publish=False, device_type=None):
        if device_name not in self.__saved_devices:
            ...
            if wait_for_publish:
                self.wt_client.client.gw_connect_device(device_name, device_type).wait_for_publish()
            else:
                self.wt_client.client.gw_connect_device(device_name, device_type)
    ...
    def del_device(self, device_name):
        ...
        self.wt_client.client.gw_disconnect_device(device_name)
        self.__save_persistent_devices()
    ...
    def __save_persistent_devices(self):
        log.info("__save_persistent_devices=%s" % (self.__connected_devices,))
    ...
    def get_rpc_requests_in_progress(self):
        return self.__rpc_requests_in_progress


if __name__ == '__main__':
    WTGatewayService(
        path.dirname(path.dirname(path.abspath(__file__))) + '/config/wt_gateway.yaml'.replace('/', path.sep))
