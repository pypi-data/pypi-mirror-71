...
class RemoteConfigurator:
    ...
    def send_current_configuration(self):
        try:
            ...
            self.__gateway.wt_client.client.send_attributes(
            ...
            for connector in input_connector_config['wthings']['connectors']:
                for input_connector in input_connector_config[connector['type']]:
                    if input_connector['name'] == connector['name']:
                        ...
                        connector_class = WTUtility.check_and_import(connector["type"], self.__gateway._default_connectors.get(connector["type"], connector.get("class")))
    ...
    def __safe_apply_connection_configuration(self):
        ...
        self.__old_wt_client = self.__gateway.wt_client
        try:
            self.__old_wt_client.unsubscribe('*')
            self.__old_wt_client.stop()
            self.__old_wt_client.disconnect()
            self.__gateway.wt_client = WTClient(self.__new_general_configuration_file["wthings"])
            self.__gateway.wt_client.connect()
            log.debug("__safe_apply_connection_configuration wt_client connected")
            connection_state = False
            while time() * 1000 - apply_start < self.__apply_timeout * 1000 and not connection_state:
                connection_state = self.__gateway.wt_client.is_connected()
                sleep(.1)
    ...
    def __revert_configuration(self):
        try:
            ...
            self.__gateway.wt_client.disconnect()
            self.__gateway.wt_client.stop()
            self.__gateway.wt_client = WTClient(self.__old_general_configuration_file["wthings"])
            self.__gateway.wt_client.connect()
            self.__gateway.subscribe_to_required_topics()
            log.debug("%s connection has been restored", str(self.__gateway.wt_client.client._client))
        except Exception as e:
    ...
    def __update_logs_configuration(self):
        try:
            ...
            # self.__gateway.remote_handler = WTLoggerHandler(self.__gateway)
