...
class CustomRequestUplinkConverter(RequestConverter):
    ...
    def convert(self, topic, body):
        try:
            ...
            self.dict_result["deviceName"] = WTUtility.get_value(self.__config.get("deviceNameJsonExpression"), body, expression_instead_none=True)
            self.dict_result["deviceType"] = WTUtility.get_value(self.__config.get("deviceTypeJsonExpression"), body, expression_instead_none=True)
            ...
            if self.__config.get("extension-config") is not None:
                for telemetry_key in self.__config["extension-config"]:
                    ...
                    if value is not None:
                        ...
                        telemetry_to_send = {
                            telemetry_key["key"]: value}  # creating telemetry data for sending into WThings
