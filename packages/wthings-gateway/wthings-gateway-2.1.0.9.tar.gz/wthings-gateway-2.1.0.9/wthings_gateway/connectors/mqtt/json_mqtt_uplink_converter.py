    ...
    def convert(self, config, data):
        ...
        try:
            if self.__config.get("deviceNameJsonExpression") is not None:
                dict_result["deviceName"] = WTUtility.get_value(self.__config.get("deviceNameJsonExpression"), data)
            ...
            if self.__config.get("deviceTypeJsonExpression") is not None:
                dict_result["deviceType"] = WTUtility.get_value(self.__config.get("deviceTypeJsonExpression"), data)
        ...
        try:
            if self.__config.get("attributes"):
                for attribute in self.__config.get("attributes"):
                    attribute_value = WTUtility.get_value(attribute["value"], data, attribute["type"])
                    tag = WTUtility.get_value(attribute["value"], data, attribute["type"], get_tag=True)
        ...
        try:
            if self.__config.get("timeseries"):
                for ts in self.__config.get("timeseries"):
                    ts_value = WTUtility.get_value(ts["value"], data, ts["type"])
                    tag = WTUtility.get_value(ts["value"], data, ts["type"], get_tag=True)
