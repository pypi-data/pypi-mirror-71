...
class ModbusConnector(Connector, threading.Thread):
    ...
    def __load_converters(self):
        log.debug("config=", self.__config)
        try:
            for device in self.__config["devices"]:
                if self.__config.get("converter") is not None:
                    converter = WTUtility.check_and_import(self._connector_type, self.__config["converter"])(device)
                else:
                    converter = BytesModbusUplinkConverter(device)
                if self.__config.get("downlink_converter") is not None:
                    downlink_converter = WTUtility.check_and_import(self._connector_type, self.__config["downlink_converter"])(device)
                else:
        ...
        for device in self.__devices:
            ...
            try:
                for config_data in device_responses:
                    if self.__devices[device]["config"].get(config_data):
                        ...
                        if self.__devices[device]["next_"+config_data+"_check"] < current_time:
                            ...
                            for interested_data in range(len(self.__devices[device]["config"][config_data])):
                                ...
                                log.debug("current_data=%s unit_id=%s" % (current_data, unit_id))
                ...
                log.error("Connection lost! Trying to reconnect %s" % (threading.currentThread().ident),)
            except Exception as e:
                log.exception(e)
...
if __name__ == '__main__':
    master = ModbusSerialClient(method="rtu", port="com1", timeout=35, baudrate=19200)
    master.connect()
    from pymodbus.register_read_message import *
    request = ReadHoldingRegistersRequest(5, 1, unit=1)
    rr = master.execute(request)
    # rr = master.read_holding_registers(4, 1, unit=1)
    master.close()
