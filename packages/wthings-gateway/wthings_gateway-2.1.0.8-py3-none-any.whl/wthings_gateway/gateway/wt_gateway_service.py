...
class TBGatewayService:
    def __init__(self, config_dir, log_dir, token):
        ...
        self.tb_client = TBClient(config["wthings"])
        ...
        try:
            ...
            while True:
                ...
                if cur_time - gateway_statistic_send > 60000 and self.tb_client.is_connected():
    ...
    def on_applied_configuration(self, new_configuration):
        ...
        self._published_events.put(
            self.tb_client.client.send_attributes({"appliedConfiguration": new_configuration}))
                    ...
                    if events:
                        ...
                        if self.tb_client.is_connected() and (
                            ...
                            while not self._published_events.empty():
                                if (
                                                self.__remote_configurator is not None and self.__remote_configurator.in_process) or not self.tb_client.is_connected() or self._published_events.empty():
                                    break
