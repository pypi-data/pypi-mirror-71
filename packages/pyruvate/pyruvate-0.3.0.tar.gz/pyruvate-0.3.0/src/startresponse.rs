...
use cpython::{PyDict, PyClone, PyObject, PyResult, Python, PyTuple, PyList};
...
pub trait WriteResponse {
    ...
    fn new(environ: PyDict, headers_set: Vec<(String, Vec<(String, String)>)>, py: Python) -> PyResult<StartResponse>;
...
impl WriteResponse for StartResponse {
    ...
    fn new(environ: PyDict, headers_set: WSGIHeaders, py: Python) -> PyResult<StartResponse> {
        StartResponse::create_instance(py, environ, RefCell::new(headers_set), RefCell::new(Vec::new()), Cell::new(None), Cell::new(0))
    }
    ...
    fn write(&mut self, data: &[u8], output: &mut Vec<u8>, py: Python) {
        if self.headers_sent(py).borrow().is_empty() {
            ...
            self.headers_sent(py).replace(self.headers_set(py).borrow().clone());
