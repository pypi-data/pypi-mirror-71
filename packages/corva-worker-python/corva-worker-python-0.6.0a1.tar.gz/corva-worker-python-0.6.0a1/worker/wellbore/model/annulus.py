...
class Annulus(SectionsMixin):
    ...
    def create_annulus(self, drillstring: Drillstring, hole: Hole):
        ...
        for i in range(len(md_list) - 2):
            drillstring_section = drillstring.find_section_at_measured_depth(md_list[i] + epsilon)
            ...
            annulus_section.set_properties_from_hole(hole.find_section_at_measured_depth(md_list[i] + epsilon))
    ...
    def find_section_at_measured_depth(self, measured_depth: float) -> Tuple[Ann, int]:
        for index, ann in enumerate(self):
            if ann.top_depth < measured_depth <= ann.bottom_depth:
                return ann, index
