...
class OdooBus:
    ...
    # Default timeout
    odoo_bus_timeout = 300

    def start(self):
        # Set some options
        self.odoo_bus_timeout = int(
            __salt__['config.get']('odoo_bus_timeout', 300))
        # Start workers
        update_token_thread = threading.Thread(target=self.update_token)
