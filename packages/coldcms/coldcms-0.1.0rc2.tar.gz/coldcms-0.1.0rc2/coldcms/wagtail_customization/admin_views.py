...
def _is_not_minified(app_css_path):
    with open(app_css_path) as f:
        app_css_content = f.read()
    # More than 10 line feeds -> not minified
    return app_css_content.count("\n") > 10
...
def append_custom_css(app_css_path: str):
    ...
    if css_sheet and css_sheet.CSS_stylesheet:
        uploaded_file = css_sheet.CSS_stylesheet
        custom_css = uploaded_file.file.read()
...
def minify_css(page_paths: [str], app_css_path: str):
    ...
    if not os.path.exists(original_app_css) or _is_not_minified(app_css_path):
        shutil.copyfile(app_css_path, original_app_css)
    else:
        shutil.copyfile(original_app_css, app_css_path)

    append_custom_css(app_css_path)

    if not ORIGINAL_CSS_SIZE:
        ORIGINAL_CSS_SIZE = os.path.getsize(original_app_css)
