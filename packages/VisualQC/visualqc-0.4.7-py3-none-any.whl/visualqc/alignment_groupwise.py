...
Module to inspect the accuracy of spatial alignment (registration) across a group of images (subjects).
...
class GroupwiseAlignmentInterface(BaseReviewInterface):
    ...
    def __init__(self,
        ...
        self.show_ref_image_callback = show_ref_image_callback
    ...
    def add_radio_buttons_comparison_method(self):
        ...
        self.radio_bt_vis_type = RadioButtons(ax_radio, cfg.choices_alignment_comparison_groupwise,
...
class GroupwiseAlignmentRatingWorkflow(BaseWorkflowVisualQC, ABC):
    """
    ...
    def __init__(self,
                 ...
                 in_dir_type='generic',
                 prepare_first=True,
                 vis_type=cfg.alignment_groupwise_default_vis_type,
        ...
        self.ref_path = realpath(ref_path)
        self.in_dir_type = in_dir_type

        self.expt_id = 'rate_groupwise_alignment_{}_{}'.format(self.image_name, self.ref_path)
        self.suffix = self.expt_id
        self.current_alert_msg = None
        self.prepare_first = prepare_first

        self.open_figure(views, num_rows_per_view, num_slices_per_view)
        self.init_getters()
    ...
    def init_getters(self):
        ...
        self.path_getter_inputs = lambda sub_id: (path_joiner(sub_id, self.image_name),
                                                  self.ref_path)


    def open_figure(self, views, num_rows_per_view,
        ...
        self.fig.canvas.set_window_title('VisualQC Group-wise Alignment : {} {} {} '
                                         ''.format(self.in_dir, self.image_name, self.ref_path))
    ...
    def add_UI(self):
        ...
        self.UI = GroupwiseAlignmentInterface(self.fig, self.issue_list,
                                              ...
                                              show_ref_image_callback=self.show_first_image)
    ...
    def load_unit(self, unit_id):
        ...
        if np.count_nonzero(self.image_one) == 0:
            skip_subject = True
            print(
                'image {} of {} is empty!'.format(self.image_name, self.current_unit_id))

        if np.count_nonzero(self.image_two) == 0:
            skip_subject = True
            print(
                'image {} of {} is empty!'.format(self.ref_path, self.current_unit_id))
...
def make_workflow_from_user_options():
    ...
    wf = GroupwiseAlignmentRatingWorkflow(id_list,
                                          in_dir,
                                          image1,
                                          image2,
                                          out_dir=out_dir,
