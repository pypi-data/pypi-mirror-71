...
class Informatics(Ejudge):
    ...
    def __init__(self, url, login, password):
        if not url.endswith('#oldapi'):
            raise EJError("Not an informatics.msk.ru v1 URL")
        url = url[:-7]
        sp = url.split('/', 3)
        sp[2] = '.'.join(sp[2].split('.')[-3:])
        url = '/'.join(sp)
        ...
        for proto in ('http', 'https'):
            for domain in ('mccme', 'msk'):
                if (url+'/').startswith('%s://informatics.%s.ru/'%(proto, domain)):
                    for vt in (('', ''), ('', '3'), ('moodle/', ''), ('moodle/', '3')):
                        ...
                        raise EJError("URL must point to a contest or a task.")
                    if domain == 'mccme':
            ...
            raise EJError("Not an informatics.msk.ru URL")
    ...
    def download_file(self, prob_id, filename):
        raise EJError("File download doesn't exist on informatics.msk.ru")
    def clars(self):
        raise EJError("Clarifications don't exits on informatics.msk.ru")
    def submit_clar(self, *args):
        raise EJError("Clarifications don't exits on informatics.msk.ru")
    def read_clar(self, id):
        raise EJError("Clarifications don't exits on informatics.msk.ru")
    def get_samples(self, subm_id):
        raise EJError("Can't get samples on informatics.msk.ru")
    def scoreboard(self):
        raise EJError("Scoreboard is not supported on informatics.msk.ru")
