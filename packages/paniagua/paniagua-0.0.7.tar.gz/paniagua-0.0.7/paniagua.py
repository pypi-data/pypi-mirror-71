...
def df_colored(pandas_df, cmap='RdYlGn', reversed = False, axis=None):
    """This function returns a styled dataframe with conditional foramting"""
    if (reversed == True) and ('_r' not in cmap):
        cmap = cmap + '_r'
    elif (reversed == True) and ('_r' in cmap):
...
def df_colored_perc(pandas_df, cmap='RdYlGn', reversed = False ,axis=None):
    ...
    if (reversed == True) and ('_r' not in cmap):
        ...
        cmap = cmap
    x = pandas_df.style.format('{:,.1%}'.format).background_gradient(cmap=cmap,axis=axis)
    return x
