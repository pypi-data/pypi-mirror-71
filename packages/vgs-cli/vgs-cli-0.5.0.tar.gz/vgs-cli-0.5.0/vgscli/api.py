...
def create_api(tenant, environment, token):
    api = API(
        api_root_url=__resolve_api_url(tenant, environment, token),
...
def __resolve_api_url(tenant, environment, token):
    vault_management_api = accounts_api.create_api(token, environment)
    return accounts_api.get_api_url(tenant, vault_management_api, environment)
