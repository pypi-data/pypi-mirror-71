...
def get_function_id(function: Callable, func_name: str = '') -> str:
    ...
    name: str = func_name or function.__name__
...
def record_event(
    ...
    STACK.get().append(Frame(
        event=event,
        function=get_function_id(function, func_name),
        level=LEVEL.get(),
        ...
        if not do_trace or not DO_TRACE.get():
            ...
            async def wrapper(*args, **kwargs):
                ...
                with increase_counter(LEVEL):
                    record_event(clock_id, 'call', function, func_name)
                    result = await function(*args, **kwargs)
                    record_event(clock_id, 'return', function, func_name)
