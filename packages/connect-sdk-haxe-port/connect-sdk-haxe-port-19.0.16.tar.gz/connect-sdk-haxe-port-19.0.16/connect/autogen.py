...
class connect_api_Query(connect_Base):
    ...
    def toString(self):
        ...
        _g1 = 0
        while (_g1 < len(likeKeys)):
            key = (likeKeys[_g1] if _g1 >= 0 and _g1 < len(likeKeys) else None)
            _g1 = (_g1 + 1)
            x1 = (((("like(" + ("null" if key is None else key)) + ",") + HxOverrides.stringOrNull(self.like_.h.get(key,None))) + ")")
            rql.append(x1)
        _g2 = []
        k2 = self.ilike_.keys()
        while k2.hasNext():
            k3 = k2.next()
            _g2.append(k3)
        ilikeKeys = connect_api_Query.sortStringArray(_g2)
        _g3 = 0
        while (_g3 < len(ilikeKeys)):
            key1 = (ilikeKeys[_g3] if _g3 >= 0 and _g3 < len(ilikeKeys) else None)
            _g3 = (_g3 + 1)
            x2 = (((("ilike(" + ("null" if key1 is None else key1)) + ",") + HxOverrides.stringOrNull(self.ilike_.h.get(key1,None))) + ")")
            rql.append(x2)
        _g4 = []
        k4 = self.in__.keys()
        while k4.hasNext():
            k5 = k4.next()
            _g4.append(k5)
        inKeys = connect_api_Query.sortStringArray(_g4)
        _g5 = 0
        while (_g5 < len(inKeys)):
            key2 = (inKeys[_g5] if _g5 >= 0 and _g5 < len(inKeys) else None)
            _g5 = (_g5 + 1)
            _this1 = self.in__.h.get(key2,None)
            x3 = (((("in(" + ("null" if key2 is None else key2)) + ",(") + HxOverrides.stringOrNull(",".join([python_Boot.toString1(x1,'') for x1 in _this1]))) + "))")
            rql.append(x3)
        _g6 = []
        k6 = self.out_.keys()
        while k6.hasNext():
            k7 = k6.next()
            _g6.append(k7)
        outKeys = connect_api_Query.sortStringArray(_g6)
        _g7 = 0
        while (_g7 < len(outKeys)):
            key3 = (outKeys[_g7] if _g7 >= 0 and _g7 < len(outKeys) else None)
            _g7 = (_g7 + 1)
            _this2 = self.out_.h.get(key3,None)
            x4 = (((("out(" + ("null" if key3 is None else key3)) + ",(") + HxOverrides.stringOrNull(",".join([python_Boot.toString1(x1,'') for x1 in _this2]))) + "))")
            rql.append(x4)
        _g8 = []
        k8 = self.relOps.keys()
        while k8.hasNext():
            k9 = k8.next()
            _g8.append(k9)
        relOpsKeys = connect_api_Query.sortStringArray(_g8)
        _g9 = 0
        while (_g9 < len(relOpsKeys)):
            relOp = (relOpsKeys[_g9] if _g9 >= 0 and _g9 < len(relOpsKeys) else None)
            _g9 = (_g9 + 1)
            arguments = self.relOps.h.get(relOp,None)
            _g91 = 0
            while (_g91 < len(arguments)):
                argument = (arguments[_g91] if _g91 >= 0 and _g91 < len(arguments) else None)
                _g91 = (_g91 + 1)
                x5 = (((((("" + ("null" if relOp is None else relOp)) + "(") + HxOverrides.stringOrNull(argument.key)) + ",") + HxOverrides.stringOrNull(argument.value)) + ")")
                rql.append(x5)
...
class connect_util_Collection(connect_Base):
    ...
    def toString(self):
        if ((self.length() > 0) and Std._hx_is(self.get(0),connect_models_Model)):
            def _hx_local_0(el):
