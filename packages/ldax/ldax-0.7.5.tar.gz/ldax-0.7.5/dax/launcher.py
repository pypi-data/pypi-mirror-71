...
""" Edit by Shunxing Bao 09/05/2019 for optimizing dax build """
...
class Launcher(object):
    ...
    def __init__(self,
        ...
        :return: None
        """
        #LOGGER.warn('SHUNXING: class initialization')
...
cluster queue"
    ...
    # BUILD Main Method
    def build(self, lockfile_prefix, cachedir, project_local, sessions_local,
...
# - temporary remove this line for making build faster on individual session of BLSA - SHUNXING 11/27/2019
    ...
    def build_project(self, xnat, project_id, lockfile_prefix, sessions_local,
        ...
        #LOGGER.warn('SHUNXING build_project 1')
        proj_mods = self.project_modules_dict.get(project_id, None)
        #LOGGER.warn('SHUNXING build_project 2 proj_mods:%s' % str(proj_mods))
        proj_procs = self.project_process_dict.get(project_id, None)
        #LOGGER.warn('SHUNXING build_project 3 proj_procs:%s' % str(proj_procs))
        exp_mods, scan_mods = modules.modules_by_type(proj_mods)
        #LOGGER.warn('SHUNXING build_project 4 exp_mods:%s' % str(exp_mods))
        #LOGGER.warn('SHUNXING build_project 4 scan_mods:%s' % str(scan_mods))

        exp_procs, scan_procs = processors.processors_by_type(proj_procs)
        #LOGGER.warn('SHUNXING build_project 5 exp_procs:%s' % str(exp_procs))
        #LOGGER.warn('SHUNXING build_project 5 scan_procs:%s' % str(scan_procs))

        if mod_delta:
        #    LOGGER.warn('SHUNXING build_project 6')
            lastmod_delta = str_to_timedelta(mod_delta)
        else:
        #    LOGGER.warn('SHUNXING build_project 7')
            lastmod_delta = None

        # Check for new processors
        #LOGGER.warn('SHUNXING build_project 8')
        has_new = self.has_new_processors(xnat, project_id, exp_procs,
                                          scan_procs)

        # Get the list of sessions:
        #LOGGER.warn('SHUNXING build_project 9')
 ...
 last_mod=%s"
    ...
    def build_session(self, xnat, sess_info, sess_proc_list,
        ...
        #LOGGER.warn('SHUNXING try one more time')
        #csess = XnatUtils.CachedImageSession(xnat,
        #                                     sess_info['project_label'],
        #                                     sess_info['subject_label'],
        #                                     sess_info['session_label'])
        ...
        while mod_count < 3:
            ...
            if not sess_was_modified(xnat, sess_info, start_time):
                #LOGGER.warn('SHUNXING Build modules this if 3?')
                break

            #LOGGER.warn('SHUNXING Build modules this if 4?')
            csess.reload()
            mod_count += 1
            #LOGGER.warn('SHUNXING Build modules this if 5?')
    ...
    def build_session_processors(self, xnat, csess, sess_proc_list):
        ...
        #LOGGER.warn('sess_info %s' % str(sess_info['session_label']))
        res_dir = DAX_SETTINGS.get_results_dir()
        ...
        for sess_proc in sess_proc_list:
            ...
            #LOGGER.warn('p_assr %s' % str(p_assr))
            #LOGGER.warn('assr_name %s' % str(assr_name))
            #
            #
            ...
            if self.launcher_type in ['diskq-xnat', 'diskq-combined']:
                if p_assr is None or \
                   ...
                   p_assr.info()['qcstatus'] in [task.RERUN, task.REPROC]:
                    #LOGGER.warn('SHUNXING get assr_name: %s'% str(assr_name))
                    #if p_assr is not None:
                    #    LOGGER.warn('SHUNXING p_assr procstatus is %s ' % str(p_assr.info()['procstatus']) )
                    #    LOGGER.warn('SHUNXING p_assr qcstatus is %s ' %  str(p_assr.info()['qcstatus']))

                    assessor = csess.full_object().assessor(assr_name)                    

                    #LOGGER.debug('SHUNXING XnatTask initialization sess_proc: %s, assessor: %s, res_dir: %s' % (str(sess_proc),str(assessor),str(res_dir)))
                    ...
                    if p_assr is not None and \
                       p_assr.info()['qcstatus'] in [task.RERUN, task.REPROC]:
                        #LOGGER.warn('SHUNXING xtask.update_status')
                        xtask.update_status()

                    LOGGER.debug('building task:' + assr_name)
                    
                    #########SHUNXING add p_assr proc_status and qc_status to save time on grabing old (current) statuses
                    old_proc_status = None
                    old_qc_status = None

                    if p_assr is not None:
                        old_proc_status = p_assr.info()['procstatus']
                        old_qc_status = p_assr.info()['qcstatus']
                ...
                #LOGGER.warn('SHUNXING p_assr is None or task is NEED_INPUTS' )
                if p_assr is None or \
...
setting assessor status'
    ...
    def build_session_modules(self, xnat, csess, sess_mod_list):
        ...
        sess_obj = None
        sess_info = csess.info()
        #LOGGER.warn('SHUNXING sess_info: %s' % str(sess_info))
    ...
    def build_scan_processors(self, xnat, cscan, scan_proc_list):
        ...
        #LOGGER.warn('SHUNXING cscan type: %s' % str(type(cscan)))
        #LOGGER.warn('SHUNXING scan_info:%s' % str(scan_info))
        res_dir = DAX_SETTINGS.get_results_dir()
        ...
        for scan_proc in scan_proc_list:
            ...
            #LOGGER.warn('SHUNXING from scan_processors p_assr %s' % str(p_assr))
            #LOGGER.warn('SHUNXING from scan_processors assr_name %s' % str(assr_name))
            ...
            if self.launcher_type in ['diskq-xnat', 'diskq-combined']:
                if p_assr is None or \
                   ...
                   p_assr.info()['qcstatus'] in [task.RERUN, task.REPROC]:
                    ...
                    #LOGGER.warn('SHUNXING assessor object %s: ' % str(assessor))
                    #LOGGER.warn('SHUNXING res_dir:%s' % str(res_dir))
...
in session %s'
    ...
    def module_prerun(self, project_id, settings_filename=''):
        ...
        for mod in self.project_modules_dict.get(project_id, list()):
            try:
                #LOGGER.warn('SHUNXING mod.prerun')
...
bperhaps lo
    ...
    def get_sessions_list(xnat, project_id, slocal):
        ...
        """
        #LOGGER.warn('SHUNXING get_sessions_list 1')        
        list_sessions = XnatUtils.list_sessions(xnat, project_id)
        if slocal and slocal.lower() != 'all':
            # filter the list and keep the match between both list:
            #LOGGER.warn('SHUNXING get_sessions_list 2')
            val = slocal.split(',')
            #LOGGER.warn('SHUNXING get_sessions_list 3')
            list_sessions = [x for x in list_sessions if x['label'] in val]
            #LOGGER.warn('SHUNXING get_sessions_list 4')
        ...
        #LOGGER.warn('SHUNXING get_sessions_list 5')
        sorted_list = [s for s in list_sessions if not s['last_updated']]
        #LOGGER.warn('SHUNXING get_sessions_list 6')
        new_sessions_label = [sess['label'] for sess in sorted_list]
        #LOGGER.warn('SHUNXING get_sessions_list 7')
        for session in list_sessions:
            if not session['label'] in new_sessions_label:
                #LOGGER.warn('SHUNXING get_sessions_list 8 : %s'% str(session) )
    ...
    def has_new_processors(xnat, project_id, sess_proc_list, scan_proc_list):
        ...
        #LOGGER.warn('SHUNXING has_new_processors 1')
        assr_list = XnatUtils.list_project_assessors(xnat, project_id)
        
        type_of_assr_list = type(assr_list)
        #LOGGER.warn('SHUNXING len of assr_list:%d' % len(assr_list))
        #LOGGER.warn('SHUNXING type of assr_list:%s' % str(type_of_assr_list))
        #LOGGER.warn('SHUNXING type of assr_list element:%s' % str(type(assr_list[0]))) 
        #LOGGER.warn('SHUNXING has_new_processors 2')
        assr_type_set = set([x['proctype'] for x in assr_list])

        #findIndexOfAssr(assr_list, 'assessor_label','BLSA-x-BLSA_0038-x-BLSA_0038_22-5_01-x-dtiQA_Amico_Multi_v2')
        #findIndexOfAssr(assr_list, 'assessor_label','BLSA-x-BLSA_0038-x-BLSA_0038_22-5_01-x-dtiQA_v6')
        #findIndexOfAssr(assr_list, 'assessor_label','BLSA-x-BLSA_0038-x-BLSA_0038_22-5_01-x-bedpostx_v3')
        

        #delfindIndexOfAssr(assr_list, 'session_label', 'BLSA_0038_22-5_01')

        #hack get all scans
        #hack_project_scans = XnatUtils.list_project_scans (xnat, project_id)
        #print('SHUNXING hack_project_scans.len: %d\n\n' % len(hack_project_scans) )
        #with open('scans_list.txt', 'w') as filehandle:
        #    json.dump(hack_project_scans, filehandle)
        
        #print('SHUNXING type of hack_project_scans:%s\n\n' % str(type(hack_project_scans)))
        #print('SHUNXING element of hack_project_scans:%s\n\n' % str(hack_project_scans[0]))
        #findIndexOfScan(hack_project_scans, 'session_label','BLSA_0038_22-5_01')
        #findIndexOfScan(hack_project_scans, 'session_label','BLSA_0038_21-0_03')
        #findIndexOfScan(hack_project_scans, 'session_label','BLSA_0038_22-0_02')
        #findIndexOfScan(hack_project_scans, 'session_label','BLSA_0038_23-5_01')
        #findIndexOfScan(hack_project_scans, 'session_label','136927')
        #LOGGER.warn('SHUNXING assr_type_set:%s' % str(assr_type_set))
         
        # Get unique list of processors prescribed for project
        #LOGGER.warn('SHUNXING has_new_processors 3')
        proc_name_set = set([x.name for x in sess_proc_list + scan_proc_list])

        #LOGGER.warn('SHUNXING proc_name_set:%s' % str(proc_name_set))
        # Get list of processors that don't have assessors in XNAT yet
        #LOGGER.warn('SHUNXING has_new_processors 4')
        diff_list = list(proc_name_set.difference(assr_type_set))
        #LOGGER.warn('SHUNXING diff_list_len:%d' % len(diff_list))
        # Are there any?
        return len(diff_list) > 0

def findIndexOfScan(hack_project_scans, key, value):
    """
    SHUNXING
    find value according to key within assr_list
    
    assr_list is a dictionary list 
    """
    for i, dic in enumerate(hack_project_scans):
        if dic[key] == value:
         #   print('eeeeeeeeeeeeeeee')
         #   print(i)
            print(hack_project_scans[i].get('scan_id'))
    return -1

def findIndexOfAssr(assr_list, key, value):
    """
    SHUNXING
    find value according to key within assr_list
    ...
    assr_list is a dictionary list 
    """
    for i, dic in enumerate(assr_list):
        if dic[key] == value:
            print(assr_list[i])
    return -1

def delfindIndexOfAssr(assr_list, key, value):
    """
    SHUNXING
    find value according to key within assr_list
    ...
    for i, dic in enumerate(assr_list):
        if dic[key] == value:
            print(assr_list[i].get('assessor_label'))
    return -1
...
def get_sess_lastmod(xnat, sess_info):
    ...
    xsi_type = sess_info['xsiType']
    #LOGGER.warn('SHUNXING get_sess_lastmod 1')
    sess_obj = XnatUtils.get_full_object(xnat, sess_info)
    #LOGGER.warn('SHUNXING get_sess_lastmod 2')
    #LOGGER.warn('sess_obj:%s' % str(sess_obj))
    #LOGGER.warn('sess_info %s' % str(sess_info))
    #aaa = type(sess_info)
    #print('sess_info type:%s' % str(aaa))
    last_modified_xnat = sess_obj.attrs.get('%s/meta/last_modified' % xsi_type)
    #aaa = type(last_modified_xnat)
    #print('aaa %s : ' % str(aaa))
    #LOGGER.warn('SHUNXING last_modified : %s' % last_modified_xnat) 
    #LOGGER.warn('SHUNXING get_sess_lastmod 3')
...
def sess_was_modified(xnat, sess_info, build_start_time):
    ...
    :return: False if the session change and don't set the last update date,
             True otherwise
    Modified by Shunxing
    """
    # COMMENT BY SHUNXING
    # sess_info already contained last_modified, why call get_sesslastmod?
    # get_sess_lastmod should be ignored... 
    # COMMENT END
    ...
    #print('sess_info : %s' % str(sess_info) )
    #print('sess_info type : %s' % str(type(sess_info)))
