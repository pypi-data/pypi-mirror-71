...
from http.cookies import _quote as quote_cookie, _unquote as unquote_cookie
...
def inject_args(func, injections, args, kw, start=0):
    ...
    for argname, argval in injections.items():
...
def to_unicode(value):
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            try:
                value = str(value, get_template_encoding_default())
            except UnicodeDecodeError:
                # fail early
...
def fixentities(htmltext):
    ...
    """
    def repl(matchobj):
        entity = html.entities.entitydefs.get(matchobj.group(1).lower())
        if not entity:
