...
def genshi_et(element):
    ...
    if hasattr(element, 'tag'):
        ...
        return chain(*map(genshi_et, element))
    else:
        return element
...
def _get_plugin_options(plugin_name, defaults=None):
    ...
    if defaults is not None:
        options = dict((k, v) for k, v in list(defaults.items())
...
def load_engines():
    ...
    engine_defaults = {
        ...
        'genshi.new_text_syntax': True,
        'genshi-text.default_format': 'text',
