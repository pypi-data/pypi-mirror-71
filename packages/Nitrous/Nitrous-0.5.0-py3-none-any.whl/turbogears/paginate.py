...
def paginate(var_name, default_order='', limit=10,
    ...
    def entangle(func):
        ...
        def decorated(func, *args, **kw):
            ...
            page_count = int(ceil(old_div(float(row_count),limit_)))
            ...
            for input_key in list(input_values.keys()):
            ...
            if _simulate_offset:
                ...
                try:
                    ...
                    for i in range(offset):
                        next(var_data)
                    output[var_name] = [next(var_data)
                        for i in range(offset, endpoint)]
