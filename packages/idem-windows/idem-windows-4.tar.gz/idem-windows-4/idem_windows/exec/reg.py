"""
TODO put the commit number where this forked from winreg in salt and the exact file path in salt that this came from
...
def __init__(hub):
    ...
    hub.exec.reg.VTYPE_REVERSE = {v: k for k, v in hub.exec.reg.VTYPE.items()}

    hub.exec.reg.OPTTYPE = {"REG_OPTION_NON_VOLATILE": 0, "REG_OPTION_VOLATILE": 1}

    # delete_key_recursive uses this to check the key contains enough \
...
def key_exists(hub, hive: str, key: str, use_32bit_registry: bool = False) -> bool:
    """
    ...
    """
    try:
        hkey = hub.exec.reg.HKEYS[hive]
    except KeyError:
        raise KeyError(f"Invalid Hive: {hive}")
    access_mask = hub.exec.reg.REGISTRY_32[use_32bit_registry]

    handle = None
    try:
        handle = win32api.RegOpenKeyEx(hkey, key, 0, access_mask)
        return True
...
def value_exists(
    hub, hive: str, key: str, vname: str, use_32bit_registry: bool = False
) -> bool:
    """
    ...
    try:
        ...
        _, _ = win32api.RegQueryValueEx(handle, vname)
...
def list_keys(hub, hive: str, key: str = None, use_32bit_registry: bool = False):
    """
    ...
    try:
        ...
        hub.log.debug(r"Cannot find key: %s\%s", hive, key, exc_info=True)
...
def list_values(
    hub, hive: str, key: str = None, use_32bit_registry: bool = False,
):
    """
    ...
    try:
        ...
        for i in range(win32api.RegQueryInfoKey(handle)[1]):
            ...
            value = {
                ...
                "key": key,
                "vname": vname,
                "vtype": hub.exec.reg.VTYPE_REVERSE[vtype],
                "success": True,
            }
        ...
        hub.log.debug(r"Cannot find key: %s\%s", hive, key, exc_info=True)
        return False, f"Cannot find key: {hive}\\{key}"
    finally:
        if handle:
            handle.Close()
    return values
...
def read_value(
    hub, hive: str, key: str, vname: str = None, use_32bit_registry: bool = False
) -> Dict[str, Any]:
    r"""
    ...
    ret = {"hive": hive, "key": key, "vname": vname, "vdata": None, "success": True}
    ...
    try:
        ...
        try:
            # RegQueryValueEx returns and accepts unicode data
            vdata, vtype = win32api.RegQueryValueEx(handle, vname)
            if vdata or vdata in [0, ""]:
                # Only convert text types to unicode
                ret["vtype"] = hub.exec.reg.VTYPE_REVERSE[vtype]
                ret["vdata"] = vdata
            else:
            ...
            if exc.winerror == 2 and vname is None:
                ...
                msg = "Cannot find {0} in {1}\\{2}" "".format(vname, hive, key)
                hub.log.trace(exc)
                hub.log.trace(msg)
                ret["comment"] = msg
                ret["success"] = False
    except Exception as exc:  # pylint: disable=E0602
        msg = "Cannot find key: {0}\\{1}".format(hive, key)
        hub.log.trace(exc)
        hub.log.trace(msg)
        ret["comment"] = msg
        ret["success"] = False
...
def set_value(
    ...
    try:
        ...
        raise KeyError(f"Invalid Hive: {hive}")
    vtype_value = hub.exec.reg.VTYPE[vtype]
    access_mask = hub.exec.reg.REGISTRY_32[use_32bit_registry] | win32con.KEY_ALL_ACCESS

    vdata = cast_vdata(vdata=vdata, vtype=vtype)

    if volatile:
        create_options = hub.exec.reg.OPTTYPE["REG_OPTION_VOLATILE"]
    else:
        create_options = hub.exec.reg.OPTTYPE["REG_OPTION_NON_VOLATILE"]

    handle = None
    try:
        handle, _ = win32api.RegCreateKeyEx(
            hkey, key, access_mask, Options=create_options
        )
        win32api.RegSetValueEx(handle, vname, 0, vtype_value, vdata)
        ...
        hub.log.exception("Encountered error setting registry value")
        return False
    finally:
        if handle:
            win32api.RegCloseKey(handle)
...
def cast_vdata(hub, vdata: str = None, vtype: str = "REG_SZ") -> Any:
    """
...
def _traverse_registry_tree(
    _hkey: str, _keypath: str, _ret: Dict[str, Any], _access_mask: int
) -> List[str]:
    """
    ...
    for subkeyname in _subkeys(_key):
        ...
        _ret = _traverse_registry_tree(_hkey, subkeypath, _ret, _access_mask)
        _ret.append(subkeypath)
    return _ret


def delete_key_recursive(
    hub, hive: str, key: str, use_32bit_registry: bool = False
) -> Dict[str, Any]:
    """
    ...
    if not key_exists(hive, key, use_32bit_registry):
        return False

    if (len(key) > 1) and (key.count("\\", 1) < hub.exec.reg.SUBKEY_SLASH_CHECK[hkey]):
        hub.log.error(
            "Hive:%s Key:%s; key is too close to root, not safe to remove", hive, key
        )
        return False
    ...
    for sub_key_path in key_list:
        try:
            ...
            hub.log.error(exc, exc_info=True)
...
def delete_value(
    ...
    try:
        handle = win32api.RegOpenKeyEx(hkey, key, 0, access_mask)
        win32api.RegDeleteValue(handle, vname)
        broadcast_change()
        return True
    except Exception as exc:  # pylint: disable=E0602
        if exc.winerror == 2:
            ...
            hub.log.error(exc, exc_info=True)
            hub.log.error("Hive: %s", hive)
            hub.log.error("Key: %s", key)
            hub.log.error("ValueName: %s", vname)
            hub.log.error("32bit Reg: %s", use_32bit_registry)
            return False
    finally:
        if handle:
            win32api.RegCloseKey(handle)
