...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':nfgMK(perm='m',auth=nfgMv,arg=nfgMv,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=nfgMv)
 time.sleep(3)
def do_start_ftp(port,asynchronous=nfgMv):
...
def main():
 do_start_ftp(nfgMU(sys.argv[1]),asynchronous=nfgMe)
