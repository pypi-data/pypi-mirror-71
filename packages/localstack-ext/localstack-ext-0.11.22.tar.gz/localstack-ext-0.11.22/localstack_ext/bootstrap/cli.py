...
def cmd_daemons(argv,args):
 ...
 if args['<subcommand>']in[xNubr,'start']:
...
def register_commands():
 config.CLI_COMMANDS['daemons']={'command':'  localstack daemons','description':'Manage local daemon processes','parameters':[],'function':cmd_daemons}
 return
...
def import_auth():
 try:
  ...
  raise xNubv('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=xNubE)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=xNubh)
def get_params(args,names=[],mandatory=xNubE):
 result=()
 for name in names:
  value=xNubr
  if name in args:
  ...
  if not value and mandatory:
   raise xNubv('Please provide %s=... parameter'%name)
  result+=(value)
 if xNubB(result)==1:
  return result[0]
 return result
