...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':QlHuy(perm='m',auth=QlHuw,arg=QlHuw,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=QlHuw)
 time.sleep(3)
def do_start_ftp(port,asynchronous=QlHuw):
...
def main():
 do_start_ftp(QlHuo(sys.argv[1]),asynchronous=QlHuz)
