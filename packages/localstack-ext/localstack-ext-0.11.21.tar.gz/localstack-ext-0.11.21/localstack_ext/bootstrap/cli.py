...
def import_auth():
 try:
  ...
  raise mRSKt('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=mRSKW)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=mRSKP)
def get_params(args,names=[],mandatory=mRSKW):
 result=()
 for name in names:
  value=mRSKb
  if name in args:
  ...
  if not value and mandatory:
   raise mRSKt('Please provide %s=... parameter'%name)
  result+=(value)
 if mRSKl(result)==1:
  return result[0]
 return result
