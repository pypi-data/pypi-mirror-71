...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':PrQyA(perm='m',auth=PrQyn,arg=PrQyn,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=PrQyn)
def do_start_ftp(port,asynchronous=PrQyn):
...
def main():
 do_start_ftp(PrQyb(sys.argv[1]),asynchronous=PrQyB)
