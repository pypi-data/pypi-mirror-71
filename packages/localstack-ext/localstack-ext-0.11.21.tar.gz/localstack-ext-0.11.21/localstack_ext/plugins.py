...
def register_localstack_plugins():
 ...
 EXTERNAL_PORT_APIS=('rds','elasticache','ecs','athena','apigateway','codecommit','transfer')
 if gjqli([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if 'No module named' not in gjqlo(e):
   print(e)
def _setup_logging():
