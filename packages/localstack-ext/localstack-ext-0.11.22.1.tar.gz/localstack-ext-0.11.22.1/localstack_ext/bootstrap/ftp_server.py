...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':QRhOa(perm='m',auth=QRhOS,arg=QRhOS,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=QRhOS)
 time.sleep(3)
def do_start_ftp(port,asynchronous=QRhOS):
...
def main():
 do_start_ftp(QRhOY(sys.argv[1]),asynchronous=QRhOE)
