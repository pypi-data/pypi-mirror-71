...
class BinoutData(object):
    ...
    def _read_data_types(self, elem_name=None):
        ...
        if isinstance(ids[0], tuple):
            for i in range(0, ids.shape[0]):
                ...
                if indices[0].size != 0:
                    insert_indices = np.append(insert_indices, indices[0] + len(ids[0]) * i - np.arange(len_indices, len_indices + len(indices[0])))
                    ind = np.append(ind, indices[0] + len(ids[0]) * i)
                    len_indices += len(indices[0])

        available_channels = [value for value in self.__class__.DATA_CHANNEL_TYPES if value in self._binout.read(*args_)]
    ...
    def get_data_of_defined_json(self, json_object, data_offsets):
        ...
        if (len(ids) == 0):
            if "ID" in json_object:
                self._logger.emit(LOGConstants.ERROR[0], "Missing ID in the input data with identifier: " + json_object["ID"])
        ...
        if array_definition[0].split(',')[0].strip(' (') == "all":
            converted_tuples = [(index, array_definition[0].split(',')[1].strip(' )')) for index, id_ in enumerate("ids")]
        else:
        ...
        for tpl in converted_tuples:
            d = self.get_measurement_channel(id=ids[tpl[0]], channel_name=tpl[1])
    ...
    def get_measurement_channel(self, id, channel_name):
        ...
        if np.issubdtype(self._ids.dtype, np.number):
            ...
            data_index = np.where(id == self._ids)[0]

        if len(data_index) == 0:
            self._logger.emit(LOGConstants.ERROR[0], 'ID ' + str(id) + ' not in binout')
            exit()
