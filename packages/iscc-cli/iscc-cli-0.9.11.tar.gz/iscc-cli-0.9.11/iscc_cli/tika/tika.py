...
Translator = os.getenv(
    "TIKA_TRANSLATOR", "org.apache.tika.language.translate.Lingo24Translator"
)
...
def runCommand(
    ...
    if cmd == "parse":
        return parseAndSave(
            option, urlOrPaths, outDir, serverEndpoint, verbose, tikaServerJar
        )
    elif cmd == "detect":
    ...
    headers.update(
        {
            ...
            "Content-Disposition": make_content_disposition_header(
                path.encode("utf-8") if type(path) is unicode_string else path
            ),
        }
    )
...
def detectLang1(
    ...
    if option not in services:
        log.exception(
            "Language option must be one of %s " % binary_string(services.keys())
        )
        raise TikaException(
            "Language option must be one of %s " % binary_string(services.keys())
        )
...
def doTranslate1(
    ...
    if ":" in option:
        ...
        if len(options) != 2:
            log.exception(
                "Translate options are specified as srcLang:destLang or as destLang"
            )
            raise TikaException(
                "Translate options are specified as srcLang:destLang or as destLang"
            )
...
def detectType1(
    ...
    if option not in services:
        log.exception(
            "Detect option must be one of %s" % binary_string(services.keys())
        )
        raise TikaException(
            "Detect option must be one of %s" % binary_string(services.keys())
        )
...
def callServer(
    ...
    if not TikaClientOnly:
        serverEndpoint = checkTikaServer(
            scheme, serverHost, port, tikaServerJar, classpath, config_path
        )

    serviceUrl = serverEndpoint + service
    if verb not in httpVerbs:
        log.exception(
            "Tika Server call must be one of %s" % binary_string(httpVerbs.keys())
        )
        raise TikaException(
            "Tika Server call must be one of %s" % binary_string(httpVerbs.keys())
        )
...
def startServer(
    ...
    if not config_path:
        cmd_string = (
            '%s %s -cp "%s" org.apache.tika.server.TikaServerCli --port %s --host %s &'
            % (java_path, java_args, classpath, port, host)
        )
    else:
        cmd_string = (
            '%s %s -cp "%s" org.apache.tika.server.TikaServerCli --port %s --host %s --config %s &'
    ...
    try:
        ...
        log.error(
            "Unable to create tika-server.log at %s due to permission error."
    ...
    if Windows:
        if sys.version.startswith("2"):
            # Python 2.x
            TikaServerProcess = Popen(
                cmd_string, stdout=logFile, stderr=STDOUT, shell=True
            )
        ...
        TikaServerProcess = Popen(
            cmd_string, stdout=logFile, stderr=STDOUT, shell=True, preexec_fn=os.setsid
        )
    ...
    if not is_started:
        log.error(
            "Tika startup log message not received after %d tries."
            % (TikaStartupMaxRetry)
        )
        return False
    else:
