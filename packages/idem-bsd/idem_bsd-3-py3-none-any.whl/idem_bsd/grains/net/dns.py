...
async def load_dns(hub):
    ...
    resolv.read_resolv_conf("/etc/resolv.conf")
    resolv.read_resolv_conf("/var/etc/resolv.conf")
    ...
    hub.grains.GRAINS.dns.sortlist = await _get_sortlist(resolv.nameservers)
    domain = str(resolv.domain)
    hub.grains.GRAINS.dns.domain = domain if domain.strip(".") else ".localdomain"
