...
def haz(X0, W0, Y0, T, P, model, good_initial = False,
    ...
    if not good_initial:
        out = multiflash(x0, b0, ['L','L','V'], Z0, T, P, model, v0, K_tol, True)
    else:
        global vx, vw, vy
        sol = fsolve(haz_objt, x0.flatten() ,args = (T, P, model, v0))
        x0 = sol.reshape([model.nc, 3])
        Z0 = x0.mean(axis=0)
        out = multiflash(x0, b0, ['L','L','V'], Z0, T, P, model, [vx,vw,vy], K_tol, True)
    ...
    if error_inner > 1e-6:
        ...
        out = multiflash(Xm, betatetha, equilibrio, Z0, T, P, model, v0 , K_tol, full_output = True)
        order = [1, 2, 0]
        Xm, beta, tetha, equilibrio = out.X, out.beta, out.tetha, out.states
        error_inner =  out.error_inner
        if error_inner > 1e-6:
...
def vlle(X0, W0, Y0, Z, T, P, model, v0 = [None, None, None], K_tol =1e-10, full_output = False):
    ...
    if error_inner > 1e-6:
        ...
        equilibrio = np.asarray(equilibrio)[order]
        v0 = np.asarray(v)[order]
        out = multiflash(Xm, betatetha, equilibrio, Z, T, P, model, v0 , K_tol, full_output = True)
        order = [1, 2, 0]
        Xm, beta, tetha, equilibrio = out.X, out.beta, out.tetha, out.states
        error_inner =  out.error_inner
        if error_inner > 1e-6:
