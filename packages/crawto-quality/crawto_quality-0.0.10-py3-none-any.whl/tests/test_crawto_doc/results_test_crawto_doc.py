...
class CrawtoDocBase:
    ...
    Attributes
    ----------
    docs_offset : <#TODO type definition>
    ...
    def check_dunder(function_name: str):
        ...
        """
        return re.fullmatch('__[a-z]+__', function_name)
...
class CrawtoClass(CrawtoDocBase):
    ...
    keywords : <#TODO type definition>
        <#TODO Description>

    lineno : <#TODO type definition>
        <#TODO Description>

    name : str
        <#TODO Description>

    doc_string : Optional, default=None
        <#TODO Description>

    Attributes
    ----------
    init : <#TODO type definition>
        <#TODO Attribute Description>

    attributes : <#TODO type definition>
        <#TODO Attribute Description>

    bases : <#TODO type definition>
        <#TODO Attribute Description>

    methods : <#TODO type definition>
        <#TODO Attribute Description>

    example : <#TODO type definition>
        <#TODO Attribute Description>

    docs : <#TODO type definition>
        <#TODO Attribute Description>

    method_docs_dict : <#TODO type definition>
        <#TODO Attribute Description>

    """

    def __init__(self, *, bases: List, body: List, col_offset: int,
        decorator_list: List, end_col_offset: int, end_lineno: int,
        keywords, lineno, name: str, doc_string: Optional[str]=None):
    ...
    def init(self):
        for i in self.ast_body:
            if type(i) is ast.FunctionDef and i.name == '__init__':
                self._init = CrawtoFunction(**i.__dict__, doc_string=ast.
                    get_docstring(i), class_=self.ast_name)
    ...
    def attributes(self):
        ...
        assigns = [assign for method in methods for assign in method.
            ast_body if type(assign) is ast.Assign]
        attrs = [target.attr for targets in assigns for target in targets.
            targets if type(target) is ast.Attribute]
        for i in attrs:
            self._attributes += (
    ...
    @property
    def methods(self):
        self._methods = [CrawtoFunction(**i.__dict__, doc_string=ast.
            get_docstring(i), is_method=True) for i in self.ast_body if 
            type(i) is ast.FunctionDef and not self.check_dunder(i.name) and
            not self.check_internal_method(i.name) and not self.
            check_property_decorators(i)]
...
class CrawtoFunction(CrawtoDocBase):
    ...
    returns : <#TODO type definition>
        <#TODO Description>

    type_comment : <#TODO type definition>
        <#TODO Description>
    ...
    class_ : <#TODO type definition>, default=None
        <#TODO Description>

    Attributes
    ----------
    args : <#TODO type definition>
        <#TODO Attribute Description>

    parameters : <#TODO type definition>
        <#TODO Attribute Description>
    ...
    def __init__(self, *, args: List, body: List, col_offset: int,
        decorator_list: List, end_col_offset: int, end_lineno: int, lineno:
        int, name: str, returns, type_comment, doc_string: Optional[str]=
        None, is_method: bool=False, class_=None):
        self.ast_args = args
        self.ast_body = body
        self.ast_col_offset = col_offset
        self.ast_decorator_list = decorator_list
...
class CrawtoArgs:
    ...
    Attributes
    ----------
    arg_list : <#TODO type definition>
        <#TODO Attribute Description>

    example_inputs : <#TODO type definition>
        <#TODO Attribute Description>

    Examples
    --------
    >>> from crawto-quality import crawto_doc
    >>> example = CrawtoArgs(posonlyargs=<#TODO Example Value>, args=<#TODO Example Value>, vararg=<#TODO Example Value>, kwonlyargs=<#TODO Example Value>, kw_defaults=<#TODO Example Value>, kwarg=<#TODO Example Value>, defaults=<#TODO Example Value>)
    """

    def __init__(self, posonlyargs: List, args: List, vararg: Optional[ast.
        arg], kwonlyargs: List, kw_defaults: List, kwarg: Optional[ast.arg],
        defaults: List):
    ...
    def arg_list(self):
        ...
        arg_default = [Arg(name=k.arg, ast_annotation=k.annotation,
            default_input=v) for k, v in zip_longest(args, defaults,
            fillvalue='no_default') if k.arg != 'self'][::-1]
        if self.ast_vararg is not None:
            vararg = [Arg(name=self.ast_vararg.arg, ast_annotation=self.
                ast_vararg.annotation, default_input='no_default')]
        else:
            vararg = []
        if self.ast_kwarg is not None:
            kwargs = [Arg(name=self.ast_kwarg.arg, ast_annotation=self.
                ast_kwarg.annotation, default_input='no_default')]
        else:
            kwargs = []
        kwonlyargs = self.ast_kwonlyargs
        kw_defaults = self.ast_kw_defaults
        keywords = [Arg(name=k.arg, ast_annotation=k.annotation,
            default_input=v) for k, v in zip(kwonlyargs, kw_defaults) if k.
            arg != 'self']
    ...
    def example_inputs(self):
        self._example_inputs = ', '.join([f'{i.name}={i.example}' for i in
            self.arg_list])
...
class Arg:
    ...
    ast_annotation : Optional
        <#TODO Description>

    Attributes
    ----------
    default : <#TODO type definition>
        <#TODO Attribute Description>

    example : <#TODO type definition>
        <#TODO Attribute Description>

    type_def : <#TODO type definition>
        <#TODO Attribute Description>

    type_annotation : <#TODO type definition>
        <#TODO Attribute Description>
    ...
    def type_annotation(self):
        if type(self.ast_annotation) is ast.Subscript or type(self.
            ast_annotation) is ast.Attribute:
...
def add_class_doc_to_ast(ast_class: ast.ClassDef) ->None:
    ...
    """
    classdef = CrawtoClass(**ast_class.__dict__, doc_string=ast.
        get_docstring(ast_class))
...
def add_function_doc_to_ast(ast_function: ast.FunctionDef) ->None:
    ...
    <#TODO Method Return Value>
    """
    functiondef = CrawtoFunction(**ast_function.__dict__, doc_string=ast.
        get_docstring(ast_function))
...
def create_documentation(filename: str, new_filename: str) ->str:
    ...
    --------
    >>> from crawto-quality import crawto_doc
    >>> create_documentation(filename=<#TODO Example Value>, new_filename=<#TODO Example Value>)
    <#TODO Method Return Value>
    """
...
class DocStringParser:
    ...
    --------
    >>> from crawto-quality import crawto_doc
    >>> example = DocStringParser(doc_string=<#TODO Example Value>)
    >>> example.parse_attr_param_doc_string(doc_string_part=<#TODO Example Value>)
    <#TODO Method Return Value>
    """
...
""")
        ...
        if (self.description and self.parameter_string and self.
            attribute_string and self.example_string):
    ...
    def parse_attr_param_doc_string(doc_string_part: str) ->List:
        ...
        return [list(map(lambda x: x.strip(), (i.split(':')[0], *i.split(
            ':')[1].split(', default=')))) for i in param_list if i]
...
class Attribute:
    ...
    default_value : str, default=no_default
        <#TODO Description>
