...
class CrawtoDocBase:
    ...
    @staticmethod
    def check_property_decorators(ast_function: ast.FunctionDef) -> Optional[str]:
        for i in ast_function.decorator_list:
            if type(i) is ast.Name and i.id == "property":
                return ast_function.name
...
class CrawtoClass(CrawtoDocBase):
    ...
    def attributes(self):
        ...
        attrs = [
            Attribute(name=target.attr)
            for targets in assigns
            for target in targets.targets
            if type(target) is ast.Attribute
        ]
        for i in attrs +self.property_:
            self._attributes += (
                f"""\n{self.docs_offset}{i.name} : {i.type_def}\n{self.docs_offset +"    "}<#TODO Attribute Description>\n"""
            )
    ...
    @property
    def property_(self):
        # holds all @property and @x.setter for now
        self._property = []
        names = set()
        for i in self.ast_body:
            if (
                ...
                and self.check_property_decorators(i)
                and i.name not in names
            ):
                self._property.append(Attribute(name=i.name, ast_annotation=i.returns))
                names.add(i.name)
...
class CrawtoDataClass(CrawtoClass):
    @property
    def parameters(self):
        self._parameters = (
        ...
        ann_assign = [
            Attribute(
                name=i.target.id, ast_annotation=i.annotation, default_input=i.value
            )
            for i in self.ast_body
            if type(i) is ast.AnnAssign
        ]
        for i in ann_assign:
            self._parameters += f"""\n{self.docs_offset}{i.name} : {i.type_annotation}{i.default}\n{self.docs_offset +"    " }<#TODO Description>\n"""
...
class CrawtoFunction(CrawtoDocBase):
    ...
    def parameters(self):
        ...
        for i in self.args.arg_list:
            self._parameters += (
                f"""\n{self.docs_offset}{i.name} : {i.type_def}{i.default}\n{self.docs_offset +"    "}<#TODO Description>\n"""
            )
...
class Arg:
    ...
    ast_annotation: Optional[Any]

    @property
    def default(self):
        # if self.default_input is type(ast.Constant) and self.default_input.value == "None":
        #    self._default = f", default=None"
        if self.default_input and self.default_input != "no_default":
...
class Attribute(Arg):
    name: str
    ast_annotation: Any = None
    default_input: str = "no_default"


def add_class_doc_to_ast(ast_class: ast.ClassDef) -> None:
    if "dataclass" in [i.id for i in ast_class.decorator_list]:
        classdef = CrawtoDataClass(
