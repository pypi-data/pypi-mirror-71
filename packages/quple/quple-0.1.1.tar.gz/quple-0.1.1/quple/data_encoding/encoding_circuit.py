...
class EncodingCircuit(EntanglementCircuit, ABC):
    ...
    def __init__(self, feature_dimension:int, depth:int=1,
        ...
        self._parameter_symbol = parameter_symbol or 'x'
    ...
    def get_parameters(self, indices:List[int]):
        ...
        (x[1], x[2])
        '''
        return self._parameter_table.get_array(self._parameter_symbol, indices) 
    ...
    def encode_parameters(self, indices:List[int]):
        ...
        mapped_value = self.encoding_map(parameters)
        return mapped_value
    
    def _input_values_to_dict(self, x:np.ndarray):
        return {'x[{}]'.format(i):value for i,value in enumerate(a)}
