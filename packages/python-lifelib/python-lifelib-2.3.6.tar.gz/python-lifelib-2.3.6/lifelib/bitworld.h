...
namespace apg {
    ...
    std::vector<bitworld> rle2vec(std::string rle, RuleMapper *rm) {
        ...
        for (unsigned int i = 0; i < rle.size(); i++) {
            ...
            if ((c >= '0') && (c <= '9')) {
                ...
                uint64_t bp = 0;
                if (rm != 0) { colour = rm->golly_to_ll(colour); }
                while (colour > 0) {
                    if (colour & 1) {
                        if (bp >= planes.size()) { planes.resize(bp+1); }
    ...
    std::vector<bitworld> rle2vec(std::string rle) { return rle2vec(rle, 0); }
