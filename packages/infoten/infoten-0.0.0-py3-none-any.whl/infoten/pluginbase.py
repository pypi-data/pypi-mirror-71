...
class AbstractPluginEntity(ABC):
    ...
    def name(self) -> str:
        ...
        For instances of `infoten.pluginbase.AbstractDevice`, "name"
        must be unique among all instances of
        `infoten.pluginbase.AbstractDevice`. For instances of
        `infoten.pluginbase.AbstractImplementation`, "name" only needs
        to be unique among all instances of
        `infoten.pluginbase.AbstractImplementation` bound to a specific
        instance of `infoten.pluginbase.AbstractDevice`.
        """
    ...
    def status(self) -> bool:
        ...
        `infoten.pluginbase.AbstractPluginEntity`.
        """
...
class Visibility(Enum):
    ...
    `infoten.pluginbase.AbstractDevice`. USER means the device can be
...
class AbstractDevice(AbstractPluginEntity):
    ...
    def __init__(self, name: str, visibility: Visibility):
        ...
        `infoten.pluginbase.AbstractImplementation` for a specific
        instance of `infoten.pluginbase.AbstractDevice`.
        """
    ...
    def imp(self) -> AbstractImplementation:
        ...
        `infoten.pluginbase.AbstractImplementation` for this device.
        This value is set to the 0th item in
        `infoten.pluginbase.AbstractDevice.imps` the first time
        `infoten.pluginbase.AbstractDevice.add_imp` is called in order
    ...
    def imp_names(self) -> List[str]:
        """The names of all `infoten.pluginbase.AbstractImplementation`
        instances belonging to an instance of
        `infoten.pluginbase.AbstractDevice`.
        """
    ...
    def user_gui(self) -> UserGui:
        """The GUI, an instance of `infoten.pluginbase.UserGui`, that
    ...
    def admin_gui(self) -> AdminGui:
        """The GUI, an instance of `infoten.pluginbase.AdminGui`, that
    ...
    def add_imp(self, imp: AbstractImplementation):
        """Adds an instance of
        `infoten.pluginbase.AbstractImplementation` to an instance of
        `infoten.pluginbase.AbstractDevice`. The first instance of
        `infoten.pluginbase.AbstractImplementation` that's added is
        automatcially set as the current implementation.

        Parameters
        ----------
        imp : An instance of
            `infoten.pluginbase.AbstractImplementation`.
        ...
        ValueError
            ...
            `infoten.pluginbase.AbstractDevice` already has an instance
            of `infoten.pluginbase.AbstractImplementation` with the
            same value for `infoten.pluginbase.AbstractImplementation.name`.
        """
    ...
    def status(self) -> bool:
        """Returns the status of an `infoten.pluginbase.AbstractDevice`
        instance. This assumes that as long as one instance of
        `infoten.pluginbase.AbstractImplementation` has an
        `infoten.pluginbase.AbstractImplementation.status` of True,
        then the instance of `infoten.pluginbase.AbstractDevice` itself
        is True.

        This assumption may of course be incorrect. For example, maybe
        other conditions need to be met for an instance of
        `infoten.pluginbase.AbstractDevice` to have a
        `infoten.pluginbase.AbstractPluginEntity.status` of True. In
