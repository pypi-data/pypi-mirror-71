...
class Metadata:
    ...
    def load_from_file(json_file, unique_identifier):
        ...
        try:
            for i in data['datasets'].keys():
                for j in data['datasets'][i].keys():
                    data['datasets'][i][j] = str(data['datasets'][i][j])
        except ValueError:
            raise MetadataFormatError('The metadata file can''t be stringnify.')
