...
class SparkMonitorHandler(IPythonHandler):
    ...
    async def get(self):
        ...
        port = os.environ.get("SPARKMONITOR_UI_PORT", str(SparkContext._active_spark_context.uiWebUrl.split(':')[-1]))
        logging.info('CURRENT PORT:' + str(port))
        global proxy_root
        proxy_root = proxy_root.format(port) 
        url = "http://" + baseurl + ":" + port
        logging.info("SPARKMONITOR_SERVER: Request URI" + self.request.uri)
        logging.info("SPARKMONITOR_SERVER: Getting from " + url)
        ...
        logging.info("SPARKMONITOR_SERVER: Request_path " + request_path + " \n Replace_path:" + self.replace_path)
