...
def discretize_genexp(data, method, nb_bins=2, axis=0):
    ...
    if method == 'kmeans':
        discr = KBinsDiscretizer(nb_bins, 'ordinal', 'kmeans')
    elif method == 'ewd':
        discr = KBinsDiscretizer(nb_bins, 'ordinal', 'uniform')
    elif method == 'efd':
        discr = KBinsDiscretizer(nb_bins, 'ordinal', 'quantile')
    ...
    if len(data.shape) < 2 :
        ...
        discr_data = pd.Series(discr_data.flatten(), index = data.index)
