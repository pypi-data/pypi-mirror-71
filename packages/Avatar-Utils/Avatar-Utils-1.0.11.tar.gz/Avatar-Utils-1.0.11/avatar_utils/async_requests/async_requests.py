...
async def async_make_requests(*requests_data) -> RequestsResult:
    ...
    requests_count_str_len = len(str(requests_count))

    async with ClientSession() as session:
        for i in range(requests_count):
            method = requests_data_expanded[i]['method']
            url = requests_data_expanded[i]['url']
            request_id = f'{i+1:0>{requests_count_str_len}}'
    ...
    logger.info(f'ASYNC REQUESTS RESULT:\n'
...
async def _request_fetch(session: ClientSession,
    ...
    async with semaphore:
        ...
        try:
            ...
            return ResponseData(exception=err,
    ...
    try:
        ...
        logger.warning(f'[ {request_id} ] not JSON response ({response.status} {response.reason})')
