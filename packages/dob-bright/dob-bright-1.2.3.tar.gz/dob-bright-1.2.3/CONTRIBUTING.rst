...
Ready to contribute? Here's how to set up |dob-bright|_
...
2. Clone your fork locally.
   ...
   and run the command::
    ...
    $ git clone git@github.com:<your_login>/dob-bright.git

3. Install |dob-bright|_ into a Python virtual instance,
   or |virtualenv|_.
   ...
   Next, set up a virtual environment for local development::
    ...
    $ cd dob-bright/
    $ mkvirtualenv -a $(pwd) dob-bright
    (dob-bright) $

   *Note:* We use the ``-a`` option so that ``cdproject`` changes directories
   to the ``dob-bright/`` directory when we're in the virtual
   environment.
...
4. Before starting work on a new feature or bug fix, make sure your
   ``proving`` branch is up to date with the official branch::
    ...
    (dob-bright) $ git push origin HEAD
...
1. You can lint the code easily with one command.
   ...
   .. code-block:: Bash
      ...
      $ flake8 setup.py dob_bright/ tests/
