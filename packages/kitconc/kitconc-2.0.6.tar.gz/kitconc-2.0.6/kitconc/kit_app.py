...
class KitApp(Tk):
    ...
    def __add_widgets(self):
        ...
        models.add_command(label='Create model...',command=self.__createmodelsdialog)
        models.add_command(label='Delete model...',command=self.__on_delete_model)
        models.add_separator()
        models.add_command(label='Add reference list...',command=self.__add_reference_list)
        models.add_command(label='Add stoplist...',command=self.__add_stoplist)
    ...
    def __load_tools (self):
        ...
        self.frm_trv_tools.insert('','end','Word clouds',text=' Word clouds',image=self.icon_tool)
    ...
    def __newcorpusdialog(self):
        def callback():
            ...
            lbl_language = ttk.Label(self.top,text='Language (model)')
    ...
    def __createmodelsdialog(self):
        def callback():
            self.top = Toplevel()
            self.top.title('Create language model')
            window_height = 150
            window_width = 390
            screen_width = self.top.winfo_screenwidth()
            screen_height = self.top.winfo_screenheight()
            ...
            self.top.focus()
            self.top.update_idletasks()
            lbl_source = ttk.Label(self.top,text='Tagged corpus (directory)')
            lbl_source.place(x=10,y=10)
            self.source = StringVar()
            txt_source = ttk.Entry(self.top,state='readonly',textvariable=self.source)
            txt_source.place(x=10,y=35,width=260)
            btn_browse = ttk.Button(self.top,text='Browse...',command=self.__newcorpus_browse)
            btn_browse.place(x=280,y=33,width=100)
            lbl_language = ttk.Label(self.top,text='Model name (language)')
            ...
            btn_create = ttk.Button(self.top,text='Create',width=10,command=self.__on_create_model)
            btn_create.place(x=200,y=93)
            btn_cancel = ttk.Button(self.top,text='Cancel',width=10,command=self.__create_model_cancel)
    ...
    def __deletemodeldialog(self):
        def callback():
            ...
            btn_delete = ttk.Button(self.top,text='Delete',width=10,command=self.__delete_model)
            btn_delete.place(x=10,y=60)
            btn_cancel = ttk.Button(self.top,text='Cancel',width=10,command=self.__delete_model_cancel)
    ...
    def __on_create_model(self):
        def callback():
            self.__show_progress('Creating model...')
    ...
    def __create_model(self):
        ...
        if os.path.exists(source_folder):
            ...
            messagebox.showinfo('Create model', 'The model was created successfully!')
        else:
            messagebox.showwarning('Create model', 'The source directory is not valid.')
        self.focus()
        self.__hide_progress()
    ...
    def __delete_model(self):
        ...
        if os.path.exists(path):
            os.remove(path)
        self.top.destroy()
        messagebox.showinfo('Delete model', 'The model was deleted successfully!')
        self.focus()
        ...
        if self.kit.corpus_in_use != None:
            ...
            if len(filename)!= 0:
                if os.path.exists(filename):
                    ...
                    messagebox.showinfo('Add reference list', 'Success!\nThe list was saved.')
            else:
                messagebox.showwarning('Add reference list', 'Attention!\nYou need to select a file to continue.')
        self.__hide_progress()
        ...
        if self.kit.corpus_in_use != None:
            ...
            if len(filename)!= 0:
                if os.path.exists(filename):
                    ...
                    messagebox.showinfo('Add stoplist', 'Success!\nThe list was saved.')
            else:
                messagebox.showwarning('Add stoplist', 'Attention!\nYou need to select a file to continue.')
        self.__hide_progress()
    ...
    def __on_tool(self):
        def callback():
            ...
            # Word clouds
            elif tool == 'Word clouds' and self.kit.corpus_in_use!=None:
                self.btn_execute = ttk.Button(self.frm_lbl_options,text='Execute',command=self.__on_wordclouds)
                self.btn_execute.place(x=5,y=350)
                self.lbl_list = ttk.Label(self.frm_lbl_options,text='Wordlist type:')
                ...
                self.lbl_colors = ttk.Label(self.frm_lbl_options,text='Colors:')
                ...
                self.chk_vertical = ttk.Checkbutton(self.frm_lbl_options,text='allow vertical')
                ...
                self.lbl_format = ttk.Label(self.frm_lbl_options,text='Output format:')
    ...
    def __on_wordclouds(self):
        def callback():
            self.__show_progress('Running Word clouds...')
