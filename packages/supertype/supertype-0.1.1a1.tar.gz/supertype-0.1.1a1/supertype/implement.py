...
def produce(data, attribute, supertype_id):
    sk = keys.UmbralPrivateKey.from_bytes(bytes.fromhex(os.environ.get('SUPERTYPE_SECRET_ACCESS_KEY')))
...
def consume(supertype_id, attribute, date):
    sk = keys.UmbralPrivateKey.from_bytes(bytes.fromhex(os.environ.get('SUPERTYPE_SECRET_ACCESS_KEY')))
