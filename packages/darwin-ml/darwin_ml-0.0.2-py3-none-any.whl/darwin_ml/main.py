...
from darwin_ml.handlers import TAPredictionTransformer, TAPriceData, IncrementalEngine
...
class MainStateSpace(InjectableTime):
    def __init__(self):
        ...
        self.__incremental_engine = IncrementalEngine()
    ...
    @space.setter
    def space(self, _space: StateSpaceModel):
        """ Should probably manage"""
        self.__space = _space
    ...
    def step(self) -> None:
        ...
        ss = self.space.step(allocation_container, feature_container, price_container)
        
        return ss

    def __get_incremental_engine(self):
        self.__incremental_engine.processor = self.processor 
        # = IncrementalEngine(processor=self.processor)
        self.__incremental_engine.name = f"{self.user_id}-{self.episode}-{self.exchange}-{self.asset}-online"
        self.__incremental_engine.category = "information"
        self.__incremental_engine.subcategories = {}
        self.__incremental_engine.metatype = "state"
        self.__incremental_engine.submetatype = "space"
        self.__incremental_engine.abbreviation = "INKKY"
        return self.__incremental_engine

    # def _save_incremental(self):
    #     engine = self.__get_incremental_engine()
    #     engine.blobfile = 

    def __reset_statespace(self) -> None:
        engine = self.__get_incremental_engine()
        engine.blobfile = StateSpaceModel()
        engine.reset()
        self.space = engine.models

    def reset(self):
        self.technical.reset()
        self.__reset_statespace()
