...
class PBItem=WeaponSheetData:
    def __init__(self, idColumnIndex):
        self.idColumnIndex = idColumnIndex
        self.dict = self.PBItem=WeaponCollect(self.idColumnIndex - 1)

    def PBItem=WeaponCollect(self, idColumnIndex):
        data = xlrd.open_workbook(r"/Users/shiboxuan/testTools/getSheetData/conf/excel/item.xlsx")
        table = data.sheet_by_name("PBItem=Weapon")
        rowNum = table.nrows
        colNum = table.ncols
