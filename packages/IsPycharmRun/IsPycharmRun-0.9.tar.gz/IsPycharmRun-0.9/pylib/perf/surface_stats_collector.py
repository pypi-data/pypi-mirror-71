...
class SurfaceStatsCollector(object):
  ...
  def _GetNormalizedDeltas(data, refresh_period, min_normalized_delta=None):
    ...
    if min_normalized_delta != None:
      print("1", min_normalized_delta)
      print("2", refresh_period)
      print("3", min_normalized_delta)
      print("4", deltas)
  ...
  def _CalculateResults(refresh_period, timestamps, result_suffix):
    ...
    print(round((frame_count - 1) / seconds))
    return 'avg_surface_fps is' + " " + str(int(round((frame_count - 1) / seconds))) + " " + 'fps'
    '''
    return [
  ...
  def _StorePerfResults(self,refresh_period,timestamps):
    ...
    self.result = self._CalculateResults(refresh_period, timestamps, '')
  ...
  def _GetSurfaceFlingerFrameData(self,getframecmd):
    ...
    results = os.popen(getframecmd).readlines()
    if not len(results):
      return (None, None)
    ...
    for line in results[1:]:
      ...
      if len(fields) != 3:
        continue
      #print fields
      if fields[0] == "0" and fields[1] == "0" and fields[2]=="0":
        continue
