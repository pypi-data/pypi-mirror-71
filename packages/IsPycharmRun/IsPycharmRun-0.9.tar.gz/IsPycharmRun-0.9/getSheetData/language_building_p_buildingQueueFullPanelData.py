...
class p_buildingQueueFullPanelSheetData:
    def __init__(self, idColumnIndex):
        self.idColumnIndex = idColumnIndex
        self.dict = self.p_buildingQueueFullPanelCollect(self.idColumnIndex - 1)

    def p_buildingQueueFullPanelCollect(self, idColumnIndex):
        data = xlrd.open_workbook(r"/Users/shiboxuan/testTools/getSheetData/conf/excel/Language/language_building.xlsx")
        table = data.sheet_by_name("p_buildingQueueFullPanel")
        rowNum = table.nrows
        colNum = table.ncols
