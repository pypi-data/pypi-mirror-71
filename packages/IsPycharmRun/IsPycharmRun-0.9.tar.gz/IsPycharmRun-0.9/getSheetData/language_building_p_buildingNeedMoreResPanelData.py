...
class p_buildingNeedMoreResPanelSheetData:
    def __init__(self, idColumnIndex):
        self.idColumnIndex = idColumnIndex
        self.dict = self.p_buildingNeedMoreResPanelCollect(self.idColumnIndex - 1)

    def p_buildingNeedMoreResPanelCollect(self, idColumnIndex):
        data = xlrd.open_workbook(r"/Users/shiboxuan/testTools/getSheetData/conf/excel/Language/language_building.xlsx")
        table = data.sheet_by_name("p_buildingNeedMoreResPanel")
        rowNum = table.nrows
        colNum = table.ncols
