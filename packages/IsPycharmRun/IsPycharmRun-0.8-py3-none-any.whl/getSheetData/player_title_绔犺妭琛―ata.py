...
class 章节表SheetData:
    def __init__(self, idColumnIndex):
        ...
        self.dict = self.章节表Collect(self.idColumnIndex)

    def 章节表Collect(self, idColumnIndex):
        data = xlrd.open_workbook("E:\TestTools\getSheetData\conf\excel\player_title.xlsx")
        table = data.sheet_by_name("章节表")
        rowNum = table.nrows
        colNum = table.ncols
