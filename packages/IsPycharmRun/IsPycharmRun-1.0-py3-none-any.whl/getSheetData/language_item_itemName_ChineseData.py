...
class itemName_ChineseSheetData:
    def __init__(self, idColumnIndex):
        self.idColumnIndex = idColumnIndex
        self.dict = self.itemName_ChineseCollect(self.idColumnIndex)

    def itemName_ChineseCollect(self, idColumnIndex):
        data = xlrd.open_workbook("E:\TestTools\getSheetData\conf\excel\Language\language_item.xlsx")
        table = data.sheet_by_name("itemName_Chinese")
        rowNum = table.nrows
        colNum = table.ncols
