...
class PBItem=BoxSheetData:
    def __init__(self, idColumnIndex):
        self.idColumnIndex = idColumnIndex
        self.dict = self.PBItem=BoxCollect(self.idColumnIndex - 1)

    def PBItem=BoxCollect(self, idColumnIndex):
        data = xlrd.open_workbook(r"/Users/shiboxuan/testTools/getSheetData/conf/excel/item.xlsx")
        table = data.sheet_by_name("PBItem=Box")
        rowNum = table.nrows
        colNum = table.ncols
