...
class PBItem=HeroSheetData:
    def __init__(self, idColumnIndex):
        self.idColumnIndex = idColumnIndex
        self.dict = self.PBItem=HeroCollect(self.idColumnIndex - 1)

    def PBItem=HeroCollect(self, idColumnIndex):
        data = xlrd.open_workbook(r"/Users/shiboxuan/testTools/getSheetData/conf/excel/item.xlsx")
        table = data.sheet_by_name("PBItem=Hero")
        rowNum = table.nrows
        colNum = table.ncols
