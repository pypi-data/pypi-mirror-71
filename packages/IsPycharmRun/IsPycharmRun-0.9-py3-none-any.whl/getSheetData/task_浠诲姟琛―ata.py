...
class 任务表SheetData:
    def __init__(self, idColumnIndex):
        ...
        self.dict = self.任务表Collect(self.idColumnIndex)

    def 任务表Collect(self, idColumnIndex):
        data = xlrd.open_workbook("E:\TestTools\getSheetData\conf\excel\task.xlsx")
        table = data.sheet_by_name("任务表")
        rowNum = table.nrows
        colNum = table.ncols
