...
class PBCharacterPerformanceSheetData:
    def __init__(self, idColumnIndex):
        self.idColumnIndex = idColumnIndex
        self.dict = self.PBCharacterPerformanceCollect(self.idColumnIndex - 1)

    def PBCharacterPerformanceCollect(self, idColumnIndex):
        data = xlrd.open_workbook(r"D:\testTools\getSheetData\conf\excel\performance.xlsx")
        table = data.sheet_by_name("PBCharacterPerformance")
        rowNum = table.nrows
        colNum = table.ncols
