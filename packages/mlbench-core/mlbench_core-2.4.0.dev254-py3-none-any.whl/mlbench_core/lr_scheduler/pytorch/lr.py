...
class ReduceLROnPlateauWithWarmup(ReduceLROnPlateau):
    ...
    def __init__(self, optimizer, warmup_init_lr, scaled_lr, warmup_epochs, batches_per_epoch=None, **kwargs):
        self.warmup_epochs = warmup_epochs
        self.warmup_duration = warmup_epochs * (batches_per_epoch or 1)
        self.warmup_init_lr = warmup_init_lr
        ...
        for group in self.optimizer.param_groups:
            group["initial_lr"] = self.warmup_init_lr
    ...
    def step(self, metrics, epoch=None):
        if self.last_epoch >= self.warmup_epochs: # Reduce only if we finished warmup
            ...
            if self.warmup_epochs == self.warmup_duration: # warmup per epoch
                self.batch_step()
            self.last_epoch += 1
