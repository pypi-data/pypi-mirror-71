...
class BlackFox:
    ...
    def __optimize_ann(
        ...
        if config.problem_type == ProblemType.BINARYCLASSIFICATION:
            metric = config.binary_optimization_metric
    ...
    def continue_ann_optimization(self, id, model_type=NeuralNetworkType.H5, integrate_scaler=False, model_path=None, delete_on_finish=True, metric_string='metric', status_interval=5, log_writer=LogWriter()):
        ...
        delete_on_finish : bool
            Delete optimization from service after it has finished
        metric_string : str
            Text to write in log as metric name
        status_interval : int
            Time interval for repeated server calls for optimization info and logging
        log_writer : list[LogWriter]
            Optional log writer used for logging the optimization process
    ...
    def continue_random_forest_optimization(self, id, model_type=RandomForestModelType.BINARY, model_path=None, delete_on_finish=True, metric_string='metric', status_interval=5, log_writer=LogWriter()):
        """Continue optimization.
    ...
    def continue_xgboost_optimization(self, id, model_path=None, delete_on_finish=True, metric_string='metric', status_interval=5, log_writer=LogWriter()):
        """Continue optimization.
