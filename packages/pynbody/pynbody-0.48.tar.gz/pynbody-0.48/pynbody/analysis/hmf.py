...
def velocity_variance(powspec, min_k=None):
    context = powspec.k.sim.properties
    assert np.allclose(context['omegaM0']+context['omegaL0'], 1.0)
    if min_k is None:
        min_k = powspec.min_k
    integrand_ln_k = lambda k: np.exp(k) * powspec(np.exp(k))
    norm = 1./(2*math.pi**2) * 1e4 * context['a']**2 * (context['omegaM0']/context['a']**3 + context['omegaL0'])
    v = scipy.integrate.cumtrapz(powspec.Pk*powspec._norm *powspec._lingrowth,powspec.k)
    return norm*v
