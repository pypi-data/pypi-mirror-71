...
def run_as_docker(
    ...
    :param container_alias: 容器别名
    :param pip_alias: 宿主机pip别名
    :param container_pip_alias: 容器pip别名
    :param volumes: 容器磁盘暴露映射
    :return:
    """
    # 依赖包的缓存
    package_cache = []
    try:
        ...
        # 生成依赖文件
        logger.info('生成依赖文件')
        os.system(f'{pip_alias} freeze --all >pre_requirements.txt')
        ...
        with open('./Dockerfile', 'w') as file:
            ...
            for k, v in volumes.items():
                assert k.startswith('~/') or k.startswith('/'), 'container path must starts with ~/ or /'
                assert v.startswith('~/') or v.startswith('/'), 'host path must starts with ~/ or /'
                file.write(f'VOLUME {k.replace("~",".")}:{v.replace("~",".")}\n')
            ...
            if package_repository:
                # 阿里云仓库  =>  https://mirrors.aliyun.com/pypi/simple/
                file.write(f'RUN {container_pip_alias} install -i {package_repository} -r requirements.txt' + '\n')
                pass
            else:
                file.write(f'RUN {container_pip_alias} install --no-cache-dir -r requirements.txt' + '\n')
                pass
            # 设置系统时区
