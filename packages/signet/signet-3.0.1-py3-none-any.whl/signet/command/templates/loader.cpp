...
char* sha1hexdigest(const char fname[]) {
	...
	while((rdsz=fread(buf, 1, sizeof(buf), fin)) > 0) {
		Sha1Update(&ctx, buf, (uint32_t)rdsz);
		}
...
int validate(const string script_path) {
	...
	for(Py_ssize_t i = 0; i < PyList_Size(pypath.get()); i++) {
        ...
        paths.push_back(PyUnicode_AsUTF8(py_item));
        }
...
int parse_options(int argc, char* argv[], const char* script) {
	...
	args[0] = Py_DecodeLocale(script, NULL);
	...
	for(int i = 1; i < argc; i++) {
		...
		args[args_used++] = Py_DecodeLocale(argv[i], NULL);
		}
    ...
    if (senv) {
        if (stricmp(senv, "OFF") == 0) {
            TAMPER = 0;
            }
        else if (stricmp(senv, "WARN") == 0) {
            TAMPER = 1;
            }
        else if (stricmp(senv, "NORMAL") == 0) {
            TAMPER = 2;
            }
        else if (stricmp(senv, "MAX") == 0) {
            TAMPER = 3;
            }
        else{
...
void initialize_virtualenv() {
	...
	Py_SetPythonHome(Py_DecodeLocale(venv, NULL));
	}
...
int run_validation(int argc, char* argv[], const char* script) {
	...
	Py_SetProgramName(Py_DecodeLocale(script, NULL));
...
int main(int argc, char* argv[]) {
	...
	Py_SetProgramName(Py_DecodeLocale(script.c_str(), NULL));
	initialize_virtualenv();
	Py_Initialize();
