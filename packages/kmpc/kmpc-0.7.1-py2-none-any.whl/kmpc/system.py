...
class GuiSync(Sync):
    ...
    def run_at_end(self, result):
        self.popup.dismiss()
        App.get_running_app().root.system.syncPopup.dismiss()
        App.get_running_app().root.do_idle_handler = True

    def print_line(self, line):
        label = Factory.BaseLabel(
                text=line.rstrip(),
                size_hint=(None, None),
                font_size='12sp',
                halign='left')
...
class SystemScreen(Screen):
    """The System screen, for internal functions."""
    ...
    def enter_screen(self, *args):
        ss = self.ids.ss
        ss.ids.systembutton.state = 'down'
        ss.ids.systembutton.disabled = True
    ...
    def sync_cache(self):
        ...
        stdoutPopup.open()
        GuiSync(stdoutPopup, self.config, ['cache'])
    ...
    def sync_all(self):
        ...
        if self.config.getboolean('system', 'exportfirst'):
            GuiSync(stdoutPopup,
                    self.config,
                    ['music', 'cache', 'exportratings', 'importratings'])
        else:
            GuiSync(stdoutPopup,
                    self.config,
    ...
    def do_plugins(self):
        ...
        for dirpath, dirnames, filenames in os.walk(
                os.path.join(App.get_running_app().configdir, 'plugins')):
        ...
        for script in scripts:
            ...
            pbutton.script = os.path.join(App.get_running_app().configdir,
                    'plugins', script, 'plugin.sh')
    ...
    def load_plugin(self, plugin, instance):
        ...
        sys.path.append(os.path.join(App.get_running_app().configdir,
                'plugins', plugin))
        pluginmodule = __import__('plugin')
        reload(pluginmodule)
        Builder.load_file(os.path.join(App.get_running_app().configdir,
                                       'plugins',
                                       plugin,
                                       'plugin.kv'))
    ...
    def unload_plugin(self, plugin, pluginmodule, instance):
        ...
        del pluginmodule
        Builder.unload_file(os.path.join(App.get_running_app().configdir,
                                         'plugins',
                                         plugin,
                                         'plugin.kv'))
