...
class Sync(object):
    ...
    def __init__(self, config, runparts=[]):
        ...
        self.cachepath = config.get('paths', 'cachepath')
        self.syncbasepath = config.get('sync', 'syncmusicpath')
        self.synccachepath = config.get('sync', 'synccachepath')
        ...
        if self.mpdhost == self.synchost:
            ...
            syncWarningPopup = Factory.SyncWarningPopup()
            syncWarningPopup.open()
            self.run_at_end('ABORTED')
            return
        ...
        if 'cache' in runparts:
            self.callbacks.append(self.d.addCallbacks(
                    self.sync_cache,
                    self.errback))
            self.callbacks.append(self.d.addCallbacks(
                    self.finish_cache_sync,
                    self.errback))
...
#########################
    def sync_cache(self, result):
        from twisted.internet import reactor
        self.print_line("Syncing cache")
        # rsync the files
        cmdline = ['rsync',
                   '-vruxhm',
                   '--delete-before',
                   '--exclude=.DS_Store',
                   '--delete-excluded',
                   self.synchost+':'+self.synccachepath+'/',
                   self.cachepath]
    ...
    def finish_cache_sync(self, result):
        self.print_line("Cache synced from synchost")
        return True
