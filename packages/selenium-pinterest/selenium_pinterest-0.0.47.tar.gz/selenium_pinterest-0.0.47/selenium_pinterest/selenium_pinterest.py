    ...
    def unfollow(self, user_name: str) -> bool:
        try:
            ...
            if user_container is not None:
                user_cont = self.browser.find_all(By.XPATH, "//div[contains(@class, 'Jea XiG zI7 iyn Hsu')]")
                user_element = self.browser.find(By.XPATH, "//button[contains(@class, 'RCK Hsu USg Vxj aZc Zr3 hA- GmH adn a_A gpV hNT iyn BG7 NTm KhY')]", user_cont)

                user_element.click()
                # for user_element in user_elements:
                #     print(user_element.text)
                #     if user_element.text == "Following":
                #         user_element.click()
                #         rand.sleep(0.5, 1)
                #     else:
                #         return False
