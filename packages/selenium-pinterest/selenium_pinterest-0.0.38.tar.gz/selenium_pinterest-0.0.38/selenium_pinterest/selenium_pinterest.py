...
class Pinterest:
    ...
    def follow(self, user_name: str) -> bool:
        try:
            ...
            follow_container = self.browser.find(By.XPATH, "//div[contains(@data-test-id, 'user-follow-button')]")
            ...
            if follow_container:
                ...
                following_cont = self.browser.find(By.XPATH, "//div[contains(@data-test-id, 'user-unfollow-button')]")
                follow_button_updated = self.browser.find(By.XPATH, "//div[contains(@class, 'tBJ dyH iFc yTZ erh tg7 mWe')]", following_cont, timeout=3)
                print('user.text', follow_button_updated.text)
                
                return follow_button_updated.text == "Following"
    ...
    def unfollow(self, user_name: str) -> bool:
        try:
            ...
            if user_container:
                user_element = self.browser.find(By.XPATH, "//div[contains(@class, 'tBJ dyH iFc yTZ pBj tg7 mWe')]", user_container)

                if user_element is not None and user_element.text == "Following":
                    user_element.click()
                    rand.sleep(0.5, 1)
                else:
                    return False
                
                user_cont = self.browser.find(By.XPATH, "//div[contains(@data-test-id, 'user-follow-button')]")
                user_updated = self.browser.find(By.XPATH, "//div[contains(@class, 'tBJ dyH iFc yTZ erh tg7 mWe')]", user_cont)
                print('user.text:', user_updated.text)
