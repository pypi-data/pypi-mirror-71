...
class Pinterest:
    ...
    def follow(self, user_name: str) -> bool:
        try:
            ...
            follow_container = self.browser.find(By.XPATH, "//div[contains(@class, 'Jea XiG zI7 iyn Hsu')]")
            ...
            follow_button = self.browser.find(By.XPATH, "//div[contains(@class, 'tBJ dyH iFc yTZ erh tg7 mWe')]", follow_container, timeout=3)
            
            if follow_button is not None and follow_button.text == "Following":
                return True

            if follow_button is None or follow_button.text != "Following":
                print('im in this if')
                follow_container_after_follow = self.browser.find(By.XPATH, "//div[contains(@class, 'Eqh Jea gRy jx- zI7 iyn Hsu')]")
                container_elements = self.browser.find_all(By.XPATH, "//div[contains(@class, 'tBJ dyH iFc yTZ pBj tg7 mWe')]", follow_container_after_follow)
    ...
    def unfollow(self, user_name: str) -> bool:
        try:
            self.browser.get(self.url_creator.user_url(user_name))
            rand.sleep(0.5, 1)
            user_elements = self.browser.find_all(By.XPATH, "//div[contains(@class, 'tBJ dyH iFc yTZ pBj tg7 mWe')]")
