...
class RequestMetaData(object):
    ...
    def __init__(self, access_key_id, access_key_secret, method,
        ...
        self.method = method
        self.bucket = bucket
        self.key = str(key,encoding='utf8') if isinstance(key,bytes) else key
    ...
    def _complete_headers(self):
        ...
        if self.body is not None:
            md5 = hashlib.md5()
            if isinstance(self.body, IOBase):
                ...
                md5.update(self.body.encode('utf8') if isinstance(self.body,str) else self.body)
                md5sum = md5.hexdigest()
                self.headers[HTTP_HEADER.CONTENT_MD5] = md5sum
        ...
        if None not in (self.access_key_id, self.access_key_secret):
            ...
            hmac_sha1 = hmac.new(bytes(self.access_key_secret,encoding='utf8'),
                                 bytes(str_to_sign,encoding='utf8'), hashlib.sha256)
            b64_hmac_sha1 = base64.encodebytes(hmac_sha1.digest()).decode('utf8').strip()
    ...
    def _complete_url(self):
        ...
        for k, v in sorted(self.params.items()):
            piece = k
            if v is not None:
                piece += "=%s" % urllib.parse.quote(str(v), '*')
            pairs.append(piece)
    ...
    def _get_string_to_sign(self):
        ...
        sorted_meta_headers = list(meta_headers.keys())
