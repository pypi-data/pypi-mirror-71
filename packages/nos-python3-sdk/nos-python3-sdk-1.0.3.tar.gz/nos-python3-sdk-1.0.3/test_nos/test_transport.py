...
class TestTransport(TestCase):
    def test_init_default_args(self):
        transport = Transport()
        self.assertEqual(None, transport.access_key_id)
        self.assertEqual(None, transport.access_key_secret)
        self.assertEqual(2, transport.max_retries)
        self.assertEqual(False, transport.retry_on_timeout)
        self.assertEqual('nos-eastchina1.126.net', transport.end_point)
        self.assertEqual((500, 501, 503, ), transport.retry_on_status)
        self.assertEqual(None, transport.timeout)
    ...
    def test_init_args(self):
        ...
        self.assertEqual(access_key_id, transport.access_key_id)
        self.assertEqual(access_key_secret, transport.access_key_secret)
        self.assertEqual(retry_on_timeout, transport.retry_on_timeout)
        self.assertEqual(retry_on_status, transport.retry_on_status)
        self.assertEqual(kwargs['max_retries'], transport.max_retries)
        self.assertEqual(kwargs['end_point'], transport.end_point)
        self.assertEqual(kwargs['timeout'], transport.timeout)
        self.assertEqual(serializer, transport.serializer)
        self.assertIsInstance(transport.connection, Urllib3HttpConnection)
    ...
    def test_perform_request_default_args(self, mock_meta_data):
        ...
        self.assertEqual((200, {}, ''), transport.perform_request('GET'))
