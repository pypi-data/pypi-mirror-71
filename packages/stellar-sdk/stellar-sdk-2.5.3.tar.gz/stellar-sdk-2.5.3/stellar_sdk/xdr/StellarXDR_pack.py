...
class StellarXDRPacker(xdrlib.Packer):
    ...
    def pack_CryptoKeyType(self, data):
        ...
        if self.check_enum and data not in [
            const.KEY_TYPE_ED25519,
            const.KEY_TYPE_PRE_AUTH_TX,
            const.KEY_TYPE_HASH_X,
            const.KEY_TYPE_MUXED_ED25519,
        ]:
    ...
    def pack_SignerKeyType(self, data):
        ...
        if self.check_enum and data not in [
            const.SIGNER_KEY_TYPE_ED25519,
            const.SIGNER_KEY_TYPE_PRE_AUTH_TX,
    ...
    def pack_SCPStatementType(self, data):
        ...
        if self.check_enum and data not in [
            const.SCP_ST_PREPARE,
            const.SCP_ST_CONFIRM,
            const.SCP_ST_EXTERNALIZE,
    ...
    def pack_SCPStatement(self, data):
        ...
        if data.pledges.type == const.SCP_ST_PREPARE:
            ...
            if len(data.pledges.prepare.prepared) > 1 and self.check_array:
                raise XDRError(
                    "array length too long for data.pledges.prepare.prepared"
                )
            ...
            if len(data.pledges.prepare.preparedPrime) > 1 and self.check_array:
                raise XDRError(
                    "array length too long for data.pledges.prepare.preparedPrime"
                )
    ...
    def pack_StellarValueType(self, data):
        ...
        if self.check_enum and data not in [
            const.STELLAR_VALUE_BASIC,
            const.STELLAR_VALUE_SIGNED,
        ]:
    ...
    def pack_LedgerUpgradeType(self, data):
        ...
        if self.check_enum and data not in [
            const.LEDGER_UPGRADE_VERSION,
            const.LEDGER_UPGRADE_BASE_FEE,
            const.LEDGER_UPGRADE_MAX_TX_SET_SIZE,
            const.LEDGER_UPGRADE_BASE_RESERVE,
        ]:
    ...
    def pack_LedgerEntryChangeType(self, data):
        ...
        if self.check_enum and data not in [
            const.LEDGER_ENTRY_CREATED,
            const.LEDGER_ENTRY_UPDATED,
            const.LEDGER_ENTRY_REMOVED,
            const.LEDGER_ENTRY_STATE,
        ]:
    ...
    def pack_AccountFlags(self, data):
        ...
        if self.check_enum and data not in [
            const.AUTH_REQUIRED_FLAG,
            const.AUTH_REVOCABLE_FLAG,
    ...
    def pack_EnvelopeType(self, data):
        ...
        if self.check_enum and data not in [
            const.ENVELOPE_TYPE_TX_V0,
            const.ENVELOPE_TYPE_SCP,
            const.ENVELOPE_TYPE_TX,
            const.ENVELOPE_TYPE_AUTH,
            const.ENVELOPE_TYPE_SCPVALUE,
            const.ENVELOPE_TYPE_TX_FEE_BUMP,
        ]:
    ...
    def pack_OperationType(self, data):
        ...
        if self.check_enum and data not in [
            ...
            const.PAYMENT,
            const.PATH_PAYMENT_STRICT_RECEIVE,
            const.MANAGE_SELL_OFFER,
            const.CREATE_PASSIVE_SELL_OFFER,
            const.SET_OPTIONS,
            const.CHANGE_TRUST,
            const.ALLOW_TRUST,
            const.ACCOUNT_MERGE,
            ...
            const.MANAGE_DATA,
            const.BUMP_SEQUENCE,
            const.MANAGE_BUY_OFFER,
            const.PATH_PAYMENT_STRICT_SEND,
        ]:
    ...
    def pack_CreateAccountResultCode(self, data):
        ...
        if self.check_enum and data not in [
            ...
            const.CREATE_ACCOUNT_MALFORMED,
            const.CREATE_ACCOUNT_UNDERFUNDED,
            const.CREATE_ACCOUNT_LOW_RESERVE,
    ...
    def pack_PaymentResultCode(self, data):
        ...
        if self.check_enum and data not in [
            ...
            const.PAYMENT_MALFORMED,
            const.PAYMENT_UNDERFUNDED,
            const.PAYMENT_SRC_NO_TRUST,
            const.PAYMENT_SRC_NOT_AUTHORIZED,
            const.PAYMENT_NO_DESTINATION,
            const.PAYMENT_NO_TRUST,
            const.PAYMENT_NOT_AUTHORIZED,
            const.PAYMENT_LINE_FULL,
    ...
    def pack_PathPaymentStrictReceiveResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.PATH_PAYMENT_STRICT_RECEIVE_SUCCESS,
            const.PATH_PAYMENT_STRICT_RECEIVE_MALFORMED,
            const.PATH_PAYMENT_STRICT_RECEIVE_UNDERFUNDED,
            const.PATH_PAYMENT_STRICT_RECEIVE_SRC_NO_TRUST,
            const.PATH_PAYMENT_STRICT_RECEIVE_SRC_NOT_AUTHORIZED,
            const.PATH_PAYMENT_STRICT_RECEIVE_NO_DESTINATION,
            const.PATH_PAYMENT_STRICT_RECEIVE_NO_TRUST,
            const.PATH_PAYMENT_STRICT_RECEIVE_NOT_AUTHORIZED,
            const.PATH_PAYMENT_STRICT_RECEIVE_LINE_FULL,
            ...
            raise XDRError(
                "value=%s not in enum PathPaymentStrictReceiveResultCode" % data
            )
        self.pack_int(data)
    ...
    def pack_PathPaymentStrictSendResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.PATH_PAYMENT_STRICT_SEND_SUCCESS,
            const.PATH_PAYMENT_STRICT_SEND_MALFORMED,
            const.PATH_PAYMENT_STRICT_SEND_UNDERFUNDED,
            const.PATH_PAYMENT_STRICT_SEND_SRC_NO_TRUST,
            const.PATH_PAYMENT_STRICT_SEND_SRC_NOT_AUTHORIZED,
            const.PATH_PAYMENT_STRICT_SEND_NO_DESTINATION,
            const.PATH_PAYMENT_STRICT_SEND_NO_TRUST,
            const.PATH_PAYMENT_STRICT_SEND_NOT_AUTHORIZED,
            const.PATH_PAYMENT_STRICT_SEND_LINE_FULL,
            const.PATH_PAYMENT_STRICT_SEND_NO_ISSUER,
            const.PATH_PAYMENT_STRICT_SEND_TOO_FEW_OFFERS,
            const.PATH_PAYMENT_STRICT_SEND_OFFER_CROSS_SELF,
            const.PATH_PAYMENT_STRICT_SEND_UNDER_DESTMIN,
        ]:
            raise XDRError(
                "value=%s not in enum PathPaymentStrictSendResultCode" % data
            )
        self.pack_int(data)
    ...
    def pack_ManageSellOfferResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.MANAGE_SELL_OFFER_SUCCESS,
            const.MANAGE_SELL_OFFER_MALFORMED,
            const.MANAGE_SELL_OFFER_SELL_NO_TRUST,
            const.MANAGE_SELL_OFFER_BUY_NO_TRUST,
            const.MANAGE_SELL_OFFER_SELL_NOT_AUTHORIZED,
            const.MANAGE_SELL_OFFER_BUY_NOT_AUTHORIZED,
            const.MANAGE_SELL_OFFER_LINE_FULL,
            const.MANAGE_SELL_OFFER_UNDERFUNDED,
            const.MANAGE_SELL_OFFER_CROSS_SELF,
            const.MANAGE_SELL_OFFER_SELL_NO_ISSUER,
            const.MANAGE_SELL_OFFER_BUY_NO_ISSUER,
            const.MANAGE_SELL_OFFER_NOT_FOUND,
            const.MANAGE_SELL_OFFER_LOW_RESERVE,
        ]:
    ...
    def pack_ManageOfferEffect(self, data):
        ...
        if self.check_enum and data not in [
            const.MANAGE_OFFER_CREATED,
            const.MANAGE_OFFER_UPDATED,
    ...
    def pack_ManageOfferSuccessResult(self, data):
        ...
        if (
            data.offer.effect == const.MANAGE_OFFER_CREATED
            or data.offer.effect == const.MANAGE_OFFER_UPDATED
        ):
    ...
    def pack_ManageBuyOfferResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.MANAGE_BUY_OFFER_SUCCESS,
            const.MANAGE_BUY_OFFER_MALFORMED,
            const.MANAGE_BUY_OFFER_SELL_NO_TRUST,
            const.MANAGE_BUY_OFFER_BUY_NO_TRUST,
            const.MANAGE_BUY_OFFER_SELL_NOT_AUTHORIZED,
            const.MANAGE_BUY_OFFER_BUY_NOT_AUTHORIZED,
            const.MANAGE_BUY_OFFER_LINE_FULL,
            const.MANAGE_BUY_OFFER_UNDERFUNDED,
            const.MANAGE_BUY_OFFER_CROSS_SELF,
            const.MANAGE_BUY_OFFER_SELL_NO_ISSUER,
            const.MANAGE_BUY_OFFER_BUY_NO_ISSUER,
            const.MANAGE_BUY_OFFER_NOT_FOUND,
    ...
    def pack_SetOptionsResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.SET_OPTIONS_SUCCESS,
            const.SET_OPTIONS_LOW_RESERVE,
            const.SET_OPTIONS_TOO_MANY_SIGNERS,
            const.SET_OPTIONS_BAD_FLAGS,
            const.SET_OPTIONS_INVALID_INFLATION,
            const.SET_OPTIONS_CANT_CHANGE,
            const.SET_OPTIONS_UNKNOWN_FLAG,
            const.SET_OPTIONS_THRESHOLD_OUT_OF_RANGE,
            const.SET_OPTIONS_BAD_SIGNER,
            const.SET_OPTIONS_INVALID_HOME_DOMAIN,
        ]:
    ...
    def pack_ChangeTrustResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.CHANGE_TRUST_SUCCESS,
            const.CHANGE_TRUST_MALFORMED,
            const.CHANGE_TRUST_NO_ISSUER,
            const.CHANGE_TRUST_INVALID_LIMIT,
            const.CHANGE_TRUST_LOW_RESERVE,
            const.CHANGE_TRUST_SELF_NOT_ALLOWED,
        ]:
    ...
    def pack_AllowTrustResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.ALLOW_TRUST_SUCCESS,
            const.ALLOW_TRUST_MALFORMED,
            const.ALLOW_TRUST_NO_TRUST_LINE,
            const.ALLOW_TRUST_TRUST_NOT_REQUIRED,
            const.ALLOW_TRUST_CANT_REVOKE,
            const.ALLOW_TRUST_SELF_NOT_ALLOWED,
        ]:
    ...
    def pack_AccountMergeResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.ACCOUNT_MERGE_SUCCESS,
            const.ACCOUNT_MERGE_MALFORMED,
            const.ACCOUNT_MERGE_NO_ACCOUNT,
            const.ACCOUNT_MERGE_IMMUTABLE_SET,
            const.ACCOUNT_MERGE_HAS_SUB_ENTRIES,
            const.ACCOUNT_MERGE_SEQNUM_TOO_FAR,
            const.ACCOUNT_MERGE_DEST_FULL,
        ]:
    ...
    def pack_InflationResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.INFLATION_SUCCESS,
            const.INFLATION_NOT_TIME,
        ]:
    ...
    def pack_ManageDataResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.MANAGE_DATA_SUCCESS,
            const.MANAGE_DATA_NOT_SUPPORTED_YET,
            const.MANAGE_DATA_NAME_NOT_FOUND,
            const.MANAGE_DATA_LOW_RESERVE,
    ...
    def pack_BumpSequenceResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.BUMP_SEQUENCE_SUCCESS,
            const.BUMP_SEQUENCE_BAD_SEQ,
        ]:
    ...
    def pack_OperationResultCode(self, data):
        ...
        if self.check_enum and data not in [
            ...
            const.opBAD_AUTH,
            const.opNO_ACCOUNT,
            const.opNOT_SUPPORTED,
            const.opTOO_MANY_SUBENTRIES,
            const.opEXCEEDED_WORK_LIMIT,
        ]:
    ...
    def pack_OperationResult(self, data):
        ...
        if data.code == const.opINNER:
            ...
            if data.tr.type == const.CREATE_ACCOUNT:
                ...
                self.pack_PathPaymentStrictReceiveResult(
                    data.tr.pathPaymentStrictReceiveResult
                )
                ...
                self.pack_PathPaymentStrictSendResult(
                    data.tr.pathPaymentStrictSendResult
                )
            else:
    ...
    def pack_TransactionResultCode(self, data):
        ...
        if self.check_enum and data not in [
            const.txFEE_BUMP_INNER_SUCCESS,
            const.txSUCCESS,
            const.txFAILED,
            const.txTOO_EARLY,
            const.txTOO_LATE,
            const.txMISSING_OPERATION,
            const.txBAD_SEQ,
            const.txBAD_AUTH,
            const.txINSUFFICIENT_BALANCE,
            const.txNO_ACCOUNT,
            const.txINSUFFICIENT_FEE,
            const.txBAD_AUTH_EXTRA,
            const.txINTERNAL_ERROR,
            const.txNOT_SUPPORTED,
            const.txFEE_BUMP_INNER_FAILED,
        ]:
    ...
    def pack_InnerTransactionResult(self, data):
        ...
        if data.result.code == const.txSUCCESS or data.result.code == const.txFAILED:
            ...
            data.result.code == const.txTOO_EARLY
            or data.result.code == const.txTOO_LATE
            or data.result.code == const.txMISSING_OPERATION
            or data.result.code == const.txBAD_SEQ
            or data.result.code == const.txBAD_AUTH
            or data.result.code == const.txINSUFFICIENT_BALANCE
            or data.result.code == const.txNO_ACCOUNT
            or data.result.code == const.txINSUFFICIENT_FEE
            or data.result.code == const.txBAD_AUTH_EXTRA
            or data.result.code == const.txINTERNAL_ERROR
            or data.result.code == const.txNOT_SUPPORTED
        ):
            pass
        else:
    ...
    def pack_TransactionResult(self, data):
        ...
        self.pack_TransactionResultCode(data.result.code)
        if (
            data.result.code == const.txFEE_BUMP_INNER_SUCCESS
            or data.result.code == const.txFEE_BUMP_INNER_FAILED
        ):
    ...
    def pack_MessageType(self, data):
        ...
        if self.check_enum and data not in [
            ...
            const.TX_SET,
            const.TRANSACTION,
            const.GET_SCP_QUORUMSET,
            const.SCP_QUORUMSET,
            const.SCP_MESSAGE,
            const.GET_SCP_STATE,
            const.HELLO,
            const.SURVEY_REQUEST,
...
class StellarXDRUnpacker(xdrlib.Unpacker):
    ...
    def unpack_SCPStatement(self):
        ...
        if data.pledges.type == const.SCP_ST_PREPARE:
            ...
            data.pledges.prepare.preparedPrime = self.unpack_array(
                self.unpack_SCPBallot
            )
            if len(data.pledges.prepare.preparedPrime) > 1 and self.check_array:
                raise XDRError(
                    "array length too long for data.pledges.prepare.preparedPrime"
    ...
    def unpack_Operation(self):
        ...
        if data.body.type == const.CREATE_ACCOUNT:
            ...
            data.body.pathPaymentStrictReceiveOp = (
                self.unpack_PathPaymentStrictReceiveOp()
            )
        elif data.body.type == const.MANAGE_SELL_OFFER:
    ...
    def unpack_OperationResult(self):
        ...
        if data.code == const.opINNER:
            ...
            if data.tr.type == const.CREATE_ACCOUNT:
                ...
                data.tr.pathPaymentStrictReceiveResult = (
                    self.unpack_PathPaymentStrictReceiveResult()
                )
            elif data.tr.type == const.MANAGE_SELL_OFFER:
                data.tr.manageSellOfferResult = self.unpack_ManageSellOfferResult()
            elif data.tr.type == const.CREATE_PASSIVE_SELL_OFFER:
                data.tr.createPassiveSellOfferResult = (
                    self.unpack_ManageSellOfferResult()
                )
            elif data.tr.type == const.SET_OPTIONS:
                ...
                data.tr.pathPaymentStrictSendResult = (
                    self.unpack_PathPaymentStrictSendResult()
                )
            else:
                raise XDRError("bad switch=%s" % data.tr.type)
        else:
