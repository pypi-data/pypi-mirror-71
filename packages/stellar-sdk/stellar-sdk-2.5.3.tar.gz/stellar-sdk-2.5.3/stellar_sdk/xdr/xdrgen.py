...
def t_error(t):
    print(
        u"Illegal character {0:s} at line {1:d} type {2:s}".format(
    ...
    if id_unique(id, "enum", lineno):
        ...
        if not (value[0].isdigit() or value[0] == "-"):
            ...
            if value not in name_dict:
                error_occurred = True
                print(
                    "ERROR - can't derefence {0:s} at line {1:s}".format(value, lineno)
                )
            elif not isinstance(name_dict[value], const_info):
                error_occurred = True
                print(
                    "ERROR - reference to {0:s} at line {1:s} is not a constant".format(
                        value, lineno
                    )
                )
            else:
...
def id_unique(dict_id, name, lineno):
    ...
    if dict_id in name_dict:
        ...
        error_occurred = True
        print(
            "ERROR - {0:s} definition {1:s} at line {2:d} conflicts with {3:s}".format(
                name, dict_id, lineno, name_dict[dict_id]
            )
        )
        return False
    else:
...
class Info(object):
    ...
    def typeinit(self, varlist, prefix=indent):
        ...
        initvars = "".join(
            ["%s%sself.%s = %s\n" % (prefix, indent, var.id, var.id) for var in varlist]
        )
    ...
    def packenum(self, prefix, data="data"):
        ...
        check = (
            "%sif self.check_enum and %s not in [%s]:\n"
            "%s%sraise XDRError('value=%%s not in enum %s' %% %s)\n"
            % (prefix, data, ", ".join(varlist), prefix, indent, self.id, data)
        )
    ...
    def unpackstruct(self, prefix, data="data"):
        ...
        if isinstance(self, struct_info):
            ...
            classname = "nullclass"
        unpack = "%s%s = %s()\n" % (prefix, data, classname) + "".join(
            [l.unpackout(prefix, data) for l in self.body]
        )
    ...
    def packunion(self, prefix, data="data"):
        ...
        for l in self.body[1:-1]:
            cases = " or ".join(
                ["%s.%s == %s" % (data, switch.id, self.fullname(c)) for c in l.cases]
            )
            check = "%s%sif %s:\n" % (prefix, first, cases)
            body = "".join([d.packout(prefix + indent, data) for d in l.declarations])
            pack += check + body
            first = "el"
    ...
    def unpackunion(self, prefix, data="data"):
        ...
        for l in self.body[1:-1]:
            ...
            check = "%s%sif %s:\n" % (prefix, first, cases)
            body = "".join([d.unpackout(prefix + indent, data) for d in l.declarations])
        ...
        unpack += "%selse:\n" % (prefix)
        if default != []:
            unpack += "".join([d.unpackout(prefix + indent, data) for d in default])
    ...
    def xdrbody(self, prefix=""):
        ...
        if self.type == "enum":
            ...
            for l in self.body[1:-1]:
                body += "".join(["%scase %s:\n" % (prefix, case) for case in l.cases])
                body += "".join(
...
class enum_info(Info):
    ...
    def unpack_output(self):
        ...
        return header + self.unpackenum(indent2) + self._get_unpack_footer()
...
class struct_info(Info):
    ...
    def unpack_output(self):
        header = "%sdef unpack_%s(self):\n" % (indent, self.id)
        return header + self.unpackstruct(indent2) + self._get_unpack_footer()
...
class union_info(Info):
    ...
    def unpack_output(self):
        header = "%sdef unpack_%s(self):\n" % (indent, self.id)
        return header + self.unpackunion(indent2) + self._get_unpack_footer()
...
class type_info(Info):
    ...
    def unpack_output(self):
        ...
        header = "%sdef unpack_%s(self):\n" % (indent, self.id)
        return header + self._unpack_array(indent2) + self._get_unpack_footer()
    ...
    def _pack_array(self, prefix, data="data"):
        if self.fixed or self.len is None:
            ...
            limit = (
                "%sif len(%s) > %s and self.check_array:\n"
                "%s%sraise XDRError("
...
packer_start = "".join(
    ["%spack_%s = xdrlib.Packer.%s\n" % (indent, k, v) for k, v in known_basics.items()]
)
...
def run(infile, filters=True, pass_attrs=True, debug=False):
    ...
    # stellar XDR dir/files
        ...
        global file_list
        file_list = [
            os.path.abspath(os.path.join(infile, x)) for x in os.listdir(infile)
        ]
        while file_list:
    ...
    print(
        "Will use output files {0:s}.py, {1:s}.py, and {2:s}.py".format(
