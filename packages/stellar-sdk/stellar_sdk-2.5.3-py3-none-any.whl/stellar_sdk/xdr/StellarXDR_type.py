...
class SignerKey:
    ...
    switch = property(
        lambda s: {
            const.SIGNER_KEY_TYPE_ED25519: s.ed25519,
            const.SIGNER_KEY_TYPE_PRE_AUTH_TX: s.preAuthTx,
            const.SIGNER_KEY_TYPE_HASH_X: s.hashX,
        }[s.type]
    )
...
class LedgerHeader:
    ...
    def __init__(
        self,
        ledgerVersion=None,
        previousLedgerHash=None,
        scpValue=None,
        txSetResultHash=None,
        bucketListHash=None,
        ledgerSeq=None,
        totalCoins=None,
        feePool=None,
...
class LedgerUpgrade:
    ...
    def __init__(
        self,
        type=None,
        newLedgerVersion=None,
        newBaseFee=None,
        newMaxTxSetSize=None,
        newBaseReserve=None,
    ):
        self.type = type
    ...
    switch = property(
        lambda s: {
            const.LEDGER_UPGRADE_VERSION: s.newLedgerVersion,
            const.LEDGER_UPGRADE_BASE_FEE: s.newBaseFee,
            const.LEDGER_UPGRADE_MAX_TX_SET_SIZE: s.newMaxTxSetSize,
            const.LEDGER_UPGRADE_BASE_RESERVE: s.newBaseReserve,
        }[s.type]
    )
...
class BucketEntry:
    ...
    switch = property(
        lambda s: {
            const.LIVEENTRY: s.liveEntry,
            const.INITENTRY: s.liveEntry,
            const.DEADENTRY: s.deadEntry,
            const.METAENTRY: s.metaEntry,
        }[s.type]
    )
...
class LedgerEntryChange:
    ...
    switch = property(
        lambda s: {
            const.LEDGER_ENTRY_CREATED: s.created,
            const.LEDGER_ENTRY_UPDATED: s.updated,
            const.LEDGER_ENTRY_REMOVED: s.removed,
            const.LEDGER_ENTRY_STATE: s.state,
        }[s.type]
    )
...
class Asset:
    ...
    switch = property(
        lambda s: {
            const.ASSET_TYPE_NATIVE: None,
            const.ASSET_TYPE_CREDIT_ALPHANUM4: s.alphaNum4,
            const.ASSET_TYPE_CREDIT_ALPHANUM12: s.alphaNum12,
        }[s.type]
    )
...
class TrustLineEntry:
    ...
    # };
    def __init__(
        self, accountID=None, asset=None, balance=None, limit=None, flags=None, ext=None
    ):
        self.accountID = accountID
        self.asset = asset
        self.balance = balance
...
class MuxedAccount:
    ...
    switch = property(
        lambda s: {
            const.KEY_TYPE_ED25519: s.ed25519,
            const.KEY_TYPE_MUXED_ED25519: s.med25519,
        }[s.type]
    )
...
class ManageSellOfferOp:
    ...
    # };
    def __init__(
        self, selling=None, buying=None, amount=None, price=None, offerID=None
    ):
        self.selling = selling
        self.buying = buying
        self.amount = amount
...
class ManageBuyOfferOp:
    ...
    # };
    def __init__(
        self, selling=None, buying=None, buyAmount=None, price=None, offerID=None
    ):
        self.selling = selling
        self.buying = buying
...
class Memo:
    ...
    switch = property(
        lambda s: {
            const.MEMO_NONE: None,
            const.MEMO_TEXT: s.text,
            const.MEMO_ID: s.id,
            const.MEMO_HASH: s.hash,
            const.MEMO_RETURN: s.retHash,
        }[s.type]
    )
...
class TransactionEnvelope:
    ...
    switch = property(
        lambda s: {
            const.ENVELOPE_TYPE_TX_V0: s.v0,
            const.ENVELOPE_TYPE_TX: s.v1,
            const.ENVELOPE_TYPE_TX_FEE_BUMP: s.feeBump,
        }[s.type]
    )
...
class PathPaymentStrictReceiveResult:
    ...
    switch = property(
        lambda s: {
            const.PATH_PAYMENT_STRICT_RECEIVE_SUCCESS: s.success,
            const.PATH_PAYMENT_STRICT_RECEIVE_NO_ISSUER: s.noIssuer,
        }.get(s.code, None)
    )
    ...
    def __repr__(self):
        ...
        if self.code is not None:
            out += [
                "code=%s"
                % const.PathPaymentStrictReceiveResultCode.get(self.code, self.code)
            ]
...
class PathPaymentStrictSendResult:
    ...
    switch = property(
        lambda s: {
            const.PATH_PAYMENT_STRICT_SEND_SUCCESS: s.success,
            const.PATH_PAYMENT_STRICT_SEND_NO_ISSUER: s.noIssuer,
        }.get(s.code, None)
    )
    ...
    def __repr__(self):
        ...
        if self.code is not None:
            out += [
                "code=%s"
                % const.PathPaymentStrictSendResultCode.get(self.code, self.code)
            ]
        if self.success is not None:
            out += ["success=%s" % repr(self.success)]
        if self.noIssuer is not None:
...
class ManageSellOfferResult:
    ...
    switch = property(
        lambda s: {const.MANAGE_SELL_OFFER_SUCCESS: s.success,}.get(s.code, None)
    )
    ...
    def __repr__(self):
        ...
        if self.code is not None:
            out += [
                "code=%s" % const.ManageSellOfferResultCode.get(self.code, self.code)
            ]
        if self.success is not None:
            out += ["success=%s" % repr(self.success)]
...
class ManageBuyOfferResult:
    ...
    switch = property(
        lambda s: {const.MANAGE_BUY_OFFER_SUCCESS: s.success,}.get(s.code, None)
    )
    ...
    def __repr__(self):
        ...
        if self.code is not None:
            out += [
                "code=%s" % const.ManageBuyOfferResultCode.get(self.code, self.code)
            ]
        if self.success is not None:
            out += ["success=%s" % repr(self.success)]
...
class AccountMergeResult:
    ...
    switch = property(
        lambda s: {const.ACCOUNT_MERGE_SUCCESS: s.sourceAccountBalance,}.get(
            s.code, None
        )
    )
...
class SurveyRequestMessage:
    ...
    def __repr__(self):
        ...
        if self.commandType is not None:
            out += [
                "commandType=%s"
                % const.SurveyMessageCommandType.get(self.commandType, self.commandType)
            ]
...
class PeerStats:
    ...
    def __init__(
        ...
        versionStr=None,
        messagesRead=None,
        messagesWritten=None,
        bytesRead=None,
        bytesWritten=None,
        secondsConnected=None,
        uniqueFloodBytesRecv=None,
        duplicateFloodBytesRecv=None,
        uniqueFetchBytesRecv=None,
        duplicateFetchBytesRecv=None,
        uniqueFloodMessageRecv=None,
        duplicateFloodMessageRecv=None,
        uniqueFetchMessageRecv=None,
        duplicateFetchMessageRecv=None,
    ):
        self.id = id
        self.versionStr = versionStr
    ...
    def __repr__(self):
        ...
        if self.duplicateFloodMessageRecv is not None:
            out += [
                "duplicateFloodMessageRecv=%s" % repr(self.duplicateFloodMessageRecv)
            ]
        ...
        if self.duplicateFetchMessageRecv is not None:
            out += [
                "duplicateFetchMessageRecv=%s" % repr(self.duplicateFetchMessageRecv)
            ]
...
class TopologyResponseBody:
    ...
    def __init__(
        ...
        inboundPeers=None,
        outboundPeers=None,
        totalInboundPeerCount=None,
        totalOutboundPeerCount=None,
    ):
...
class SurveyResponseBody:
    ...
    switch = property(
        lambda s: {const.SURVEY_TOPOLOGY: s.topologyResponseBody,}[s.type]
    )
    ...
    def __repr__(self):
        ...
        if self.type is not None:
            out += [
                "type=%s" % const.SurveyMessageCommandType.get(self.type, self.type)
            ]
...
class StellarMessage:
    ...
    def __init__(
        ...
        txSet=None,
        transaction=None,
        signedSurveyRequestMessage=None,
        signedSurveyResponseMessage=None,
        qSetHash=None,
        qSet=None,
        envelope=None,
        getSCPLedgerSeq=None,
    ):
        self.type = type
        self.error = error
        self.hello = hello
    ...
    switch = property(
        lambda s: {
            const.ERROR_MSG: s.error,
            const.HELLO: s.hello,
            const.AUTH: s.auth,
            const.DONT_HAVE: s.dontHave,
            const.GET_PEERS: None,
            const.PEERS: s.peers,
            const.GET_TX_SET: s.txSetHash,
            const.TX_SET: s.txSet,
            const.TRANSACTION: s.transaction,
            const.SURVEY_REQUEST: s.signedSurveyRequestMessage,
            const.SURVEY_RESPONSE: s.signedSurveyResponseMessage,
            const.GET_SCP_QUORUMSET: s.qSetHash,
            const.SCP_QUORUMSET: s.qSet,
            const.SCP_MESSAGE: s.envelope,
            const.GET_SCP_STATE: s.getSCPLedgerSeq,
        }[s.type]
    )
    ...
    def __repr__(self):
        ...
        if self.signedSurveyRequestMessage is not None:
            out += [
                "signedSurveyRequestMessage=%s" % repr(self.signedSurveyRequestMessage)
            ]
        if self.signedSurveyResponseMessage is not None:
            out += [
                "signedSurveyResponseMessage=%s"
                % repr(self.signedSurveyResponseMessage)
            ]
