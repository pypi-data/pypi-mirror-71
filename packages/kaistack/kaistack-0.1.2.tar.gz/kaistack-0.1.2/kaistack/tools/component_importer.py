...
@click.option("--convert", is_flag=True, default=True, help='If configured, will convert from kfp component spec. If not set, the progam will detect')
def upload_component(path, name, service, convert):
    spec = ''
    if not convert:
        # Sniff the file
    ...
    try:
        store.create_component(yaml.dump(spec), name)
        ...
        store.update_component(yaml.dump(spec), name)
