...
def generateRecordLayout(record, workspace, file=sys.stdout, options=[]):
    """Generates Vertica SQL."""
    # Init
    id = record.getId()
    
    # Generate
    print("VERTICA: Generating Record "+record.getReference())
    ...
    # Generate
    if validated:
        print("Vertica: Generating table for "+record.getReference())
        print(file=file)

        print("--",record.getReference(),  file=file)            
        print("create table if not exists ", id, "(", file=file)
        ...
        for index, layout in enumerate(layouts):
            ...
            if validated:
                layout.dump()
                sql_name = layout.getFacetedPropertyValue("name[vertica]")
                if layout.hasProperty("datatype[vertica]"):
                    sql_datatype = layout.getFacetedPropertyValue("datatype[vertica]")
                else:
                    sql_datatype = infer_vertica_datatype(layout)
