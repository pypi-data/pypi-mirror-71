...
class NestedViewMixin:
    ...
    def get_nest_object(self):
        try:
            ...
            raise Exception(f"{self.lookup_url_kwarg_nest} not found in {self.kwargs}")

        try:
            return self.queryset_nest.get(**{self.pk_field_nest: pk})
        except self.queryset_nest.model.DoesNotExist:
            model_name = self.queryset_nest.model
            raise Http404(f"{model_name} with ID {pk} not found")
