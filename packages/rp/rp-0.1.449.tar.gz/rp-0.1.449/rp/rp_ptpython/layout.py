...
def status_bar(python_input):
    ...
    def get_tokens(cli):
        ...
        if not python_input.vi_mode and cli.current_buffer_name == SEARCH_BUFFER:
            ...
            result.extend([
                (TB.Key, '[F7]', toggle_microcompletions),
            ])
            if python_input.enable_microcompletions:
                append((TB.PasteModeOn, 'Micro:On  ', toggle_microcompletions))
            else:
                append((TB, 'Micro:Off ', toggle_microcompletions))
