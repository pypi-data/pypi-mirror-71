...
from snippets.snippetsDecorators import expose_method
    ...
    def parse(self, path, format, filter=None):
        ...
        if filter is not None:
            for filter_column, filter_range in filter.items():
                data_frame = data_frame[(data_frame[filter_column] >= float(filter_range[0])) & (data_frame[filter_column] <= float(filter_range[1]))]
                # data_frame = data_frame[data_frame[filter_column] >= float(filter_range[0])]
                # data_frame = data_frame[data_frame[filter_column] <= float(filter_range[1])]
