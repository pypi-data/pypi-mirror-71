...
class GitlabClient:
    ...
    async def run_pipeline(
        ...
        return await self.pipeline_to_workflow_run(pipeline, project, True)
    ...
    async def pipeline_to_workflow_run(
        self, pipeline: ProjectPipeline, project: Project, verbose: bool = True,
    ) -> WorkflowRun:
        job_runs: List[JobRun] = []
        jobs = await force_async(pipeline.jobs.list)()
        for job in jobs:
            project_job = project.jobs.get(job.id)
            job_runs.append(self.job_to_job_run(project_job, include_output=verbose))
    ...
    async def delete_job(self, project_id: Union[str, int], job_id: int):
        project = await self._get_project(project_id)
        try:
            job = await force_async(project.jobs.get)(job_id)
            if job.status in ["running", "pending"]:
                await force_async(job.cancel)()
            await force_async(job.erase)()
        except GitlabGetError:
            raise ValueError(
                f"No job found with the id {job_id} in project {project_id}"
            )
    ...
    async def get_pipeline(
        self, project_id: Union[str, int], pipeline_id: int, verbose: bool
    ) -> WorkflowRun:
        project = await self._get_project(project_id)
        try:
            pipeline = await force_async(project.pipelines.get)(pipeline_id)
            return await self.pipeline_to_workflow_run(pipeline, project, verbose)
        except GitlabGetError:
            raise ValueError(
