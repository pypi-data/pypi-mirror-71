...
class ConductorSubject(object):
    ...
    def _validate_http_rsp(self, resp):
        ...
        try:
            ...
            LOG.error(msg)
            LOG.error("")
            LOG.error("See if someone else has had this issue: https://bitbucket.org/link-labs_engineering/conductor-p"
                      "y/issues")
        if exception:
...
class DownlinkSubject(ConductorSubject):
    ...
    t_last_msg_sent = None  # Time that the last message was sent.
    last_msg_sent_args = None  # The args of the last message sent.
