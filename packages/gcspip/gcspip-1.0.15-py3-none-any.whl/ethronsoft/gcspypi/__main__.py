...
def main():
    ...
    parser.add_argument("--repository", metavar="R", type=str, nargs="?",
                        help="Specifies GCS bucket name hosting the packages. "
                             "If not provided, gcspypi will try to read it from ~/.gcspypirc")
    parser.add_argument("--cache-dir", metavar="C", type=str, nargs="?",
                        help="Specifies if a cache directory is used. "
                             "If not provided, gcspypi will remove all temp data after installation.")
    ...
    if not repository:
        ...
        rc = os.path.join(os.path.expanduser("~"), ".gcspiprc")
