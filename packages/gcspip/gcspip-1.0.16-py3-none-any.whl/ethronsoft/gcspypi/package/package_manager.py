...
class PackageInstaller(object): # pragma: no cover
    ...
    def install(self, resource, flags=[], cache_dir=None):
        resource = resource.split(' ')[0]
        pip = self.check_for_pip()
        if cache_dir:
            subprocess.check_call([pip, "--cache-dir", cache_dir, "install", resource] + flags)
        else:
            subprocess.check_call([pip, "install", resource] + flags)

    def uninstall(self, pkg):
        pip = self.check_for_pip()
        subprocess.check_call([pip, "uninstall", pkg.full_name.replace(":", "==")])

    def check_for_pip(self):
        if which('pip') is not None:
            pip = 'pip'
        elif which('pip3') is not None:
            pip = 'pip3'
        else:
            raise InvalidParameter("Could not find Python pip")
...
class PackageManager(object):
    ...
    def __init__(self, repo, console, installer=PackageInstaller(), overwrite=False, mirroring=True, install_deps=True,
        ...
        self.__prog = re.compile("((?:\w|-)*)(==|<=?|>=?)?((?:\d*\.?){0,3}((?:\w)*)?)?,?(==|<=?|>=?)?((?:\d*\.?){0,3})?")
        self.__repo_cache = []
        self.refresh_cache()
    ...
    def upload(self, pkg, filename):
        ...
        for pack in packages:
            if pack == pkg and not self.__overwrite:
    ...
    def download(self, pkg, dest, preferred_type):
        ...
        if matches:
            # we get the last one unless we find a preferred match
            to_install = matches[len(matches)-1]
            for p in matches:
    ...
    def install(self, syntax, preferred_type, no_user):
        ...
        if not is_internal:
            if self.__mirroring:
                self.__installer.install(syntax, cache_dir=self.cache_dir)
            else:
        ...
        if self.__repo_cache and not self.__overwrite:
            if interactive:  # pragma: no cover
                ok = self.__console.selection(
                    "Repository {} is not empty.\nDo you want to attempt to push "
                    "into an existing repository? [y|n]: ".format(self.__repo.name), ["y", "n"]
                )
                if ok.strip() == "y":
