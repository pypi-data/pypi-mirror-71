...
def __createModuleFunctionView__(moduleName, functionName, is_main=False):
    try:
        ...
        funcsFromFunc = None if not hasattr( ht.getModule(moduleName), '_funcArgFromFunc_') else dict(ht.getModule(moduleName).funcArgFromFunc)
        ...
        if functionParams:
            ...
            if funcsFromFunc and functionName in funcArgFromFunc and len(funcArgFromFunc[functionName]) > 0:
                for arg in funcArgFromFunc[functionName]:
                    if arg in moduleViewConfig:
                        if not 'options_from_function' in moduleViewConfig[arg]:
                            moduleViewConfig[arg]['options_from_function'] = {}
                        for moduleToExecute in funcArgFromFunc[functionName][arg]:
                            moduleViewConfig[arg]['options_from_function'][moduleToExecute] = funcArgFromFunc[functionName][arg][moduleToExecute]
        else:
