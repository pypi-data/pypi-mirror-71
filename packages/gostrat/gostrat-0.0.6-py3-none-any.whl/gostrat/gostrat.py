...
def main(args=sys.argv[1:]) :
    ...
    if args['download_go'] :
        ...
        for ns, g in ns_g.items() :
            ...
            # when there are missing terms, the graph has connected connected components
            for comp_nodes in nx.strongly_connected_components(g) :
                comp_nodes = list(comp_nodes)
                root_path = obodag.paths_to_top(comp_nodes[0])

                # if root_path is none, the node was missing
                if root_path is None :
                    assert len(comp_nodes) == 1
                    goea_df.loc[comp_nodes[0],['ns','parent_id','parent_name','term_name']] = [
                            ns,
                            comp_nodes[0],
                            comp_nodes[0],
                            'unknown'
