...
int quisk_hardware_cwkey;		// hardware CW key from UDP
...
{		// Size of complex sample array is SAMP_BUFFER_SIZE
	...
	while (nSamples < want_samples) {		// read several UDP blocks
		...
		if (quisk_hardware_cwkey != old_key) {
			old_key = quisk_hardware_cwkey;
...
{		// Set the key state internally. Treat like any other key and apply hang time.
	if (state)
		key_is_down = 1;
	else
		key_is_down = 0;
