...
static int startup_error;		// error opening port
...
PyObject * quisk_open_key(PyObject * self, PyObject * args)
{  // return a message for error, or "" for no error
	PyObject * msg;
	char * port, * cts, * dsr;

	if (!PyArg_ParseTuple (args, "sss", &port, &cts, &dsr))
		return NULL;
	...
	quisk_serial_ptt = 0;
	if (use_cts != 'N' || use_dsr != 'N') {	// try to open the serial port
		msg = start_up();
		if ( ! startup_error)
...
static HANDLE hComm;		// Windows handle to read the serial port
...
static int fdComm;			// File descriptor to read the serial port
