...
def test_force_draft(tmpdir):
    ...
    code = main.command('pull-force-topic', b'zetop',
                        source=as_bytes(fork_path), rev=[node.hex(fork_node)])
    assert code == 0
...
def test_overlapping(tmpdir):
    ...
    fork2.command('pull', as_bytes(fork1_path), rev=[node.hex(fork1_node)])
