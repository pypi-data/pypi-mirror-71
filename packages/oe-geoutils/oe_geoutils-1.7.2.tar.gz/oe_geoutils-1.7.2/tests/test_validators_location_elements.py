...
class LocatieAdresSchemaNodeLocatieElementAdresTests(unittest.TestCase):
    ...
    def test_adres_validatie_huisnummer_alphanumeric(self):
        self.data['huisnummer'] = '68#'
        with self.assertRaises(colander.Invalid) as inv:
            self.schema.deserialize(self.data)
        self.assertEqual('Het huisnummer kan enkel alfanumerieke tekens bevatten.',
                         inv.exception.msg)

    def test_adres_validatie_huisnummer_underscore(self):
        self.data['huisnummer'] = '68_12'
        res = self.schema.deserialize(self.data)
        self.assertEqual('68_12', res['huisnummer'])
    ...
    def test_postcode(self):
        ...
        with self.assertRaises(colander.Invalid) as inv:
            self.schema.deserialize(self.data)
        self.assertEqual(
            "postcode 1025 is niet correct voor dit adres, mogelijke postcode is 8300",
            inv.exception.asdict()['']
        )
