...
class LocationElementSchemaNode(colander.MappingSchema):
    ...
    def _validate_locatie_adres(self, crab_gateway, locatie_element, node):
        ...
        crab_huisnummer = self._validate_huisnummer(
            node, crab_gateway, huisnummer, huisnummer_id, straat, postcode
        )
    ...
    def _validate_huisnummer(self, node, crab_gateway, huisnummer, huisnummer_id,
        ...
        if not re.match(r'^\d+((/|bus|)(\d+(-\d+)?|[A-Z](-[A-Z])?)|[\-_]\d+)?$',
                        huisnummer):
            msg = "Het huisnummer kan enkel alfanumerieke tekens bevatten."
            ...
            if postcode is not None:
                ...
                if postcode != str(postkanton.id):
                    raise colander.Invalid(
                        ...
                        'mogelijke postcode is %s' % (postcode, postkanton.id)
                    )
