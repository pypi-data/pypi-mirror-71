...
class pi7db:
  def __init__(self,db_name,db_path=os.getcwd()):
   self.db_np,self.db_name,self.doc_size,self.temp_limt = os.path.join(db_path,db_name),db_name,15000000,1
   ...
   self.config = {'secret-key':None,'enc_type':'bin','doc_size':self.doc_size}
   writedoc(f"{self.config_file}.bin",self.config)
   self.config=opendoc(f"{self.config_file}.bin")
   #else:self.config=opendoc(f"{self.config_file}.bin")
     
  def __getattr__(self, attrname):
   if attrname == "temp":
    path=self.coll_name=os.path.join(self.db_np,attrname)
    SubClass = type(attrname,(subclass,),{'p_filter':self.filter,'p_sortdict':self.sortdict,'p_update':self.update,'p_trash':self.trash,'p_write':self.write,'config':self.config,'db_np':self.db_np,'db_name':self.db_name,"temp_limt":self.temp_limt})
    SubClass = SubClass()
    return SubClass
  ...
  def changekey(self,old_key,New_key):
   ...
   if old_key == opendoc(self.config_file)['secret-key']:
    for x_js in files:
     writedoc(x_js[:-len(self.config['enc_type'])-1],opendoc(x_js[:-len(self.config['enc_type'])-1],old_key),New_key)
  ...
  def update(self,coll_name,file_name=None,data_arg=None,**kwargs):
   if "where" in kwargs:
     ...
     if isinstance(coll_name,dict) and file_name is None:
      ...
      return True 
   try:
    js_data=opendoc(f"{self.db_np}/{coll_name}/{file_name}.{self.config['enc_type']}",self.config['secret-key'])
    if isinstance(data_arg,dict):js_data=nes_update(js_data,data_arg)
    else:return error.e2
    writedoc(f"{self.db_np}/{coll_name}/{file_name}.{self.config['enc_type']}",js_data,self.config['secret-key'])
  ...
  def read(self,coll_name=None,file_name=None,key_name=None,**kwargs):
   ...
   if key_name is not None:return {"data":opendoc(f"{self.db_np}/{coll_name}/{file_name}.{self.config['enc_type']}",self.config['secret-key'])[key_name],"status":1}
  ...
  def trash(self,coll_name=None,file_name=None,key_name=None,**kwargs):
   ...
   if key_name is not None and isinstance(key_name,set) or isinstance(key_name,dict):
     tr_data = opendoc(f"{self.db_np}/{coll_name}/{file_name}.{self.config['enc_type']}",self.config['secret-key'])
     writedoc(f"{self.db_np}/{coll_name}/{file_name}.{self.config['enc_type']}",nes_trash(tr_data,key_name),self.config['secret-key'])
