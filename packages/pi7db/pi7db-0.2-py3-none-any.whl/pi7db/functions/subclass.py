...
class subclass:
  ...
  def read(self,file_name=None,key_name=None,**kwargs):
   ...
   if key_name is not None:return {"data":opendoc(f"{self.db_np}/$temp/{file_name}.{self.config['enc_type']}",self.config['secret-key'])[key_name],"status":1}
   elif file_name is not None:data_files=[f"{self.db_np}/$temp/{file_name}.{self.config['enc_type']}"]
