...
class hf_optimizer:
    ...
    def __init__(self, p, inputs, s, costs, h=None, ha=None):
        ...
        self.f_gc = theano.function(
            inputs, g + costs, on_unused_input='ignore')
        # for quick cost evaluation
        self.f_cost = theano.function(inputs, costs, on_unused_input='ignore')
        ...
        if h is not None:  # structural damping with cross-entropy
            ...
            structural_damping = coefficient * \
                (-h_constant*T.log(h + 1e-10) - (1-h_constant)
                 * T.log((1-h) + 1e-10)).sum() / h.shape[0]
            if ha is None:
                ha = h
    ...
    def quick_cost(self, delta=0):
        ...
        cost = numpy.mean([self.f_cost(*i)[0]
    ...
    def cg(self, b):
        if self.preconditioner:
            ...
            for inputs in self.cg_dataset.iterate(update=False):
                ...
                M += self.list_to_flat(self.f_gc(*inputs)[:len(self.p)])**2
    ...
    def batch_Gv(self, vector, lambda_=None):
        ...
        for inputs in self.cg_dataset.iterate(False):
            result += self.list_to_flat(self.function_Gv(
                *(inputs + v + [lambda_*self.mu]))) / self.cg_dataset.number_batches
        return result
    ...
    def train(self, gradient_dataset, cg_dataset, initial_lambda=0.1, mu=0.03, global_backtracking=False, preconditioner=False, max_cg_iterations=250, num_updates=100, validation=None, validation_frequency=1, patience=numpy.inf, save_progress=None):
        ...
        if isinstance(save_progress, str) and os.path.isfile(save_progress):
            save = cPickle.load(open(save_progress, 'rb'))
        ...
        try:
            for u in range(first_iteration, 1 + num_updates):
                ...
                delta_cost = numpy.dot(
                    flat_delta, gradient + 0.5*self.batch_Gv(flat_delta, lambda_=0))  # disable damping
                before_cost = self.quick_cost()
                for i, delta in zip(self.p, self.flat_to_list(flat_delta)):
                    i.set_value(i.get_value() + delta)
                cg_dataset.update()

                rho = (after_cost - before_cost) / \
                    delta_cost  # Levenberg-Marquardt
                # print 'rho=%f' %rho,
                if rho < 0.25:
                    self.lambda_ *= 1.5
                elif rho > 0.75:
                ...
                if validation is not None and u % validation_frequency == 0:
                    if hasattr(validation, 'iterate'):
                        costs = numpy.mean([self.f_cost(*i)
                                            for i in validation.iterate()], axis=0)
                    elif callable(validation):
                        costs = validation()
                    print('validation=', costs)
                    if costs[0] < best[1]:
                        best = u, costs[0], [i.get_value().copy()
                                             for i in self.p]
                        print('*NEW BEST')
                ...
                if isinstance(save_progress, str):
                    ...
                    cPickle.dump(save, open(save_progress, 'wb'),
    ...
    def __init__(self, data, batch_size, number_batches, minimum_size=10):
        ...
        for i_sequence in range(len(data[0])):
            if batch_size is None:
                self.items.append([data[i][i_sequence]
                                   for i in range(len(data))])
            else:
                for i_step in range(0, len(data[0][i_sequence]) - minimum_size + 1, batch_size):
                    self.items.append(
                        [data[i][i_sequence][i_step:i_step + batch_size] for i in range(len(data))])
