...
class sync:
    ...
    def synctable(self, sourceDb, targetDb, sourceTable, targetTable):
        ...
        if targetTable.lastUpdatetime > sourceTable.lastUpdatetime:
            pbar.set_description("%s timestamp bigger, skip." % (targetTable.tablename))
            ...
            pbar.set_description("%s RowCount bigger but timestamp the same, skip." % (targetTable.tablename))
        elif targetTable.lastUpdatetime == sourceTable.lastUpdatetime and targetTable.rowCount < sourceTable.rowCount:
            pbar.set_description("%s RowCount less but timestamp the same, skip." % (targetTable.tablename))
        pbar.close()
