...
def detect_repo(ctx, github_url, github_ref, force, description, commit_ref, allow_missing=True):
    ...
    for repo_spec in repo_specs:
        ...
        try:
            workspace_id, commit_hash, _, _ = push_workspace(
                ctx, git_repo, commit_ref, force=force, include_uncommitted=include_uncommitted,
            )
...
def push_workspace(ctx, git_repo, commit_ref, force=False, include_uncommitted=False):
    ...
    if has_untracked(git_repo):
        if not force:
            click.confirm(
                "There are some untracked files in repo {}. They won't be available on this run."
                "\n{}\nContinue the run anyway?".format(
                    git_repo.working_dir, get_untracked(git_repo)
                ),
                default=True,
                abort=True,
            )
