...
class ModelVersion(Model):
    ...
    @property
    def specifier(self):
        return ":".join((self.model_name, self.version)) if self.model_name else ""


class PodAutoscaleConfig(Model):
    compare_fields = ["min_pods", "max_pods", "target_cpu_utilization"]

    def __init__(self, min_pods, max_pods, target_cpu_utilization=None):
...
class ContainerResourceRequirements(Model):
    ...
    def __init__(self, resource_request=None, resource_limit=None):
...
class ResourceRequirement(Model):
    compare_fields = ["memory_mebibytes", "cpu_minicores"]

    def __init__(self, memory_mebibytes=None, cpu_minicores=None):
        self.memory_mebibytes = memory_mebibytes
        self.cpu_minicores = cpu_minicores

    def to_payload(self):
        return {
            "memory_mebibytes": self.memory_mebibytes,
            "cpu_minicores": self.cpu_minicores,
        }
