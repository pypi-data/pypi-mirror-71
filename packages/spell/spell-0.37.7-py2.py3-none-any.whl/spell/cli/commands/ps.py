...
@click.pass_context
def ps(ctx, number, raw, trunc, repository, labels, verbose, include_jupyter):
    """
    ...
    with api_client_exception_handler():
        ...
        if repository is None:
            runs = client.list_runs(number=number, include_jupyter=include_jupyter, labels=labels)
        else:
