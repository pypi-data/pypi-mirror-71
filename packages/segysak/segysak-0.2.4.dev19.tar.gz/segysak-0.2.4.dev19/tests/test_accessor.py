...
class TestCreate_xysel:
    ...
    def test_full_stack_dataset_xysel(self, d, translate, scale, shear, rotate, samp):
        ...
        ix_pairs = np.apply_along_axis(trsfm, 1, ix_pairs.reshape(-1, 2)).reshape(ix_pairs.shape)
    ...
    def test_angle_stack_dataset_xysel(self, o, f, s, translate, scale, shear, rotate, samp):
        dims = (50, 30, 5, o)
        dataset = create3d_dataset(
            dims, first_offset=f, offset_step=s
        )
        xlines_, ilines_ = np.meshgrid(dataset.xline, dataset.iline)
        ix_pairs = np.dstack([ilines_, xlines_])
        tr = Affine.translation(*translate)
