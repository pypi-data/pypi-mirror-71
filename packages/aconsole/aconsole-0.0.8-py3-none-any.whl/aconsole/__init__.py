...
class InputCancelledException(Exception):
    def __init__(self, message='input cancelled'):
        super().__init__(message)

class ConsoleClosedException(Exception):
    def __init__(self, message='console closed'):
        super().__init__(message)
...
class AsyncConsole(tkinter.Tk):
    ...
    def __init_ui(self):
        ...
        self.__output_text.bind('<Control-c>', self.__output_copy_selection)
        self.__output_text.bind('<Control-r>', self.clear_output)
        ...
        self.__input_text.bind('<Control-c>', (lambda _: self.cancel_input()))
    ...
    def __output_copy_selection(self, *args):
        try:
    ...
    async def __input_loop(self):
        while self.__running:
            ...
            except InputCancelledException:
                pass
            except ConsoleClosedException:
                break
    ...
    def cancel_input(self):
        if not self.__destroyed and self.__input_future and not self.__input_future.done():
            self.__input_disable()
            self.__input_future.set_exception(InputCancelledException())
    ...
    async def mainloop(self):
        ...
        try:
            ...
            if self.__input_future and not self.__input_future.done():
                self.__input_future.set_exception(ConsoleClosedException())
            ...
            while not self.__input_queue.empty():
                ...
                future.set_exception(ConsoleClosedException())
