...
# 				read_concept_data (tushare不全，暂未开通此功能)
# @Author: Yulin Qiu
...
class DBReader:
	...
	def __init__(self, *args):
		...
		try:
			...
			try:
				...
				# 目前正常交易股票代码和名称
				allcode = pro.stock_basic(exchange='', fields='ts_code')
				allcode['ts_code'] = allcode['ts_code'].apply(lambda x: int(x[0:6]))
				self.allcode = allcode['ts_code'].to_list()
				...
				# 退市股
				self.delcode = delcode['code'].to_list()
				# 非ST股票
				...
				self.stcode = df_st
				# self.stcode = codename[codename['name'].str.contains('\*|ST')].code.to_list()
				# 交易日历——日
				self.open_date = df_opendate.apply(lambda x: x[0:4] + '-' + x[4:6] + '-' + x[6:])
			except:
				record = pd.read_json(configpath.joinpath('record_api_message.json'))
				self.delcode = record['delcode'][0]
				# self.stcode = record['stcode'][0]
				self.stcode = pd.DataFrame(record['stcode'][0])
	...
	# 确定shift和forward时间
	def check_timechange(self, start, end, shift, forward, minmode=False):
		if not isinstance(shift, int) or not isinstance(forward, int) or shift < 0 or forward < 0:
			...
			# 	time_choose = ''
			# else:
			# 	time_choose = timesector[timesector >= start]
			# 	time_choose = time_choose[time_choose <= end]
			# 	time_choose.iloc[-1] = end.replace(hour=0)
			# 	time_choose = time_choose.apply(lambda x: '{0}{1}{0}'.format('"', str(x))).tolist()
			# 	time_choose = ','.join(time_choose)
			# 	time_choose = ' AND ' + dbname_date + ' IN (' + time_choose + ') '
			# 	# time_choose = '{2}{0}{1}'.format(time_choose, ')', ' ' + dbname_date + ' IN (')
			# 	# time_choose = '{0}{1}{2}'.format(' AND (', time_choose, ')')
			return str_start, str_end

	# 检查股票代码输入是否正确并生成sql的code查询语句
	# @staticmethod
	def check_code(self, code, dbname_code, indexmode=False):
		if code is None:
			# str_code = ' '
			if indexmode:
	...
	@staticmethod
	def check_fields(fields, standard_fields):
		# 全选处理
		if fields == '*':
			return fields
		else:
	...
	@staticmethod
	def fields_append(fields, time_start=None, time_end=None, noncode=False):
	...
	def check_distinct(distinct):
		if isinstance(distinct, bool):
			...
			raise ValueError('distinct参数必须为bool值,当前输入：{}'.format(distinct))
	...
	def __st_clean(self, df, dbname_code, start, fields, stdrop):
		...
		if (dbname_code in fields and 'date' in fields) or fields == '*':
			df_stcode = self.stcode[self.stcode['end_date'] >= start]
			...
			df_stcode.set_index(keys='code', inplace=True)
			df['isinst'] = df['code'].apply(lambda x: 1 if x in df_stcode.index else 0)
			df['isst'] = df.apply(
				lambda x: df_stcode.loc[x['code'], 'start_date'] <= x['date'] <= df_stcode.loc[x['code'], 'end_date']
				if x['isinst'] == 1 else 0, axis=1)
			df = df[df['isst'] == 0]
			# df = df[~df[dbname_code].isin(self.stcode)]
			df = df.drop(['isinst', 'isst'], axis=1)
	...
	def __del_clean(self, df, dbname_code, fields, deldrop):
		...
		if dbname_code in fields or fields == '*':
			df = df[~df[dbname_code].isin(self.delcode)]
	...
	def read_ts_day_data(
		...
		assert 'tushare_data' in self.file_used, \
			'配置文件无tushare_data库目录，禁止访问该接口，请确保tushare_data库存在后执行配置文件生成firsttime_setconfig'
		dbname_date = 'date'
		dbname_code = 'code'
		...
		str_start, str_end = self.check_timechange(start=start_date, end=end_date, shift=shift, forward=forward)
		str_code = self.check_code(code=code, dbname_code=dbname_code)
		str_fields = self.check_fields(fields=fields, standard_fields=col)
		fieldsdrop, str_fields = self.fields_append(str_fields)
		...
		try:
			connect_tushare_data = sqlite3.connect(Path(self.config.loc[0, 'dbpath']).joinpath('tushare_data.sqlite3'))
			df = pd.read_sql(str_sql, con=connect_tushare_data)
			connect_tushare_data.close()
		except Exception:
			raise ConnectionError('数据库数据获取失败，连接异常，检查参数或联系管理员')
		if len(df) > 0:
			...
			df = self.fields_drop(df=df, fieldsdrop=fieldsdrop)
		else:
			pass
		return df
	...
	def read_ts_index_daily(
		...
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
		str_code = self.check_code(code=code, dbname_code=dbname_code, indexmode=True)
		str_start, str_end = self.check_timechange(start=start_date, end=end_date, shift=shift, forward=forward)
		str_fields = self.check_fields(fields=fields, standard_fields=col)
		fieldsdrop, str_fields = self.fields_append(str_fields)
	...
	def read_ts_moneyflow_hsgt(
		...
		str_start, str_end = self.check_timechange(start=start_date, end=end_date, shift=shift, forward=forward)
		str_fields = self.check_fields(fields=fields, standard_fields=col)
		fieldsdrop, str_fields = self.fields_append(str_fields, noncode=True)
		str_sql = \
	...
	def read_tdx_1min_data(
		...
		assert 'tdx_1min_data' in self.file_used, \
			'配置文件无tdx_1min_data库目录，禁止访问该接口，请确保tdx_1min_data库存在后执行配置文件生成firsttime_setconfig'
		...
		end_date = self.check_time(time=end)
		str_code = self.check_code(code=code, dbname_code=dbname_code)
		str_start, str_end = self.check_timechange(start=start_date, end=end_date, shift=shift, forward=forward, minmode=True)
		str_fields = self.check_fields(fields=fields, standard_fields=col)
		fieldsdrop, str_fields = self.fields_append(str_fields)
		str_time = self.check_mintime(start=time_start, end=time_end)
		str_sql = \
			'SELECT ' + str_fields + ' FROM ' + 'tdx_1min_data' + ' WHERE ' + str_code + ' (' + \
			dbname_date + '>=' + str_start + ' AND ' + dbname_date + '<=' + str_end + str_time + ')'
		try:
			connect_tdx_1min_data = sqlite3.connect(Path(self.config.loc[0, 'dbpath']).joinpath('tdx_1min_data.sqlite3'))
			# s = datetime.datetime.now()
			df = pd.read_sql(str_sql, con=connect_tdx_1min_data)
			# ss = datetime.datetime.now()
			# print(ss - s)
			connect_tdx_1min_data.close()
		except Exception:
			raise ConnectionError('数据库数据获取失败，连接异常，检查参数或联系管理员')
		df = self.fields_drop(df=df, fieldsdrop=fieldsdrop)
		return df
	...
	def read_jj_1min_index_data(
		...
		assert 'jj_data' in self.file_used, \
			'配置文件无tushare_data库目录，禁止访问该接口，请确保tushare_data库存在后执行配置文件生成firsttime_setconfig'
		dbname_date = 'datetime'
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
	...
	def read_tdx_5min(
		...
		assert 'tdx_data_5min' in self.file_used, \
			'配置文件无tdx_data_5min库目录，禁止访问该接口，请确保tdx_data_5min库存在后执行配置文件生成firsttime_setconfig'
		dbname_date = 'datetime'
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
		...
		end_date = self.check_time(time=end)
		str_code = self.check_code(code=code, dbname_code=dbname_code)
		str_start, str_end = self.check_timechange(
		...
		str_sql = \
			...
			dbname_date + '>=' + str_start + ' AND ' + dbname_date + '<=' + str_end + ')'
		try:
			connect_tdx_5min_data = sqlite3.connect(Path(self.config.loc[0, 'dbpath']).joinpath('tdx_data_5min.sqlite3'))
			df = pd.read_sql(str_sql, con=connect_tdx_5min_data)
			connect_tdx_5min_data.close()
		except Exception:
			raise ConnectionError('数据库数据获取失败，连接异常，检查参数或联系管理员')
		return df
	...
	def read_fundamentals(
		...
		assert 'fundamentals' in self.file_used, \
			'配置文件无fundamentals库目录，禁止访问该接口，请确保fundamentals库存在后执行配置文件生成firsttime_setconfig'
		dbname_date = 'date'
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
	...
	def read_strategy(
		...
		assert 'strategy_data' in self.file_used, \
			'配置文件无fundamentals库目录，禁止访问该接口，请确保fundamentals库存在后执行配置文件生成firsttime_setconfig'
		dbname_date = 'date'
		dbname_code = 'code'
		try:
		...
		str_start, str_end = self.check_timechange(start=start_date, end=end_date, shift=shift, forward=forward)
		str_code = self.check_code(code=code, dbname_code=dbname_code)
		str_fields = self.check_fields(fields=fields, standard_fields=standard_fields)
		fieldsdrop, str_fields = self.fields_append(str_fields)
	...
	def read_mk_tech_day_data(
		...
		assert 'mk_data' in self.file_used, \
			'配置文件无tushare_data库目录，禁止访问该接口，请确保tushare_data库存在后执行配置文件生成firsttime_setconfig'
		dbname_date = 'date'
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
	...
	def read_cal_market_1min_data(
		...
		assert 'cal_market_data' in self.file_used, \
			'配置文件无tushare_data库目录，禁止访问该接口，请确保tushare_data库存在后执行配置文件生成firsttime_setconfig'
		dbname_date = 'datetime'
		col = self.get_tableinfo_cols(
...
if __name__ == '__main__':
	...
	# SetConfig.renew_config(configpath='../test_data/')
	...
	test13,_ = test.read_concept_data(dfconcept=True)
