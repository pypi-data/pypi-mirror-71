...
def get_class(module_label, classname, module_prefix='webspace'):
    return get_classes(module_label, [classname], module_prefix)[0]


def get_class_loader():
    return import_string('webspace.loader.default_class_loader')
...
def _pluck_classes(modules, classnames):
    ...
    for classname in classnames:
        ...
        if not klass:
            ...
            raise Exception("No class '%s' found in %s" % (
