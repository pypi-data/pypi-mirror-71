...
class DummyClass:
    ...
    def __getattr__(self, name):
        raise ImportError('{0} is not installed. Failed calling {1}()'.format(self.name, name))
...
# -----------------------------
@import_wrapper(DummyClass("dativa.tools.SqlClient"))
def get_sql_client():
    from dativa.tools import SqlClient
    return SqlClient


SqlClient = get_sql_client()
...
# -----------------------------
@import_wrapper(DummyClass("dativa.tools.AthenaClient"))
def get_athena_client():
    from dativa.tools.aws import AthenaClient
    return AthenaClient


AthenaClient = get_athena_client()
