...
from newtools import CachedPep249Query, CachedAthenaQuery, BaseCachedQuery, PandasDoggo


class TestCachedQuery(BaseTest):
    s3_bucket = "s3://aws-athena-query-results-933373196108-us-west-2/"
    region = "us-west-2"
    ...
    def test_log_pep249(self):
        ...
        with self.assertLogs("dativa.attribution.cached_query", level="INFO") as cm:
        ...
        self.assertIn('INFO:dativa.attribution.cached_query:secret = *********', cm.output)
    ...
    def test_s3_archive(self):
        ...
        q.clear_cache(sql_file="athena_test.sql",
                      ...
                      params={"day_list": ['{0:%Y-%m-%d}'.format(day) for day in days]})
        try:
            q._s3fs.rm(os.path.join(self.s3_bucket, "athena_test.sql"))
        ...
        # check the archive.
        query = q._s3fs.open(os.path.join(self.s3_bucket, "athena_test.sql"), "rt").read()
    ...
    def test_multiple_output_file(self):
        class MimicRedshiftQueryMultiple(CachedAthenaQuery):
            ...
            def _execute_query(self,
                ...
                self._s3fs.cp(output_file + "000.gz", output_file + "001.gz")
