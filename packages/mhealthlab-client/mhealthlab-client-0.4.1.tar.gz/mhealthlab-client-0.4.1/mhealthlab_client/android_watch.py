    ...
    def convert_to_actigraph(self, date_range=None, sr=50):
        ...
        for i in range(len(sub_session_markers) - 1):
            ...
            output_path = os.path.join(
                self._root, 'signaligner', f'{st_display}_{et_display}_sensors', f'{self._pid}_Android_{sensor_type}_{st_display}_{et_display}.sensor.csv')
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            output_annotation_path = os.path.join(os.path.dirname(
                output_path.replace('sensors', 'labelsets')), f'{self._pid}_missing_{sensor_type}_{st_display}_{et_display}.annotation.csv')
            os.makedirs(os.path.dirname(
                output_annotation_path), exist_ok=True)
            arus.plugins.signaligner.signify_sensor_files(
                filepaths, sensor_type, output_path, output_annotation_path, sub_session_span, sr)
