...
def duplicates_detection(data, i):
    ...
    if i < len(data):
        ...
        if (data["possible_duplicates"].iloc[i][0]["screen_name"] =="Empty"):
            num_duplicates_tweets_list.append(0)
        else:          
            for t in range(len(data["possible_duplicates"].iloc[i])):
                ...
                if len(comparacion) > 0:
                    num_duplicates_tweets.append(len(comparacion))
                else:
                    num_duplicates_tweets.append(0)
    return num_duplicates_tweets, data
...
def advanced_features(data):
    ...
    for i in range(len(data)):
        ...
        num_duplicates_tweets, data = duplicates_detection(data, i)
        num_duplicates_tweets_list.append(num_duplicates_tweets)
    ...
    data["num_duplicates_list"]= num_duplicates_tweets_list
...
def filter_possible_irregular_profiles(data_join):
    ...
    #Possible influencers profile in the last days with a high iteration
    data_join["evaluate_NLP"][(data_join["trend"]==2) & (data_join["profile_type"]==False) &
