...
def parse_gtfFile(gtfFile):
		...
		return transID2GeneIDDict,geneIDNameDict,proteinCodingGeneDict,transWithStrand,transID_start_coor,transID_stop_coor,geneID_Biotype_Dict
...
def get_longest_transcripts_information(coorFile,transcriptFile,gtfFile,longestTransFile):
	...
	'''
	transID2GeneIDDict,geneIDNameDict,proteinCodingGeneDict,transWithStrand,transID_start_coor,transID_stop_coor,geneID_Biotype_Dict=parse_gtfFile(gtfFile)
	...
	with open(longestTransFile,'w') as fout:
		...
		for ltid in selectLongestTrans:
			...
			fout.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (ltid,transWithStrand[ltid],geneID,geneIDNameDict[geneID],geneID_Biotype_Dict[geneID],transID_start_coor[ltid],transID_stop_coor[ltid],startCodonCoorDict[ltid],stopCodonCoorDict[ltid],cds_length,str(Five_UTR_length),str(Three_UTR_length),str(transLengthDict[ltid])))
