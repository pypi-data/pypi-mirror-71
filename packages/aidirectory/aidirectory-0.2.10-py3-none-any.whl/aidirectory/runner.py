...
def run(
    ...
    for topic in topics:
        ...
        try:
            ...
            pass
        datapath = os.path.join(topicpath, "data")
        try:
            os.mkdir(datapath)
        except OSError:
            pass
        if isotherms:
            isograd = os.path.join(datapath, "isotherm")
            try:
                os.mkdir(isograd)
            except OSError:
                pass
        ...
        if gradients:
            isograd = os.path.join(datapath, "gradient")
            try:
                os.mkdir(isograd)
            except OSError:
                pass
...
def main():
    ...
    try:
        ...
        data_loaded = aiyaml.write_directories()
        ...
        try:
            ...
            log.warning(f"Unable to open the parameter file {e}")
            pass
        log.warning(f"Unable to find the parameter file {e}")
    try:
        run(*utils.decompress(data_loaded))
    except TypeError as e:
        log.warning(f"An errror occured: {e}")
    ...
    try:
        ...
        log.warning(f"Unable to write to the folder. Probably does not exist {e}")
        pass
