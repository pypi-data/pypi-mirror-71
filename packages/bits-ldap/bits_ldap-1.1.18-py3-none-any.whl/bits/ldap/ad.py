...
class AD(LDAPBase):
    ...
    def update_groups(self, auth):
        ...
        def dsp(auth):
            """Update DSP group from Google."""
            groupname = 'dsp-internal'
            users = self.get_google_group_members(groupname, auth)
            self.update_group_members('Data Sciences Platform', users)

        def dsp_analysis_platform(auth):
            """Update DSP Anlaysis Platform from Google."""
            groupname = 'dsp-analysis-platform'
            users = self.get_google_group_members(groupname, auth)
            self.update_group_members(groupname, users)

        def dsp_devops(auth):
            """Update DSP Devops from Google."""
            groupname = 'dsp-devops'
            users = self.get_google_group_members(groupname, auth)
            self.update_group_members(groupname, users)
        ...
        def workbench_dev(auth):
            """Update Workbench Dev from Google."""
            groupname = 'workbench-dev'
            users = self.get_google_group_members(groupname, auth)
            self.update_group_members(groupname, users)
