...
apiDirPath = "api-files"
templateFileName = "BVB_Mapping_Template.xlsx"
...
def createEmptySheet(apiName):
    # Create a workbook and add a worksheet.
    filename = apiName + '.xlsx'
...
def updateSheetWithData(apiName, params):
    filename = apiName + '.xlsx'
...
def generate_api_template_file():
    api_name = input("Please enter API name: ")
    api_info = getAPIFile(api_name)

    if api_info is not None:
        print(api_info)
        params = getParametersList(api_info[1])
        createEmptySheet(api_info[0])
        updateSheetWithData(api_info[0], params)
