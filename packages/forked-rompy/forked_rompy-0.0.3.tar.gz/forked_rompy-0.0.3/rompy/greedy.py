...
class GramSchmidt(_IteratedModifiedGramSchmidt):
    ...
    def make(self, a=0.5, max_iter=3, timerQ=False):
        ...
        if timerQ:
            print(("\nElapsed time =", time.time()-t0), flush=True)
...
class CrossValidation(object):
    ...
    def Kfold(self, training, name, K=10, parallel=True, random=True):
        ...
        if parallel is False:
            for ii in range(K):
                errors[ii], arg_errors[ii] = _Kfold(
                    training, name, K, ii, self._partitions, **self._options)
        else:
            ...
            output = []
            for ii in range(K):
                output.append(executor.submit(_Kfold, training,
                                              name, K, ii, self._partitions, **self._options))
    ...
    def Kfold_ensemble(self, x, y, n, K=10, parallel=True, random=True, verbose=False):
        ...
        for nn in range(n):
            if verbose and not (nn+1) % 10:
                print(("Trial number", nn+1), flush=True)
...
class ReducedBasis(_ReducedBasis, _IteratedModifiedGramSchmidt):
    ...
    def __init__(self, inner=None, loss='L2'):
        ...
        if inner is not None:
            ...
            print("No integration rule given.", flush=True)
    ...
    def seed(self, Nbasis, training_space, seed):
        ...
        assert Nsamples == np.size(
            self.inner.weights), "Number of samples is inconsistent with quadrature rule."
        self._Nbasis = Nbasis
        ...
        self.malloc(self._Nbasis, Npoints, Nsamples,
    ...
    def iter(self, step, errs, training_space):
        ...
        if next_index in self.indices:
            ...
            self.alpha[step +
                       1] = self.alpha_arr(self.basis[step+1], training_space)
    ...
    def make(self, training_space, index_seed, tol, num=None, rel=False, verbose=False, timer=False):
        ...
        while nn < self._Nbasis:
            if verbose:
                if rel:
                    print(
                        (nn+1, "\t", self.errors[nn]/self.errors[0]), flush=True)
                else:
                    print((nn+1, "\t", self.errors[nn]), flush=True)
            ...
            # otherwise, add another point and basis vector
                ...
                if self._loss == 'L2':
                    ...
                    errs = self.loss(
                        self.alpha[:nn+1], self.basis[:nn+1], training_space)
    ...
    def plot_errors(self, axis=None,
        ...
        try:
            ...
            print("Cannot load matplotlib.pyplot module for plotting.", flush=True)
        ...
        if hasattr(self, 'errors'):
            ...
            print("Object `errors` not found.", flush=True)
