    ...
    try:
        ...
        if html_df.shape[0] == json_df.shape[0]:
            ...
            if'CHL' in list(html_df.Event):
                shared.print_error("The number of columns in the Html and Json pbp are different because the"
                                     " Json pbp, for some reason, does not include challenges. Will instead merge on "
                                     "Period, Event, Time, and p1_id.")
            else:
                shared.print_error("The number of columns in the Html and json pbp are different because "
