...
class Collage:
    def __init__(self, img_array, mask_coordinates, svd_radius, verbose_logging=False):
        self.img_array = img_array
        self.mask_coordinates = mask_coordinates
        self.mask_polygon = geometry.Polygon(mask_coordinates)
        self.mask_min_x = self.mask_polygon.bounds[0]
        self.mask_min_y = self.mask_polygon.bounds[1]
        self.mask_max_x = self.mask_polygon.bounds[2]
        self.mask_max_y = self.mask_polygon.bounds[3]
        if verbose_logging:
            print(f'mask_coordinates={mask_coordinates}')
        scaled_coordinates = [[coordinate[0] - self.mask_min_x, coordinate[1] - self.mask_min_y] for coordinate in mask_coordinates]
        if verbose_logging:
            print(f'scaled_coordinates={scaled_coordinates}')
        self.scaled_polygon = geometry.Polygon(scaled_coordinates)
        if verbose_logging:
            print(f'scaled_polygon={self.scaled_polygon}')
    ...
    def from_rectangle(cls, img_array, mask_min_x, mask_min_y, patch_window_width, patch_window_height, svd_radius, verbose_logging=False):
        mask_coordinates = [(mask_min_x, mask_min_y), (mask_min_x + patch_window_width, mask_min_y), (mask_min_x + patch_window_width, mask_min_y + patch_window_height), (mask_min_x, mask_min_y + patch_window_height)]
        return cls(img_array, mask_coordinates, svd_radius, verbose_logging)
    ...
    def get_haralick_mt_feature(self, img, desired_haralick_feature, greylevels, haralick_window_size, symmetric=False, mean=False):
        ...
        for pos in product(range(w), range(h)):
            pnt = geometry.Point(pos[1], pos[0])
            if pnt.within(self.scaled_polygon): 
    ...
    def execute(self):
        ...
        print(f'mask_min_y {mask_min_y} patch_window_height {patch_window_height} mask_min_x {mask_min_x} patch_window_width {patch_window_width}')
