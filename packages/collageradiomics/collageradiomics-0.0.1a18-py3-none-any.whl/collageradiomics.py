...
class Collage:
    def __init__(self, img_array, mask_coordinates, svd_radius, verbose_logging=False):
        ...
        self.scaled_polygon = geometry.Polygon([[coordinate[0] - self.mask_min_x, coordinate[1] - self.mask_min_y] for coordinate in mask_coordinates])
    ...
    def from_rectangle(cls, img_array, mask_min_x, mask_min_y, patch_window_width, patch_window_height, svd_radius, verbose_logging=False):
        mask_coordinates = [(mask_min_x, mask_min_y), (mask_min_x + patch_window_width, mask_min_y + patch_window_height), (mask_min_x + patch_window_width, mask_min_y), (mask_min_x, mask_min_y + patch_window_height)]
    ...
    def get_haralick_mt_feature(self, img, desired_haralick_feature, greylevels, haralick_window_size, symmetric=False, mean=False):
        ...
        for pos in product(range(w), range(h)):
            if geometry.Point(pos[0], pos[1]).within(self.scaled_polygon): 
