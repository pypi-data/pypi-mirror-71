...
def lsqlin_cvxopt(A_lsq, b_lsq, A_ineq, b_ineq, A_eq, b_eq):

    P = matrix(A_lsq.T @ A_lsq)
    q = -matrix(A_lsq.T @ b_lsq)

    if A_ineq is not None:
        A_ineq, b_ineq = matrix(A_ineq), matrix(b_ineq)

    if A_eq is not None:
        A_eq, b_eq = matrix(A_eq), matrix(b_eq)

    sol = solvers.qp(P, q, A_ineq, b_ineq, A_eq, b_eq, options={'show_progress': 0})
...
def solve_linear(A, b, npoly, A_ineq, b_ineq, A_eq, b_eq, linear_method):
    ...
    if m == 1:                  # A is a vector, not an array
        ...
        if linear_method == 'lsqlin':
            soluz = lsqlin(A, b, A_ineq, b_ineq, A_eq, b_eq)
        elif linear_method == 'cvxopt':
            soluz = lsqlin_cvxopt(A, b, A_ineq, b_ineq, A_eq, b_eq)
...
class ppxf:
    ...
    def __init__(self, templates, galaxy, noise, velscale, start,
        ...
        assert linear_method in ['nnls', 'lsqlin', 'cvxopt'], \
            "`linear_method` must be 'nnls', 'lsqlin' or 'cvxopt'"

        if linear_method == 'cvxopt':
            assert cvxopt_installed, \
                "To use `linear_method`='cvxopt' the package must be installed"
        ...
        if self.gas_any:
            ...
            if np.any(self.gas_zero_template):
                ...
                if not quiet:
                    print("Some gas templates are identically zero in the fitted "
                          "range, the gas emissions likely fall in a masked "
                          "region or outside the fitted wavelength range:")
        ...
        for j, (st, mo) in enumerate(zip(start1, self.moments)):
            ...
            start1[j] = np.pad(st, [0, mo - len(st)], mode='constant')
...
################################################################################
    ...
    def set_linear_constraints(self, constr_templ, constr_kinem, method):
        ...
        if self.fraction is not None:
            assert self.linear_method != 'nnls', \
                "`fraction` requires `linear_method` == 'lsqlin' or 'cvxopt'"
        ...
        if self.gas_any_zero:
            assert self.linear_method != 'nnls', \
                "identically-zero gas templates require `linear_method` == 'lsqlin' or 'cvxopt'"
        ...
        self.A_ineq_templ = np.pad(-np.identity(pos), [(0, 0), (npoly, 0)], mode='constant')
        self.b_ineq_templ = np.zeros(pos)
        if constr_templ != {}:
            assert self.linear_method != 'nnls', \
                "`constr_templ` require `linear_method` == 'lsqlin' or 'cvxopt'"
            if "A_ineq" in constr_templ:
                A_ineq = np.pad(constr_templ["A_ineq"], [(0, 0), (npoly, self.nsky)], mode='constant')
            ...
            if "A_eq" in constr_templ:
                A_eq = np.pad(constr_templ["A_eq"], [(0, 0), (npoly, self.nsky)], mode='constant')
        ...
        if constr_kinem != {}:
            ...
            if "A_ineq" in constr_kinem:
                self.A_ineq_kinem = np.pad(constr_kinem["A_ineq"], [(0, 0), (0, self.npars - self.ngh)], mode='constant')
                self.b_ineq_kinem = constr_kinem["b_ineq"]
            if "A_eq" in constr_kinem:
                self.A_eq_kinem = np.pad(constr_kinem["A_eq"], [(0, 0), (0, self.npars - self.ngh)], mode='constant')
        ...
        for j in range(5):  # Do at most five cleaning iterations
            ...
            if self.method == 'capfit':
                res = capfit(
                    ...
                    A_eq=self.A_eq_kinem, b_eq=self.b_eq_kinem, cond=1e-7)
                perror = res.x_err
            else:
