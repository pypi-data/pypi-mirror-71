...
def get_equity_trade_days_between(start_date, end_date):
    trade_days = pd.Series(pd.read_csv(EQUITY_TRADE_DAYS_PATH, header=None, dtype={0:str})[0])
    return trade_days.loc[(trade_days >= start_date) & (trade_days <= end_date)]

def get_future_trade_days_between(start_date, end_date):
    trade_days = pd.Series(pd.read_csv(FUTURE_TRADE_DAYS_PATH, header=None, dtype={0:str})[0])
    return trade_days.loc[(trade_days >= start_date) & (trade_days <= end_date)]

def format_secode(x):
    """ Add market suffix SH or SZ """
