...
@LastEditors: Louis
@LastEditTime: 2020-06-15 14:34:00
'''
import os
import logging

from LouisTools.louis_os import make_parent_dir
from LouisTools.louis_consts import TODAY


def single_lvl_logger(log_name="LOG", log_file=None, global_level=logging.INFO, handler_level=logging.INFO):
    """
    Awalys with a StreamHandler, if log_file is not None, a FileHandler will also be added.
    @log_name: the flag shown in the log text.
    ...
    formatter = logging.Formatter(
        "%(asctime)s [%(log_name)s] [%(levelname)s] : %(message)s", "%Y-%m-%d %H:%M:%S"
    )
...
def multi_lvl_logger(log_name="LOG", log_dir="$program_dir/log/%Y%m%d/"):
    """
