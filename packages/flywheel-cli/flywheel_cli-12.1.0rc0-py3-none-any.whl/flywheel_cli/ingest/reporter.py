...
class Reporter:
    ...
    def final_report(self):
        ...
        self.save_reports()
        if self.ingest.status == T.IngestStatus.finished and not self.client.report.errors:
            sys.exit(0)
        else:
            sys.exit(1)
    ...
    def get_scanning_progress_counts(self):
        ...
        finished = self.progress.scans.finished + self.progress.stages.scanning.completed
    ...
    def print_status_header(self, status, timestamp):
        """Print status header"""
        status_name = status.replace("_", " ").capitalize().ljust(self.max_status_length)
    ...
    def compute_eta(self, finished: int, total: int, status: T.IngestStatus) -> typing.Optional[dt.timedelta]:
        """Compute ETA"""
        ...
        if prev and finished == prev.finished and total == prev.total:
            ...
            self.eta_report = T.ReportETA(
                eta=remaining_time, report_time=time.time(), total=total, finished=finished,
            )
    ...
    def get_status_elpased_time(self, status: T.IngestStatus) -> int:
        ...
        for old, new in reversed(list(zip(self.ingest.history, self.ingest.history[1:]))):
