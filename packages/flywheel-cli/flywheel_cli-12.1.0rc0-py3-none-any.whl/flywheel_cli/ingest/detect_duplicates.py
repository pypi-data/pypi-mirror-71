...
def detect_uid_conflicts_in_item(item, uids, insert_errors) -> bool:
    ...
    if study_instance_uid_conflict:
        insert_errors.push(
            T.Error(item_id=item.id, code=errors.DifferentStudyInstanceUID.code).dict(exclude_none=True)
        )

    if series_instance_uid_conflict:
        insert_errors.push(
            T.Error(item_id=item.id, code=errors.DifferentSeriesInstanceUID.code).dict(exclude_none=True)
        )
