...
def test_init_should_set_prefix_to_empty_string_if_url_path_is_empty(mocked_boto3, mocked_urlparse):
...
def test_listdir_should_request_paginate_from_paginator_with_path_with_ending_slash(mocked_boto3_pagination,
    ...
    mock_paginator.paginate.assert_called_with(Bucket='bucket', Prefix='path/foo/', Delimiter='/')
...
def test_listdir_should_not_yield_directories_if_they_do_not_exist_with_prefix(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'CommonPrefixes': []}])
    mocked_urlparse((None, 'bucket', 'path/'))
    walker = S3Walker(fs_url)
...
def test_listdir_should_yield_directories_if_they_exist_without_prefix(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'CommonPrefixes': [
        {'Prefix': 'dir1/'},
        {'Prefix': 'dir2/'}
    ]}])
...
def test_listdir_should_not_yield_directories_if_they_do_not_exist_without_prefix(mocked_boto3_pagination,
                                                                                mocked_urlparse):
    mocked_boto3_pagination([{'CommonPrefixes': []}])
    mocked_urlparse((None, 'bucket', '/'))
    walker = S3Walker(fs_url)
...
def test_listdir_should_yield_files_if_they_exist_with_prefix(mocked_boto3_pagination, mocked_urlparse):
...
def test_listdir_should_not_yield_files_if_they_do_not_exist_with_prefix(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'CommonPrefixes': []}])
    mocked_urlparse((None, 'bucket', 'path/'))
    walker = S3Walker(fs_url)
...
def test_listdir_should_yield_files_if_they_exist_without_prefix(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'Contents': [
...
def test_listdir_should_not_yield_files_if_they_do_not_exist_without_prefix(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'CommonPrefixes': []}])
    mocked_urlparse((None, 'bucket', '/'))
    walker = S3Walker(fs_url)
...
def test_open_should_request_download_file_from_boto3_client_if_file_does_not_exist_for_root(mocked_boto3, mocked_open,
    ...
    mocked_urlparse((None, 'bucket', '/'))
    mocked_os.path.isfile = mock.MagicMock(return_value=False)
    mocked_os.path.join = mock.MagicMock(side_effect=['/tmp/path/dir1/file.txt', 'dir1/file.txt'])
    walker = S3Walker(fs_url)
    walker.tmp_dir_path = '/tmp'
...
def test_open_should_return_result_from_open(mocked_boto3, mocked_open_return, mocked_os, mocked_shutil,
                                             mocked_tempfile, mocked_urlparse, mocker):
...
def test_open_delete_prev_file_w_cache(mocked_boto3, mocked_open, mocked_urlparse, mocked_tempfile, mocker):
    os_remove_mock = mocker.patch('os.remove')
    mocked_urlparse((None, 'bucket', '/path/'))
    walker = S3Walker(fs_url)
    walker.tmp_dir_path = '/tmp'
    ...
    assert mocked_open.mock_calls == [mock.call('/tmp/path/foo', 'rb'), mock.call('/tmp/path/bar', 'rb')]
    os_remove_mock.assert_not_called()


def test_open_delete_prev_file_no_cache(mocked_boto3, mocked_open, mocked_urlparse, mocked_tempfile, mocker):
    os_remove_mock = mocker.patch('os.remove')
    mocked_urlparse((None, 'bucket', '/path/'))
    walker = S3Walker(fs_url)
...
def test_open_delete_prev_file_no_cache_same_file(mocked_boto3, mocked_open, mocked_urlparse, mocked_tempfile, mocker):
    os_remove_mock = mocker.patch('os.remove')
    mocked_urlparse((None, 'bucket', '/path/'))
    walker = S3Walker(fs_url)
    walker.tmp_cache = False
    ...
    assert mocked_open.mock_calls == [mock.call('/tmp/path/foo', 'rb'), mock.call('/tmp/path/foo', 'rb')]
    os_remove_mock.assert_not_called()


def test_list_files_should_create_page_iterator(mocked_boto3_pagination, mocked_urlparse):
    mock_paginator = mocked_boto3_pagination()
    mocked_urlparse((None, 'bucket', 'path/'))
    walker = S3Walker(fs_url)
    ...
    mock_paginator.paginate.assert_called_once_with(Bucket='bucket', Prefix='path/', Delimiter='')
...
def test_list_files_yields_files_w_relpath_w_subdir(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'Contents': [
        {'LastModified': datetime.date(1970, 1, 1), 'Key': 'path/file1.txt', 'Size': 1000},
        {'LastModified': datetime.date(1970, 1, 2), 'Key': 'path/file2.txt', 'Size': 2000}
    ]}])
...
def test_list_files_w_include_file(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'Contents': [
        {'LastModified': datetime.date(1970, 1, 1), 'Key': 'path/file1.txt', 'Size': 1000},
        {'LastModified': datetime.date(1970, 1, 2), 'Key': 'path/file2.py', 'Size': 2000},
        {'LastModified': datetime.date(1970, 1, 3), 'Key': 'path/file3.txt', 'Size': 3000}
    ]}])
    mocked_urlparse((None, 'bucket', '/'))
...
def test_list_files_w_exclude_dir(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'Contents': [
        {'LastModified': datetime.date(1970, 1, 1), 'Key': 'dir10/file1.txt', 'Size': 1000},
        {'LastModified': datetime.date(1970, 1, 2), 'Key': 'dir11/file2.txt', 'Size': 2000},
        {'LastModified': datetime.date(1970, 1, 3), 'Key': 'dir20/file3.txt', 'Size': 3000}
    ]}])
    mocked_urlparse((None, 'bucket', '/'))
...
def test_list_files_w_subdir_and_include_dir(mocked_boto3_pagination, mocked_urlparse):
    ...
    mocked_urlparse((None, 'bucket', '/path'))
    walker = S3Walker(fs_url, filter_dirs=['subdir/dir1', 'subdir/dir2', 'subdir2/dir1'])
...
def test_get_filtering_prefixes():
    ...
    assert get_prefixes('', include=['foo/bar', 'foo/baz']) == ['path/foo/bar', 'path/foo/baz']
    assert get_prefixes('foo/', include=['foo/bar', 'foo/baz']) == ['path/foo/bar', 'path/foo/baz']
    assert get_prefixes('foo/bar/', include=['foo/bar/baz', 'foo/baz']) == ['path/foo/bar/baz']
    assert get_prefixes('foo/bar/baz/', include=['foo/bar', 'foo/baz']) == ['path/foo/bar/baz/']


def test_include_dir():
    def should_include(dirpath, ignore_dot_files=True, include=None, exclude=None):
        walker = S3Walker(fs_url, ignore_dot_files=ignore_dot_files, filter_dirs=include, exclude_dirs=exclude)
