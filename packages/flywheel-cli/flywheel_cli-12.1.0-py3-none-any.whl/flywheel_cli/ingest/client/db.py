...
class DBClient(Client):  # pylint: disable=R0904
    ...
    def audit_logs(self) -> typing.Iterable[str]:
        ...
        tasks_subq = (
            ...
            .filter(M.Task.ingest_id == self.ingest_id, M.Task.type == T.TaskType.upload)
            .subquery()
        )
        query = (
            sqla.orm.Query([
            ...
            .outerjoin(
                M.Item.container,
                M.Item.errors  # pylint: disable=E1101
            )
            .outerjoin(
                tasks_subq,
        ...
        first = True
        for item in self._iter_query(query, [M.Item.dir, M.Error.code.label("error_code")], T.AuditLogOut):
            if first:
                first = False
                header = ",".join(fields)
                yield f"{header}\n"
            ...
            values = dict(
                ...
                status="skipped" if item.skipped or not item.status else item.status.name,
        ...
        for filepath_error in self._iter_query(filepath_errors, [M.Error.created], T.Error):
            if first:
                first = False
                header = ",".join(fields)
                yield f"{header}\n"
    ...
    def get_items_with_error_count(self) -> typing.Iterable[T.ItemWithErrorCount]:
        ...
        query = (
            sqla.orm.Query([
                ...
                sqla.sql.func.max(
                    M.Container.error.cast(sqla.Integer)
                ).cast(sqla.Boolean).label("container_error"),
    ...
    def bulk(self, operation: str, model_name: str, mappings: typing.List[dict]) -> None:
    ...
    def one_session_container_multiple_study_instance_uid_item_ids(self) -> typing.Set[uuid.UUID]:
    ...
    def one_study_instance_uid_multiple_session_container_item_ids(self) -> typing.Set[uuid.UUID]:
    ...
    def one_acquisition_container_multiple_series_instance_uid_item_ids(self) -> typing.Set[uuid.UUID]:
    ...
    def one_series_instance_uid_multiple_acquisition_container_item_ids(self) -> typing.Set[uuid.UUID]:
    ...
    def find_all_items_with_uid(self, uid_condition: typing.Any) -> typing.Iterable[T.Schema]:
    ...
    def duplicated_sop_instance_uid_item_ids(self) -> typing.Set[uuid.UUID]:
        ...
        return self.call_db(db_transactions.duplicated_sop_instance_uid_item_ids, self.ingest_id)

    def find_all_containers_with_item_id(self, item_ids: typing.List) -> typing.Iterable[T.Schema]:
