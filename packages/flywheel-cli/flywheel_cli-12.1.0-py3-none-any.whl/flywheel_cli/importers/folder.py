...
class FolderImporter(AbstractImporter):
    ...
    def __init__(self, group=None, project=None, repackage_archives=False,
                 ...
                 group_override=None, project_override=None):
    ...
    def visit_dir(self, walker, queue, target):  # pylint: disable=too-many-branches
            ...
            for dirname in dirs:
                ...
                if 'packfile' in context:
                    ...
                    if target.template_node in (None, TERMINAL_NODE):
                        ...
                        next_node = target.template_node.extract_metadata(dirname.name, child_context, walker, path=child_path)
                        # if group and/or project cli argument was set replace the hierarchy's value
                        if self.group_override:
                            util.set_nested_attr(child_context, 'group._id', self.group_override)

                        if self.project_override:
                            util.set_nested_attr(child_context, 'project.label', self.project_override)
        ...
        if container:
            ...
            self.messages.append(('warn', f'Ignoring files for folder {target.path} because it represents an ambiguous node'))
