...
class DicomScanner(AbstractScanner):
    ...
    def resolve_session(self, context, dcm):
        ...
        subject_code = self.determine_subject_code(context, dcm)
    ...
    def determine_subject_code(self, context, _dcm):  # pylint: disable=no-self-use
        ...
        if subject_label:
            ...
            subject_code = self.subject_map.get_code(_dcm)
        else:
            subject_code = self.get_value(_dcm, 'PatientID', '')

        if not subject_code:
            log.error('Unable to determine valid subject code,'
    ...
    def determine_session_label(self, context, _dcm, uid, timestamp=None):  # pylint: disable=no-self-use
        ...
        if not uid:
            log.error('Unable to determine valid session label,'
                      ' please manually set subject code with --session flag')
            raise InvalidLabel('session')
