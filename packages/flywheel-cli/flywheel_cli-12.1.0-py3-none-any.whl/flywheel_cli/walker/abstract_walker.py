...
class AbstractWalker(ABC):
    ...
    def __init__(self, root, ignore_dot_files=True, follow_symlinks=False, filter=None, exclude=None,  # pylint: disable=redefined-builtin
    ...
    def __repr__(self):
        ...
        return (f'{type(self).__name__}(root={self.root}, ignore_dot_files={self._ignore_dot_files}'
    ...
    def _should_include_file(self, info):
        ...
        if self._exclude_files is not None and self.match(self._exclude_files, filename):
            return False

        if self._include_files is not None and not self.match(self._include_files, filename):
            return False
