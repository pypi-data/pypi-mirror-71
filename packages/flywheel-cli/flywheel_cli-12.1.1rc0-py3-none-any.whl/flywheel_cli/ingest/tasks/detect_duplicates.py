...
class DetectDuplicatesTask(Task):
    ...
    def _run(self):
        ...
        for containerid in self.db.find_all_containers_with_item_id(
            self.error_item_ids
        ):
    ...
    def _check_new_session_container_study_instance_uids(self):
        ...
        result = self.fw.check_uids_exist(ContainerUidcheck(sessions=list(uids)))
        item_ids = set()
    ...
    def _check_new_acquisition_container_study_instance_uids(self):
        ...
        result = self.fw.check_uids_exist(ContainerUidcheck(acquisitions=list(uids)))
        item_ids = set()
        for item in self.db.find_all_items_with_uid(
