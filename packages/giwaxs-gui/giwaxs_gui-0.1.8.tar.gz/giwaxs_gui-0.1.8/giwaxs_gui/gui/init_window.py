...
class InitWindow(QWidget):
    ...
    def __init__(self, recent_projects: List[Path] = None,
                 check_version: CheckVersionMessage = CheckVersionMessage.not_checked):
        flags = Qt.WindowFlags()
        flags |= Qt.FramelessWindowHint
        super().__init__(flags=flags)
        self.check_version: CheckVersionMessage = check_version
    ...
    def _get_title(self) -> str:
        self.log.info(f'Check version message: {self.check_version}')
        if self.check_version == CheckVersionMessage.latest_version_installed:
            return f'GIWAXS analysis (latest version installed: {__version__})'
        if self.check_version == CheckVersionMessage.failed_updating:
            return f'GIWAXS analysis (version {__version__}). New version available!'

        return f'GIWAXS analysis (version {__version__})'
