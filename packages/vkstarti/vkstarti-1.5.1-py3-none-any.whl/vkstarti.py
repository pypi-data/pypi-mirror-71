import threading, vk_api, re, json
...
class Keyboard:
	def create( name, one_time, inline ):
		keyboards [ name ] = {
					"one_time" : one_time,
					"inline" : inline,
					"buttons" : []
	...
	def add_button( label = "standart", color = "positive" , row = 0 , name = None ):
		button =	{
							"action" : {
											"type" : "text",
											"payload" : "{\"10\":\"10\"}",
		...
		keyboards[ name ][ "buttons" ][ row ].append( button )
		
	def add_row( name ):
		keyboards[ name ][ "buttons" ].append( [] )
		
	def delete_row( name, row ):
		keyboards[ name ][ "buttons" ].pop( row )
		
	def delete_button( name, row, button ):
		keyboards[ name ][ "buttons" ][ row ].pop( button )
...
class Bot:
	...
	#messages
	def sms(self,message,id="none", keyboard = "none"):
		if id=="none":
			id=self.event.peer_id
		if keyboard == "none":
			self.vk.messages.send( peer_id = id, random_id = get_random_id(), message = message )
		if keyboard != "none":
			self.vk.messages.send(peer_id=id,random_id=get_random_id(),message=message, keyboard = json.dumps( keyboards[ keyboard ], ensure_ascii = False ))

	def friendsadd(self,peer_id,text):
		self.vk.friends.add(peer_id=peer_id,text=text)

	def stick(self,stick_id,id="none"):
		if id == "none":
			id = self.event.peer_id
		self.vk.messages.send(peer_id=id,random_id=get_random_id(),sticker_id=stick_id)
