...
async def service_install(integrations: Integrations = Depends(get_integrations), data: dict = Body({})):
    ...
    :return:
    """
    if State.bootstrapper.install(integrations, data):
        return http_200_ok({"message": "Installation complete"})
...
async def service_seed(integrations: Integrations = Depends(get_integrations), data: dict = Body({})):
    ...
    :return:
    """
    if State.bootstrapper.seed(integrations, data):
        return http_200_ok({"message": "Seeding complete"})
