...
class Spliter:
    def __init__(self, **options):
        if 'language' in options:
            assert (isinstance(options['language'], str))
            if options['language'].lower() == 'zh':
                self.spliter = spliter_simple.Spliter('zh')
                self.flag = 'zh'
            elif options['language'].lower() == 'en':
                self.spliter = spliter_simple.Spliter('en')
                self.flag = 'en'
            else:
        ...
        if 'remove_blank' in options:
            assert (isinstance(options['remove_blank'], bool))
            self.remove_blank_on = options['remove_blank']
        else:
            self.remove_blank_on = True
        if 'long_short_sent_handle' in options:
            assert (isinstance(options['long_short_sent_handle'],bool))
    ...
    def cut_to_sentences(self, text):
        text = re.sub(r'\s+','',text) #先不考虑中英文混合情况
        if self.remove_blank_on:
    ...
    def refine(self, all_sentence: List[str]) -> [str]:
        ...
        while i < length:
            ...
            if self.is_valid_sntnc(sent):
                ...
                out.extend(self.refine_single(sent))
                end_sntnc = ''
