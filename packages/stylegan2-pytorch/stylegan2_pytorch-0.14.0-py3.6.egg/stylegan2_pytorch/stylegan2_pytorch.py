    ...
    def __init__(self, image_size, latent_dim, network_capacity = 16, transparent = False, attn_layers = [], no_const = False):
        ...
        for ind, (in_chan, out_chan) in enumerate(in_out_pairs):
            ...
            attn_fn = nn.Sequential(*[
                Residual(Rezero(nn.Sequential(ImageLinearAttention(in_chan), leaky_relu()))) for _ in range(2)
...
class Discriminator(nn.Module):
    def __init__(self, image_size, network_capacity = 16, fq_layers = [], fq_dict_size = 256, attn_layers = [], transparent = False):
        ...
        for ind, (in_chan, out_chan) in enumerate(chan_in_out):
            ...
            attn_fn = nn.Sequential(*[
                Residual(Rezero(nn.Sequential(ImageLinearAttention(out_chan), leaky_relu()))) for _ in range(2)
            ]) if num_layer in attn_layers else None
...
class StyleGAN2(nn.Module):
    def __init__(self, image_size, latent_dim = 512, style_depth = 8, network_capacity = 16, transparent = False, fp16 = False, cl_reg = False, steps = 1, lr = 1e-4, fq_layers = [], fq_dict_size = 256, attn_layers = [], no_const = False):
        ...
        self.D_cl = BYOL(self.D, image_size, hidden_layer='flatten') if cl_reg else None
...
class Trainer():
    ...
    def train(self):
        ...
        if self.GAN.D_cl is not None:
            ...
            cr_loss = torch.tensor(0.).cuda().requires_grad_()
            ...
            if apply_cl_reg_to_generated:
                for i in range(self.gradient_accumulate_every):
                    ...
                    cr_loss = cr_loss + self.GAN.D_cl(generated_images.clone().detach())

            for i in range(self.gradient_accumulate_every):
                image_batch = next(self.loader).cuda()
                loss = self.GAN.D_cl(image_batch)
                cr_loss = cr_loss + self.GAN.D_cl(image_batch)

            self.last_cr_loss = cr_loss.clone().detach().item()
            backwards(cr_loss, self.GAN.D_opt)

            self.GAN.D_opt.step()
            self.GAN.D_cl.update_moving_average()
