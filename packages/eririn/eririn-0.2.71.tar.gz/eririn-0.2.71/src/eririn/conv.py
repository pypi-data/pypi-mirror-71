...
def quantize(rgb=None, rgbmap=None):
    ...
    import numpy as np
    sorted_colorlist = sorted(list(rgbmap.keys()))
    colors = len(sorted_colorlist)
    X = np.array([list(t) for t in sorted_colorlist])
    clusters = 65535
    kmeans = KMeans(n_clusters=clusters, random_state=0).fit(X)
    result = kmeans.labels_
    remapped = {k: sorted_colorlist[result[k]] for k in range(colors)}
