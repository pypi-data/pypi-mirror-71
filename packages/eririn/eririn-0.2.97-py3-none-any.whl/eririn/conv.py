...
def recreate_image(rgb=None, w=0, h=0, n_colors=65535):
    ...
    kmeans = KMeans(n_clusters=n_colors, verbose=2, random_state=0, init='k-means++',
                    algorithm='full', n_init=1, max_iter=30).fit(image_array_sample)
    ...
    for i in range(h):
        for j in range(w):
            image[j][i] = codebook[labels[label_idx]]
            label_idx += 1

    print("before reshape: %s" % str(image.shape))
    # ret = np.reshape(image, (w * h, d))
...
def main(arguments):
    ...
    for fidx, f in enumerate(inputs):
        ...
        with Image.open(f) as im:
            ...
            if num_c > 65535:
                data = recreate_image(rgb, w=resized[0], h=resized[1])
                print(data.shape)
                im2 = Image.fromarray(np.uint8(data))
                print("generated im2.")
