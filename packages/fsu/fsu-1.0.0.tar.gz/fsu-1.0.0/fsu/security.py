...
def sign_required(get_secret_by_uaid, ttl):
    async def middleware(request : Request, call_next):
        ...
        to_be_signed = f"{data}.{x_ts}"

        sign = hmac.digest(secret.encode(), to_be_signed.encode(), "sha256").hex()
...
def jwt_token_required(jwt_secret):
    def get_token_info(authorization : str = Header(...)):
        token = None
        info  = None
...
def auth_required(jwt_secret):
    async def validate_admin_id(info = Depends(jwt_token_required(jwt_secret))):
        if "admin_id" not in info:
            raise UniversalLogicError(E.INVALID_TOKEN)

    return adimap(validate_admin_id)
