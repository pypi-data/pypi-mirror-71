...
from rse.main.executor import get_named_executor
...
class RelationalDatabase(Database):
    """A RelationalDatabase is a more robust relational datbase (to sqlite).
       ...
       a custom init function to handle the $HOME/.rse/rse.db file.
    """
    ...
    def __init__(self, config_dir, config=None, **kwargs):
        ...
        if not database_string:
            sys.exit(
                "A database url must be defined to use a relational database. Set with rse config --database"
            )
    ...
    def clear(self):
        ...
        """
        from rse.main.database.models import Task
