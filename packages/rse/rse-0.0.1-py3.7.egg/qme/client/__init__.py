...
def get_parser():
    ...
    parser.add_argument(
        "--config_dir",
        dest="config_dir",
        help="select database and configuration directory (defaults to $HOME/.rse).",
    )
    ...
    # Configure rse (not written yet, will be written when databases added)
    config = subparsers.add_parser("config", help="configure rse.")
    ...
    config.add_argument(
        ...
        dest="database",
        default=None,
        help="select database backend for rse. [filesystem|sqlite] or [sqlite|mysql|postgresql]:///",
    )
    ...
    generate = subparsers.add_parser(
        "generate-key",
        help="generate a key for rse start, should be exported to QME_SERVER_KEY.",
    )
    ...
    start.add_argument(
        ...
        default=5000,
        type=int,
        help="select port to run rse dashboard on (defaults to 5000)",
    )
    start.add_argument(
        "--host",
        dest="host",
