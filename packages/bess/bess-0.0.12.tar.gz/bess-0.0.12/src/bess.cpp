...
List bessCpp(Eigen::MatrixXd x, Eigen::VectorXd y, int data_type, Eigen::VectorXd weight,
    ...
    if (is_screening) {
        screening_A = screening(x, y, weight, model_type);
    }
...
//        result = pgs_path(data, algorithm, metric, s_min, s_max, log_lambda_min, log_lambda_max);
    }
...
//    std::cout<<"pywrap"<<endl;
    List mylist = bessCpp(x_Mat, y_Vec, data_type, weight_Vec, is_normal,
                          ...
                          lambda_min, lambda_max, is_screening);
