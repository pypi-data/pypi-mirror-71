...
class ImageHat():
    ...
    __HOST2_IN = 0x80 # GPIO7 IN  Host 2 power supply state
    ...
    # HOST2 IN
    def __get_host2_in(self):
        return int(self.__smbus.read_byte_data(self.__address, self.__GPIO) & self.__HOST2_IN == self.__HOST2_IN)

    host2_in = property(__get_host2_in)


    # EXT IN
    def __get_ext_in(self):
        return int(self.__smbus.read_byte_data(self.__address, self.__GPIO) & self.__EXT_IN == self.__EXT_IN)
    ...
    def __get_ext_out(self):
        return int(self.__smbus.read_byte_data(self.__address, self.__GPIO) & self.__EXT_OUT == self.__EXT_OUT)

    def __set_ext_out(self, value):
        current_state = self.__smbus.read_byte_data(self.__address, self.__GPIO)
        if value == False:
            new_state = current_state & ~self.__EXT_OUT
        else:
            new_state = current_state | self.__EXT_OUT
    ...
    def __set_host(self, value):
        ...
        if current_state != new_state:
            ...
            time.sleep(0.5 * self.__TIME_FACTOR)
        ...
        if current_state != new_state:
            self.__smbus.write_byte_data(self.__address, self.__GPIO, new_state)
            current_state = new_state
            time.sleep(1.0 * self.__TIME_FACTOR)
