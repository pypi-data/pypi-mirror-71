...
def correlation(df: DataFrame) -> DataFrame:
    ...
    | Examples
    |--------
    | >>> df = pd.DataFrame({'Soldiers Casualties': np.random.randint(10,15, size=(1092,)), 'Unknown Fighters': np.random.randint(10,12, size=(1092,)) , 'Civilian Casualties': np.random.randint(10,15, size=(1092,)), 'Local Miltia': np.random.randint(10,15, size=(1092,)) ,'Air Strikes': np.random.randint(10,15, size=(1092,))}, pd.date_range(start="2015-01-04", end="2017-12-30", freq='1D'))
    | >>> correlation(df=df)
    | >>>
    """
    if df.empty:
...
def covariance(df: DataFrame) -> float:
    """
