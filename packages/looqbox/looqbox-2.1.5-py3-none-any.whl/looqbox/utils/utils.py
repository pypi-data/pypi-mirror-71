...
def title_with_date(header=None, date_int=None, language=GlobalCalling.looq.language):
    ...
    if (isinstance(date_int[0], datetime.datetime) and isinstance(date_int[1], datetime.datetime)) or \
            ("00:00:00" in date_int[0] and "00:00:00" in date_int[1]):
        date_int_aux[0] = datetime.datetime.strptime(date_int[0], '%Y-%m-%d %H:%M:%S').date()
        date_int_aux[1] = datetime.datetime.strptime(date_int[1], '%Y-%m-%d %H:%M:%S').date() - datetime.timedelta(days=1)
    else:
