...
import locale
from locale import delocalize, atof
...
def european_to_us_number(numberstr):
...
def get_first_elem(group):
    assert group.notnull().sum() == 1
    return group.iloc[0]
...
def charge_releve_SoG(fichier_releve, encoding='iso8859-1'):
    ...
    with codecs.open(fichier_releve, 'r', encoding=encoding) as fr:
        ...
        releve_info = {'agence'   : infos[0][0].rstrip(),
                       ...
                       infos[4][0]: float(european_to_us_number(infos[4][1])),
                       'Devise': infos[4][2]}
    ...
    releve['Débit'] = releve['Débit'].astype(str)\
                        .apply(european_to_us_number).astype(float)
...
class ChargeurDonnees(object):
    ...
    exercice_colonne_fin = 11  # exclue
    ...
    def _charge_exercice_excel(soi):  # dtypes_attendus
        ...
        if soi.feuille_validation in exercice_workbook.sheet_names:
            soi.modes = pd.read_excel(exercice_workbook,
                                      ...
                                      skiprows=2, usecols=(1,), nrows=3)
            soi.classes = pd.read_excel(exercice_workbook,
                                        sheet_name=soi.feuille_validation,
                                        skiprows=6, usecols=(1,),
                                        squeeze=True).dropna()
        else:
    ...
    def _charge_exercice_gsheets(soi):
        ...
        worksheet = exercice_workbook[soi.feuille]
        ...
        soi.exercice = pd.DataFrame.from_records(
                            lignes[soi.exercice_ligne_debut+1:]\
                                [soi.exercice_colonne_debut:],
                            columns=lignes[soi.exercice_ligne_debut]\
                                      [soi.exercice_colonne_debut])

        soi.precedents_soldes = pd.DataFrame.from_records(
                            lignes[soi.exercice_ligne_debut-2:]\
                                [soi.exercice_colonne_debut+7:],
                            columns=['date', 'montant'],
                            nrows=2)
        
        if soi.feuille_validation in exercice_workbook.sheet_names:
            lignes_val = exercice_workbook[soi.feuille_validation]
            soi.modes = pd.DataFrame.from_records(lignes_val, nrows=3)
                                      #skiprows=2, usecols=(1,),
            soi.classes = pd.DataFrame.from_records(lignes_val).dropna()
                                        #skiprows=6, usecols=(1,),
                                        #squeeze=True
        else:
            loggeur.warning("La feuille de validation %r est absente du "
    ...
    def __init__(soi, fichier_exercice, feuille=0, source="excel",
        ...
        ### TODO: sanity check of exercice (no unnamed colonnes, valid data types)
        ...
        try:
            assert soi.exercice['Crédit'].dtype == float, soi.exercice['Crédit'].dtype
            assert soi.exercice['Débit'].dtype == float, soi.exercice['Débit'].dtype
            assert soi.exercice['Solde'].dtype == float, soi.exercice['Solde'].dtype
        except AssertionError:
            for colonne in ('Crédit', 'Débit', 'Solde'):
                soi.exercice[colonne].astype(float)
    ...
    def reecrit_excel(soi, nouveau_suffixe='-maj', suffixe_date=False):
        ...
        soi.nouvel_exercice.to_excel(excelwriter, soi.feuille_sortie,
    ...
    def reecrit_gsheets(soi, nouveau_suffixe='-maj', suffixe_date=False):
        ...
        gsheet = soi.gc.create(fichier_sortie)  # TODO: Que se passe-t-il si le fichier existe ?
        ...
        # 'B5:K5'
        colonnes = (lettres[soi.exercice_colonne_debut] + '{0:d}:' +
                    lettres[soi.exercice_colonne_debut + soi.nouvel_exercice.shape[1]] + '{0:d}')

        # Écrit les noms de colonnes d'abord:
        for cell, contenu in zip(feuille_sortie.range(colonnes.format(soi.exercice_ligne_debut+1)),
                                 soi.nouvel_exercice.columns):
            cell.value = contenu

        for num_ligne, (_, ligne_sortie) in enumerate(soi.nouvel_exercice.iterrows(),
                                                      start=soi.exercice_ligne_debut+2):
            for cell, contenu in zip(feuille_sortie.range(colonnes.format(num_ligne)),
                                     ligne_sortie):
                cell.value = contenu

        cellules_solde = feuille_sortie.range(
                        lettres[soi.exercice_colonne_debut + 1 + 7]
                        + str(soi.exercice_ligne_debut - 1)
                        + ':'
                        + lettres[soi.exercice_colonne_debut-1]
                        + str(soi.exercice_ligne_debut))

        for cell, contenu in zip(cellules_solde, soi.nouveaux_soldes.values.flat):
            cell.value = contenu

        plage = (colonnes[:-5]+'{1:d}').format(soi.exercice_ligne_debut-1,
                                               num_ligne+1)

        feuille_sortie.update_cells(plage)
        feuille_sortie.close()
