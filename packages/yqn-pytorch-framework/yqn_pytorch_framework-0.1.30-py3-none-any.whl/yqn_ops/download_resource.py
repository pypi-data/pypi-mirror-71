...
def get_args_parser():
    ...
    parser.add_argument('-env', type=str, required=True, default='local', help='local/online')
    parser.add_argument('-file', type=str, default='', help='custom file path')
    parser.add_argument('-force', type=bool, default=False, help='remove exist files')
    return parser
...
def download_dir(logger, file_url, dist_dir, oss_crc, remove_force):
    ...
    if remove_force and os.path.exists(cached_file):
...
def parse_command(logger, parser_fn=get_args_parser, printed=True):
    ...
    if len(args.file) > 0 and os.path.exists(args.file):
    ...
    with open(config_file_path) as file:
        ...
        for document in documents:
            if not document['local_path'].endswith(".trt"):
                download_dir(logger, document['file_url'], document['local_path'], document['crc'])
            else:
