...
class SelectEdit(QObject, vmi.Menu, vmi.Mouse):
    ...
    def apply(self):
        if self._Bind is not None:
            ...
            look = self.view.camera('look')
            lthn = vmi.imLength(self._Bind.data(), self.view.camera('look'))
            region = vmi.mkPrism(face, [-look[i] for i in range(3)], lthn)
            region = vmi.mkBoolean_Union(vmi.mkPrism(face, look, lthn), region)
            region = vmi.mkPolyData_Shape(region)

            stencil = vmi.imStencil(self._Bind.data(), region)
            ...
            def func():
                ...
                while not it.IsAtEnd():
                    if it.IsInStencil():
                        vmi.imValue(self._Bind.data(), it.GetIndex(), target)
                    it.Next()
                    count += 1
                    p[0] = 100 * count / sum
    ...
    def rebuild(self):
        for i in range(len(self.nodes)):
            self.nodeShapes[i].setData(vmi.mkVertex(vmi.mkPnt(self.nodes[i])))
        if len(self.nodes) > 2:
            if self.curveType == 0:
                self.curveShape.setData(vmi.mkWire(vmi.mkSegments(pts=self.nodes, closed=True)))
            elif self.curveType == 1:
                self.curveShape.setData(vmi.mkWire(vmi.mkBSplineKochanek(pts=self.nodes, closed=True)))
        elif len(self.nodes) > 1:
            if self.curveType == 0:
                self.curveShape.setData(vmi.mkWire(vmi.mkSegments(pts=self.nodes, closed=False)))
            elif self.curveType == 1:
                self.curveShape.setData(vmi.mkWire(vmi.mkBSplineKochanek(pts=self.nodes, closed=False)))
        else:
...
class RegionResample(QObject, vmi.Menu, vmi.Mouse):
    ...
    def apply(self):
        if self._Bind is not None:
            spc = vmi.askFloats([0.1, 0.1, 0.1], vmi.imSpacing(self._Bind.data()), [5, 5, 5], 3,
            ...
            for k in range(3):
                for j in range(2):
                    for i in range(2):
                        ...
                        self.regionShapes[k][j][i].setData(vmi.mkEdge(vmi.mkSegment(pt[0], pt[1])))
        else:
            for k in range(3):
                for j in range(2):
                    for i in range(2):
