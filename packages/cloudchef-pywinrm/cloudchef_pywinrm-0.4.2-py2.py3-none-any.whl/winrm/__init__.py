...
class Session(object):
    ...
    def run_cmd_with_stream(self, command, args=()):
        """Return a stream-like object which can yield (status_code, stdout, stderr) tuple"""
        shell_id = self.protocol.open_shell()
        command_id = self.protocol.run_command(shell_id, command, args)
        for status_code, stdout, stderr in self.protocol.get_command_output_with_stream(shell_id, command_id):
            yield (status_code, stdout, stderr)
    ...
    def run_ps_with_stream(self, script):
        """base64 encodes a Powershell script and executes the powershell
        encoded script command
        """
        # must use utf16 little endian on windows
        encoded_ps = b64encode(script.encode('utf_16_le')).decode('ascii')
        stream = self.run_cmd_with_stream('powershell -encodedcommand {0}'.format(encoded_ps))
        for status_code, stdout, stderr in stream:
            if len(stderr):
                # if there was an error message, clean it it up and make it human
                # readable
                stderr = self._clean_error_msg(stderr)
            yield status_code, stdout, stderr
