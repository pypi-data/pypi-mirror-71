...
class Protocol(object):
    ...
    def get_command_output_with_stream(self, shell_id, command_id):
        command_done = False
        while not command_done:
            try:
                stdout, stderr, return_code, command_done = \
                    self._raw_get_command_output(shell_id, command_id, combine_stdout_and_stderr=True)
                yield return_code, stdout, stderr
            except WinRMOperationTimeoutError:
                # this is an expected error when waiting for a long-running process, just silently retry
                pass
    

    def _raw_get_command_output(self, shell_id, command_id, combine_stdout_and_stderr=False):
        req = {'env:Envelope': self._get_soap_header(
            resource_uri='http://schemas.microsoft.com/wbem/wsman/1/windows/shell/cmd',  # NOQA
