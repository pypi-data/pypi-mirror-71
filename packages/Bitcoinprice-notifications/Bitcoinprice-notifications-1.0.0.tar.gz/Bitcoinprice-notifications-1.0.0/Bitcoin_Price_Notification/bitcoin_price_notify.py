...
def send_email(bitcoin_history):
  ...
  message = "Dear " + recipent_name + "\nBitcoin prices are now " + str(bitcoin_history) + ". Better buy quick.\nRegards,\n" + "\n Tejas"
    ...
    try:
        while True:
            ...
            if new_price < threshold:
                post_ifttt_emergency_telegram('bitcoin_emergency_price', new_price)
        ...
        time.sleep(5)
        print('Application is close within 5 seconds, Thank You for using our Application')
...
def twitter_notifier(bitcoin_threshold, time_gap):
    ...
    while True:
        try:
            ...
            # Send an emergency notification
            if new_price < threshold:
                post_ifttt_emergency_twitter('bitcoin_emergency_price', new_price)
...
def ifttt_app_notifier(bitcoin_threshold, time_gap):
    ...
    while True:
        try:
            ...
            # Send an emergency notification
            if new_price < threshold:
                post_ifttt_emergency_notifications('bitcoin_emergency_price', new_price)

            # Send a Telegram notification
            # Once we have 5 items in our bitcoin_history send an update
            if len(bitcoin_history) == 1:
                post_ifttt_app_notifications('bitcoin_price',
                    format_bitcoin_history(bitcoin_history))
                # Reset the history
                bitcoin_history = []
...
def send_email_notification(bitcoin_threshold, time_gap):
    ...
    try: 
        while True:
            ...
            # Send an emergency notification
            if new_price < threshold:
                post_email_emergency_notifications('bitcoin_emergency_price', new_price)

            # Send a Telegram notification
            # Once we have 5 items in our bitcoin_history send an update
            if len(bitcoin_history) == 1:
                post_email_notifications('bitcoin_price',
...
def main():
    ...
    parser.add_argument("-i", "--interval", type=int, nargs=1, metavar="interval", default=[1], help="Time interval in minutes")
    # command line variable for threshold
    parser.add_argument("-t", "--threshold", type=int, nargs=1, metavar="threshold", default=[9000], help="Threshold in USD")
    # command line variable for destination
    parser.add_argument("-d", "--destination", metavar='destnation', default='telegram', help='We are providing various options to recive notifications from us (1)IFTTT app (2) Telegram app (3) Email (4) Twitter')
    new_args = parser.parse_args()
    print('Running Application with time interval of ',  new_args.interval[0], ' and threshold = $',  new_args.threshold[0], 'and Destination = ', new_args.destination)
    # calls the function to send notifications
    if(new_args.destination == 'telegram'):
        print(''''To recive the notification 
