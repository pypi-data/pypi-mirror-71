...
def process_rebalance(current_portfolio: PortfolioHandler,
    ...
    current_requirements.reset_reports()
    ...
    while True:
        for episode in simulation_creator.episodes:
            ...
            while True:
                ...
                current_requirements.report(active)
                if current_requirements.is_valid:
                    current_requirements.reset_reports()
                    current_portfolio.time.step()

                    logger.critical(maya.MayaDT(current_portfolio.time.head))
