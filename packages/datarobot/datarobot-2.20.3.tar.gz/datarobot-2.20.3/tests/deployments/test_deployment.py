...
class TestDeploymentList(object):
    ...
    @pytest.mark.parametrize(
        'filter_name,filter_value,query_param_key,query_param_value', [
            ...
            (
                    'model_health',
                    [DEPLOYMENT_MODEL_HEALTH_STATUS.FAILING],
                    'modelHealth',
                    'failing'
            ),
            (
                    'model_health',
                    [
                        DEPLOYMENT_MODEL_HEALTH_STATUS.PASSING,
                        DEPLOYMENT_MODEL_HEALTH_STATUS.UNKNOWN
                    ],
                    'modelHealth',
                    'passing,unknown'
            ),
            ...
            (
                    'materiality',
                    [
                        DEPLOYMENT_MATERIALITY.HIGH,
                        DEPLOYMENT_MATERIALITY.LOW
                    ],
                    'materiality',
                    'HIGH,LOW'
            ),
    ...
    def test_filters(
            self, filter_name, filter_value, query_param_key, query_param_value
    ):
        ...
        assert actual_call.endswith(expected_call), (
    ...
    def test_all_query_params(self):
        ...
        for expected_arg in expected_args:
            assert expected_arg in query_params, (
...
class TestDeploymentListFilters(object):
    ...
    @pytest.mark.parametrize('filter_name,valid_value', [
        ('role', 'OWNER'),
        ('role', 'USER'),
        ('service_health', DEPLOYMENT_SERVICE_HEALTH_STATUS.PASSING),
        ...
        ('materiality', DEPLOYMENT_MATERIALITY.MODERATE),
        ('materiality', DEPLOYMENT_MATERIALITY.ALL),
    ])
    def test_valid_filter_values(self, filter_name, valid_value):
        kwargs = {filter_name: valid_value}
        DeploymentListFilters(**kwargs)

    @pytest.mark.parametrize('filter_name,invalid_value', [
        ('role', 1234),
        ('service_health', 1234),
        ('model_health', 1234),
