...
def deployment_settings_response_data():
    return {
        ...
        'predictionWarning': {'enabled': True,
                              'customBoundaries': {
                                  'upper': 1337,
                                  'lower': 0
                              }},
...
@pytest.fixture()
def deployment_settings_get_response(unittest_endpoint, deployment_data,
                                     deployment_settings_response_data):
...
@responses.activate
@pytest.mark.usefixtures('deployment_get_response', 'deployment_settings_get_response')
def test_get_association_id_settings(unittest_endpoint, deployment_data,
                                     deployment_settings_response_data):
    deployment_id = deployment_data['id']
    deployment = Deployment.get(deployment_id)
    ...
    expected_settings = {
        ...
        'required_in_prediction_requests': response[
            'association_id']['required_in_prediction_requests']
    }
    assert settings == expected_settings
...
def test_update_association_id_settings(unittest_endpoint, deployment_data,
                                        param, expected_request):
    deployment_id = deployment_data['id']
    deployment = Deployment.get(deployment_id)
...
@responses.activate
@pytest.mark.usefixtures('deployment_get_response')
def test_get_prediction_warning_settings(unittest_endpoint, deployment_data,
                                         deployment_settings_response_data):
    deployment_id = deployment_data['id']
    url = '{}/deployments/{}/settings/'.format(unittest_endpoint, deployment_id)
    responses.add(
...
def test_update_prediction_warning_settings(unittest_endpoint, deployment_data,
                                            param, expected_request):
    deployment_id = deployment_data['id']
    update_setting_url = '{}/deployments/{}/settings/'.format(unittest_endpoint, deployment_id)
    deployment_url = '{}/deployments/{}/'.format(unittest_endpoint, deployment_id)
...
@responses.activate
@pytest.mark.usefixtures('deployment_get_response')
def test_get_prediction_intervals_settings(unittest_endpoint, deployment_data,
                                           deployment_settings_response_data):
    deployment_id = deployment_data['id']
    url = '{}/deployments/{}/settings/'.format(unittest_endpoint, deployment_id)
    responses.add(
...
def test_update_prediction_intervals_settings(unittest_endpoint, deployment_data):
    ...
    responses.add(
        ...
        content_type='application/json',
        body=json.dumps({
            'status': u'COMPLETED', 'model_id': model_id, 'is_blocked': False,
            'url': '{}/projects/{}/jobs/1/'.format(unittest_endpoint, project_id),
            'job_type': 'calculate_prediction_intervals', 'project_id': project_id, 'id': 1
        }),
