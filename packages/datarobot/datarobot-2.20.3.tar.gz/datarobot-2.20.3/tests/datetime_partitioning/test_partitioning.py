...
from datarobot import (BacktestSpecification, DatetimePartitioning,
                       DatetimePartitioningSpecification, FeatureSettings, partitioning_methods,
                       Project)
...
def test_partition_to_specification(datetime_partition):
    ...
    assert (s_e_part_spec.autopilot_data_selection_method ==
            datetime_partition.autopilot_data_selection_method)
    ...
    assert (s_e_part_spec.default_to_known_in_advance ==
            datetime_partition.default_to_known_in_advance)
    assert (s_e_part_spec.feature_derivation_window_start ==
            datetime_partition.feature_derivation_window_start)
    assert (s_e_part_spec.feature_derivation_window_end ==
            datetime_partition.feature_derivation_window_end)
...
@responses.activate
def test_generate_start_end(project_id, project_url, datetime_partition_server_data,
                            datetime_partition_spec, holdout_start_date, holdout_end_date):
    responses.add(responses.POST, '{}datetimePartitioning/'.format(project_url),
                  json=datetime_partition_server_data)
...
@responses.activate
def test_dt_partitioning_with_unsup(project_id, project_url,
                                    datetime_partition_without_holdout_server_data):
    responses.add(responses.POST, '{}datetimePartitioning/'.format(project_url),
                  json=datetime_partition_without_holdout_server_data)
