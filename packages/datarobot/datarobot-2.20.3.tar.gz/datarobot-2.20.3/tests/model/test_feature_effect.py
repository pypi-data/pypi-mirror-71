...
from datarobot import Model, DatetimeModel, enums, errors
...
def feature_effect_server_data_datetime_holdout(feature_effect_server_data_datetime):
    feature_effect_server_data_datetime_holdout = dict(
        feature_effect_server_data_datetime
    )
...
@pytest.fixture
def feature_effect_response_holdout(
    feature_effect_server_data_holdout, feature_effect_url
):
    source = "holdout"
...
def feature_effect_response_datetime(
    ...
    responses.add(
        responses.GET,
        "{}?source={}&backtestIndex=".format(
            feature_effect_url_datetime, source, backtest_index
        ),
        status=200,
        content_type="application/json",
...
@pytest.fixture
def feature_effect_previously_ran_response_datetime(
    feature_effect_url_datetime, job_id
):
    """
    Loads a response that the given model has already ran its feature effect
     """
...
def test_get_feature_effect_metadata_datetime(
    ...
    for i, meta in enumerate(sorted(fe_metadata)):
        assert (
            meta.backtest_index == feature_effect_metadata_datetime_sorted[i]["backtestIndex"]
        )
        assert meta.status == feature_effect_metadata_datetime_sorted[i]["status"]
        assert sorted(meta.sources) == sorted(
            feature_effect_metadata_datetime_sorted[i]["sources"]
        )


@responses.activate
@pytest.mark.usefixtures(
    "feature_effect_job_creation_response", "feature_effect_running_response"
)
...
def test_get_feature_effect_job_result_finished(feature_effect_server_data, one_model):
    ...
    assert len(feature_effect.feature_effects) == len(
        feature_effect_server_data["featureEffects"]
    )
...
def test_get_or_request_feature_effect_previously_requested(
    one_model, feature_effect_server_data
):
...
def test_get_or_request_feature_effect_currently_running_waits(
    one_model, mock_async_time
):
    mock_async_time.time.side_effect = (0, 5)
    with pytest.raises(errors.AsyncTimeoutError):
...
def test_wait_for_feature_effect_finished(one_model, feature_effect_server_data):
    ...
    feature_effect_job = one_model.request_feature_effect()
    feature_effect = feature_effect_job.get_result_when_complete(
        max_wait=0.5, params=params
    )
...
def test_get_feature_effect_job_result_finished_datetime(
    ...
    assert feature_effect.project_id == feature_effect_server_data_datetime["projectId"]
    assert (
        feature_effect.backtest_index == feature_effect_server_data_datetime["backtestIndex"]
    )
    assert len(feature_effect.feature_effects) == len(
...
def test_get_feature_effect_job_result_finished_holdout_datetime(
    ...
    feature_effect = feature_effect_job.get_result(params)
    assert (
        feature_effect.project_id == feature_effect_server_data_datetime_holdout["projectId"]
    )
    assert (
        feature_effect.model_id == feature_effect_server_data_datetime_holdout["modelId"]
    )
    assert (
        feature_effect.source == feature_effect_server_data_datetime_holdout["source"]
    )
    assert (
        feature_effect.project_id == feature_effect_server_data_datetime_holdout["projectId"]
    )
    ...
    expected_fe = FeatureEffects.from_server_data(
        feature_effect_server_data_datetime_holdout
    )
    assert expected_fe == feature_effect
...
def test_wait_for_feature_effect_never_finished_datetime(
...
def test_get_feature_effect_invalid_source_datetime(one_datetime_model):
    ...
    source = "invalid"
    with pytest.raises(errors.ClientError):
        one_datetime_model.get_feature_effect(
            source=source, backtest_index=backtest_index
        )
