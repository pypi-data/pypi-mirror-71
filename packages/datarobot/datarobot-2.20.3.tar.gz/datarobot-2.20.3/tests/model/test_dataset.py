...
from datarobot.models.dataset import Dataset, ProjectLocation, DatasetDetails, FeatureTypeCount
...
@responses.activate
def test_list_dataset_sans_processing_state(unittest_endpoint, mock_dataset_list):
    # in 2.20 it was possible for processingState to not be present
    for d in mock_dataset_list["data"]:
        d.pop("processingState")
...
def test_get_projects(dataset_url, mock_dataset_obj):
    ...
    projects_response_one = {
        'count': 1, 'totalCount': 2, 'next': next_link, 'data': [location_one], 'previous': None
    }
    projects_response_two = {
        'count': 1, 'totalCount': 2, 'next': None, 'data': [location_two], 'previous': None
    }
...
class TestProjectCreate(object):
    ...
    def test_create_from_dataset_use_defaults(self, project_collection_url,
                                              project_without_target_json, mock_dataset_obj):
        project = mock_dataset_obj.create_project()

        create_body = request_body_to_json(responses.calls[0].request)
        assert create_body == {
            'datasetId': mock_dataset_obj.id, 'datasetVersionId': mock_dataset_obj.version_id
        }
...
class TestDatasetDownloadFile(object):
    ...
    def test_get_file_with_both_filelike_and_file_path_raises_type_error(self, mock_dataset_obj,
                                                                         file_path):
        with pytest.raises(TypeError):
            with open(file_path, 'wb') as filelike:
...
class TestDatasetGetFeatures(object):
    ...
    @responses.activate
    def test_get_all_features_order_by(self, dataset_url, mock_dataset_obj,
...
class TestDatasetCreation(object):
    ...
    def add_mock_upload_response(url, status_location):
        responses.add(
            ...
            url,
            status=200,
            adding_headers={'Location': status_location}
        )

    @pytest.fixture()
    def set_up_file_upload(self, unittest_endpoint, status_location,
                           set_status_response, file_path):
        with open(file_path, 'wb') as f:
            f.write(b'')
        self.add_mock_upload_response(
            '{}/datasets/fromFile/'.format(unittest_endpoint),
            status_location
        )
        yield file_path
    ...
    @responses.activate
    def test_create_from_file_path_with_categories(self, mock_dataset, set_up_file_upload,
                                                   dataset_url):
    ...
    @responses.activate
    def test_create_from_in_memory_data_with_dataframe(
            self, mock_dataset,
            set_up_file_upload
    ):
    ...
    @responses.activate
    def test_create_from_in_memory_data_with_source_list(
            self, mock_dataset,
            set_up_file_upload
    ):
        data = [{'a': 'x', 'b': 'y'}, {'a': 'd', 'b': 'e'}]
    ...
    def set_up_url_upload(self, unittest_endpoint, status_location, set_status_response):
        self.add_mock_upload_response(
            '{}/datasets/fromURL/'.format(unittest_endpoint),
            status_location
        )
    ...
    def test_create_from_url_with_persist_data_after_ingesiont(self, mock_dataset):
        ...
        post_params = request_body_to_json(responses.calls[0].request)
        assert post_params == {
            'url': url, 'persistDataAfterIngestion': persist_data_after_ingestion
        }
