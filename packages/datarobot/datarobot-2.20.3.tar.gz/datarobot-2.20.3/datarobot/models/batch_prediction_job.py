...
class BatchPredictionJob(AbstractSpecificJob):
    ...
    def score(
        ...
        The default intake and output options are both `local_file` which
        ...
        output_settings : dict (optional)
            ...
            To save scored data to JDBC, add the next parameters to the settings:
                ...
                - `data_store_od` : string, the ID of the external data store connected to
        ...
        if prediction_instance is not None:
            ...
            job_data['csvSettings'] = to_api(prediction_instance)
        ...
        if intake_settings.get('type') not in ('local_file', 's3', 'jdbc'):

            raise ValueError('Unsupported type parameter for intake_settings: %s' % (
                intake_settings.get('type'),
            ))

        elif intake_settings['type'] == 'local_file':
        ...
        if output_settings.get('type') not in ('local_file', 's3', 'jdbc'):

            raise ValueError('Unsupported type parameter for output_settings: %s' % (
                output_settings.get('type'),
            ))

        elif output_settings['type'] == 'local_file':
