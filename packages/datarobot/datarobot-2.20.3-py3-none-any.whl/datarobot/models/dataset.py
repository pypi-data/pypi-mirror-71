...
from datarobot.utils import encode_utf8_if_py2, dataframe_to_buffer
...
_base_dataset_schema = t.Dict({
    ...
    t.Key('creation_date') >> 'created_at': (lambda s: dateutil.parser.parse(s)),
    ...
    t.Key('processing_state', default=''): t.String(allow_blank=True),
})
...
class Dataset(APIObject):
    ...
    def __init__(self, dataset_id, version_id, name, categories, created_at, created_by,
                 is_data_engine_eligible, is_latest_version, is_snapshot, processing_state,
                 data_persisted=None, size=None, row_count=None):
        self.id = dataset_id
    ...
    @classmethod
    def create_from_file(cls, file_path=None, filelike=None, categories=None):
        """
        ...
        new_dataset_location = wait_for_async_resolution(
            cls._client, response.headers['Location']
        )
    ...
    @classmethod
    def create_from_in_memory_data(cls, data_frame=None, records=None, categories=None):
        """
        ...
        return cls.create_from_file(filelike=buff, categories=categories)

    @classmethod
    def create_from_url(cls, url, do_snapshot=None, persist_data_after_ingestion=None,
                        categories=None):
        """
        ...
        data = {key: val for key, val in base_data.items() if val is not None}
    ...
    def list(cls, category=None, filter_failed=None, order_by=None):
        ...
        if filter_failed is not None:
            query_params['filter_failed'] = str(filter_failed).lower()

        data = unpaginate(cls._path, query_params, cls._client)
    ...
    def get_all_features(self, order_by=None):
        ...
        Returns
        -------
        feature_list: list[DatasetFeature]
        """
        url = '{}{}/allFeaturesDetails/'.format(self._path, self.id)
        params = {} if order_by is None else {'order_by': order_by}
        result = unpaginate(url, params, self._client)
        return [DatasetFeature.from_server_data(el) for el in result]
    ...
    def create_project(self, project_name=None, user=None, password=None, credential_id=None,
                       use_kerberos=None):
        """
        ...
        Project
        """
        return Project.create_from_dataset(self.id, dataset_version_id=self.version_id,
                                           project_name=project_name, user=user, password=password,
                                           credential_id=credential_id, use_kerberos=use_kerberos)
...
class DatasetDetails(APIObject):
    ...
    _extra_fields = t.Dict({
        ...
        t.Key('eda1_modification_date', optional=True): (lambda s: dateutil.parser.parse(s)),
        ...
        t.Key('feature_count_by_type', optional=True): t.List(lambda d: FeatureTypeCount(**d)),
        t.Key('last_modification_date'): (lambda s: dateutil.parser.parse(s)),
