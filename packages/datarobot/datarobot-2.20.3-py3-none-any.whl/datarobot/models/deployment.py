...
class Deployment(APIObject):
    ...
    def __init__(self, id=None, label=None, description=None, default_prediction_server=None,
                 model=None, capabilities=None, prediction_usage=None, permissions=None,
                 service_health=None, model_health=None, accuracy_health=None):
        self.id = id
        self.label = label
    ...
    def submit_actuals(self, data):
        """Submit actuals for processing.
        The acutals submitted will be used to calculate accuracy metrics.
        ...
        if not isinstance(data, (list, pd.DataFrame)):
            raise ValueError(('data should be either a list of dict-like objects '
        ...
        for item in data:
            ...
            payload_data.append(actual)
        response = self._client.post(url, data={'data': payload_data})
    ...
    def update_association_id_settings(
        ...
        if required_in_prediction_requests is not None:
            payload['associationId'][
                'requiredInPredictionRequests'] = required_in_prediction_requests
        if not payload:
            raise ValueError()
    ...
    def update_prediction_warning_settings(self, prediction_warning_enabled,
                                           use_default_boundaries=None, lower_boundary=None,
                                           upper_boundary=None, max_wait=DEFAULT_MAX_WAIT):
        ...
        if use_default_boundaries is True:
            ...
            if upper_boundary is not None and lower_boundary is not None:
                payload['prediction_warning']['custom_boundaries'] = (
                    {'upper': upper_boundary, 'lower': lower_boundary}
                )
    ...
    def update_prediction_intervals_settings(self, percentiles, enabled=True,
                                             max_wait=DEFAULT_MAX_WAIT):
    ...
    def get_service_stats(self, model_id=None, start_time=None, end_time=None,
    ...
    def get_service_stats_over_time(self, metric=None, model_id=None, start_time=None,
                                    end_time=None, bucket_size=None, quantile=None, threshold=None):
    ...
    def get_accuracy_over_time(self, metric=None, model_id=None,
                               start_time=None, end_time=None, bucket_size=None):
        ...
        return AccuracyOverTime.get(self.id, metric=metric, model_id=model_id,
...
class DeploymentListFilters(object):
    ...
    materiality : list of str
        A list of strings representing the deployments' "materiality" (also known as
        "importance" in the UI). If provided, then only return those deployments whose materiality
        is one of those provided. See ``datarobot.enums.DEPLOYMENT_MATERIALITY``
    ...
    **Performing OR logic**
    ...
    service_health = attr.ib(
        default=None,
        converter=_check(
            t.List[t.String()]
        ),
    )
    model_health = attr.ib(
        default=None,
        converter=_check(t.List[t.String()]),
    )
    accuracy_health = attr.ib(
        default=None,
        converter=_check(
            t.List[t.String()]
        ),
    )
    execution_environment_type = attr.ib(
        default=None, converter=_check(t.List[t.String()])
    )
    materiality = attr.ib(
        default=None, converter=_check(t.List[t.String()])
    )
    ...
    def construct_query_args(self):
        ...
        if self.service_health:
            query_args['serviceHealth'] = self._list_to_comma_separated_string(
                self.service_health
            )
        if self.model_health:
            query_args['modelHealth'] = self._list_to_comma_separated_string(
                self.model_health
            )
        ...
        if self.materiality:
            query_args['materiality'] = self._list_to_comma_separated_string(
                self.materiality
            )
