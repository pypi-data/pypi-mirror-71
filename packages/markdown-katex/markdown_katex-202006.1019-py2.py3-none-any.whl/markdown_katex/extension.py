...
def iter_inline_katex(line):
    ...
    while True:
        ...
        marker = "<span id='katex{0}'>katex{1}</span>".format(marker_id,
            marker_id)
    ...
    def run(self, lines):
        ...
        for line in lines:
            if is_in_fence:
                ...
                marker = "<p id='katex{0}'>katex{1}</p>".format(marker_id,
                    marker_id)
                ...
                if math_fence_match:
                    ...
                    for inline_code in iter_inline_katex(line):
                        math_html = md_inline2html(inline_code.inline_text,
                            self.ext.options)
    ...
    def run(self, text):
        ...
        for marker, html in self.ext.math_html.items():
            ...
            if wrapped_marker in text:
                ...
                log.warning("KatexPostprocessor couldn't find: {0}".format(
                    marker))
        return text
