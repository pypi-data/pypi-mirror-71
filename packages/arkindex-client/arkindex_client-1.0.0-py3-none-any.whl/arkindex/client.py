...
# Endpoint accessed by the client on instanciation to retrieve the OpenAPI schema
...
class ArkindexClient(apistar.Client):
    ...
    def __init__(self, token=None, base_url=DEFAULT_BASE_URL, schema_url=None, csrf_cookie=None, sleep=0, **kwargs):
        r"""
        ...
        try:
            ...
            raise SchemaError(f'Could not retrieve a proper OpenAPI schema from {schema_url}') from e
        ...
        if not len(self.document.walk_links()):
            raise SchemaError(f'The OpenAPI schema from {base_url} has no defined endpoints')
