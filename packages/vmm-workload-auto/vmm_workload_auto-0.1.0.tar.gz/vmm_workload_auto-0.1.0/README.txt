...
4.	For every <DVS, DPG> or <Host, PG> specified in the config file, it gets the associated network mapping in DCNM.
...
2. CSV file (Sample.csv): This file has the mapping of <DVS, DVS-PG> or <Host, PG> in vCenter to Network name in DCNM. There’s a separate CSV file for each DCNM. Subsequent sections will have more information about the fields in the file.
...
This module also provides the following REST API's
