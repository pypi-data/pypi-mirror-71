...
class TestMap(unittest.TestCase):
    ...
    def test_helpers_count_truthy_no_true(self):
        ...
        self.assertEqual(
            hf.count_number_of_truthy(expect_no_true1), 0, "test 1 failed."
        )

        expect_no_true2 = [None, False, False, None]
        self.assertEqual(
            hf.count_number_of_truthy(expect_no_true2), 0, "test 2 failed."
        )

        expect_no_true3 = []
        self.assertEqual(
            hf.count_number_of_truthy(expect_no_true3), 0, "test 3 failed."
        )
