...
class FileSystemStorage(Storage):
    ...
    def get_sha(self, token, url):
        ...
        if r.status_code == 200:
            ...
            #print('r.text==============' + str(json.loads(r.text)))
        else:
            sha = 'None'
        return sha

    def put_data(self, image_name, image_path):
        ...
        if image_name.find("/") > -1:
            folder_and_image = image_name.split("/")
            url = settings.GIT_URL + folder_and_image[0] + "/" + folder_and_image[1]
        else:
            folder_and_image = image_name.split("\\")
            url = settings.GIT_URL + folder_and_image[0] + "/" + folder_and_image[1] 
    ...
    def delete_data(self, image_name):
        ...
        url = settings.GIT_URL
        folder_name = settings.MEDIA_FOLDER_NAME

        num = image_name.index(folder_name)

        #print("---===folder_name===---" + str(image_name[num + len(folder_name) + 1:]))

        folder_and_image = image_name[num + len(folder_name) + 1:]
        folder_and_image = folder_and_image.replace("\\", "/")
        url = settings.GIT_URL + folder_and_image
    ...
    def _save(self, name, content):
        ...
        #self.put_data(name, full_path)
        Storage.upload_large_file("django", name, full_path)
