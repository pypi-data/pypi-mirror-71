...
class TestSandbox:
    ...
    # Preserving older api as we have tests with those function calls
    def preserve_old_api(self):
        ...
        if self.logging:
            log.info(f"preserve_old_api(self)")

        self.recordVideo = self.record_video
        self.recordVideoPid = self.record_video_pid
        self.attachVideo = self.attach_video
        self.attachVideoOnPass = self.attach_video_on_pass
        self.attachJournal = self.attach_journal
        self.attachJournalOnPass = self.attach_journal_on_pass
        self.attachScreenshot = self.attach_screenshot
        self.failedTest = self.failed_test
        self.attachFAF = self.attach_faf
        self.attachFAFOnPass = self.attach_faf_on_pass
