...
""" 
### Triplet Loss
A triplet consists of an anchor a, 
a positive p of the same class as the anchor and 
...
As soon as n becomes an "eays negative", the loss becomes zero.
...
def _get_anchor_positive_triplet_mask(labels):
        ...
        labels_equals = tf.equal(tf.expand_dims(labels, 0), tf.expand_dims(labels, 1))
...
def batch_hard_triplet_loss(labels, embeddings, margin, squared=False):
        ...
        hardest_ap_indices = tf.where(tf.equal(hardest_positive_dist, anchor_positive_dist))
        ...
        hardest_an_indices = tf.where(tf.equal(hardest_negative_dist, anchor_negative_dist))
