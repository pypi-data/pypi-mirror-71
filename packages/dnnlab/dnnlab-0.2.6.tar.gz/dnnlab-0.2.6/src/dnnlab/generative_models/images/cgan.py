...
class cGAN():
    ...
    def fit(self,
        ...
        for epoch in range(epochs):
            step_float = 0
            start = time.time()
            for input_images, target_images in zip(input_domain,
                                                   target_domain):
                generated_images, source_images, target_images = self.train_step(
                    input_images, target_images, step, max_outputs,
                    file_writer)
                # TODO Workaround. summary image & gpu usage
                with file_writer.as_default():
                    tf.summary.image("generated", generated_images, step=step)
                    tf.summary.image("source", input_images, step=step)
                    tf.summary.image("target", target_images, step=step)
    ...
    def train_step(self, source_images, target_images, step, max_outputs,
        ...
        with file_writer.as_default():
            ...
            with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
                ...
                disc_target = self.discriminator(
                    [source_images, target_images], training=True)
                disc_generated = self.discriminator(
                    [source_images, generated_images], training=True)
            ...
            return generated_images, source_images, target_images
