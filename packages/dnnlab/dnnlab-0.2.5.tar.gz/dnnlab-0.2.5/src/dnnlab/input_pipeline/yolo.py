...
def create_dataset(tfrecords,
    ...
    parsed_dataset = raw_dataset.map(
        lambda example: _parse_tfrecords(example, height, width, depth, grid_h, grid_w, anchors, labels),
...
def _parse_tfrecords(example, height, width, depth, grid_h, grid_w, anchors,
    ...
    feature_description = {
        ...
        tf.io.FixedLenSequenceFeature(
            [], tf.float32, allow_missing=True, default_value=-1.0),
        'bbox_xmax':
        tf.io.FixedLenSequenceFeature(
            [], tf.float32, allow_missing=True, default_value=-1.0),
        'bbox_ymin':
        ...
        'class_ids':
        tf.io.FixedLenSequenceFeature(
            [], tf.int64, allow_missing=True, default_value=-1)
    }
...
def _find_best_anchor(center_w, center_h, anchors, n_anchors):
    ...
    for anchor in range(n_anchors):
        ...
        union = float(center_w * center_h
                      ) + anchor_widths[anchor] * anchor_heights[anchor]
