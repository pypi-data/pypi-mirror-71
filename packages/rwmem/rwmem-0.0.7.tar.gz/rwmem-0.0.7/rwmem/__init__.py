...
class Memory(object):
    ...
    def open(self, pid):
        ...
        if isinstance(pid, str):
            if not pid.isdigit():
                pid = getPID(pid)
            pid = int(pid)
        if not isinstance(pid, int):raise TypeError("open(): Error: expected an 'int' or 'str' of ProcessID not '{}'".format(type(pid).__name__))
        self.hProcess = k32.OpenProcess(PROCESS_ALL_ACCESS, 0 ,pid)
    ...
    def readBytes(self, address, length):
        ...
        bytes_read = c_size_t()
        remem(self.hProcess, address, buffer, length, byref(bytes_read))
        data = buffer.raw
        if not data: self.check("readBytes", exp=True)
        return data


    def readStr(self, address, length=50):
        buff = self.readBytes(address, length)
        i = buff.find(b'\x00')
        if i != -1:buff = buff[:i]
        return  str(buff.decode())


    def read(self, address, TYPE="ui"):
      self.check("read")
      sin,size = self.getTypeInfo(TYPE)
      if not size:raise Exception("read(): Error: invalid Data Size")
      buffer = create_string_buffer(size)
      bytes_read = c_size_t()
      remem(self.hProcess, address, buffer, size, byref(bytes_read))
    ...
    def writeBytes(self, address, Value):
          ...
          if not wrmem(self.hProcess, address, c_data_, len(Value), None):self.check("writeBytes", exp=True)


    def write(self, address, Value, TYPE="ui"):
      self.check("write")
      sin,size = self.getTypeInfo(TYPE)
      c_data = c_char_p(struct.pack(sin, Value))
      c_data_ = cast(c_data, POINTER(c_char))
      if not wrmem(self.hProcess,address,c_data_,size,None):self.check("write", exp=True)
