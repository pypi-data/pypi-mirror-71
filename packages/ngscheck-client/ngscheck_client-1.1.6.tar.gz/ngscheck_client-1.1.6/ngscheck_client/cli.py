...
"""
import argparse
import getpass as getpassmodule
import logging
import sys
...
def main():
    ...
    try:
        ...
        if hasattr(args, 'email'):
            password = getpass(args.password_file)
...
def getpass(passwordfile=None):
    """
    Returns password read from file or interactively from prompt.

    Args:
        passwordfile: an open file-like object, will be closed after reading
    """
    if passwordfile:
        password = passwordfile.readline().strip()
        passwordfile.close()
    else:
        password = getpassmodule.getpass('Password: ')
    return password
