    ...
    def plot_fund_alpha(fund_nav:pd.DataFrame, 
        ...
        for index_id in res:
            ...
            for d_idx, d in enumerate(sorted(set(df_index_funds.start_date))):
                ...
                index_ret = round((index_price.loc[d2,index_id] / index_price.loc[d1,index_id] - 1),3)
                dic = {
                    'start_date':d1,
                    'end_date':d2,
                    index_id:index_ret,
                ...
                for _idx, r in df_i.iterrows():
                    f_r = round((fund_nav.loc[d2,r.fund_id] / fund_nav.loc[d1,r.fund_id] - 1), 3)
                    fund_loc = f'fund_{_idx+1}'
                    #dic[fund_loc] = f'{fund_desc_name[r.fund_id]} w: {round(r.weight,2)} ret: {f_r}'
                    dic[fund_loc] = f'{fund_desc_name[r.fund_id]} {f_r}'
                    if f_r >= index_ret:
