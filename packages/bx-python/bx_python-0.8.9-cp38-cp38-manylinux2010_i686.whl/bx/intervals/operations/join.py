...
def join(leftSet, rightSet, mincols=1, leftfill=True, rightfill=True):
    ...
    for interval in leftSet:
        ...
        if not isinstance(interval, GenomicInterval):
            ...
            overlap_not_met = 0
            for item in result:
                if item.start in range(interval.start, interval.end+1) and item.end not in list(range(interval.start, interval.end+1)):
                    overlap = interval.end-item.start
                elif item.end in range(interval.start, interval.end+1) and item.start not in list(range(interval.start, interval.end+1)):
