...
class ParallelRunResultAnalyzer():
    ...
    def analyzeHourlyzationResult(self, signal_name_to_analyse, path_to_close_pkl = None, jsonized_output = True, transaction_cost=True):
        signals_df = self.analyzeAllRunSignalsResults(jsonized_output=jsonized_output)
        ohlc = pd.read_pickle(path_to_close_pkl)
        close = ohlc['close']
        signal = signals_df[signal_name_to_analyse]
        results_df = signal_utility.daily_signal_hourlyzation(close,signal,transaction_cost=transaction_cost)
        return results_df
