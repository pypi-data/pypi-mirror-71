...
class RealTimeSignalEnsemblingParalleLauncher():
    def __init__(self, min_calib_starting_date=None, min_calib_running_date=None, day_calib_starting_date=None, day_calib_running_date=None, drop_token='', dropbox_backup=True, BINANCE_PUBLIC='', BINANCE_SECRET='', AWS_PUBLIC='', AWS_SECRET='', AWS_REGION='',bucket='', local_root_directory='../data',underlying = None, pair = None, min_calib_selected_algo='', day_calib_selected_algo='', user='napoleon',  db_path_suffix = '_run.sqlite', list_pkl_file_suffix = 'my_list.pkl', backed_up_signal_histo_suffix='backed_up_signal_histo.pkl', load_daily_histo = True, from_cryptocompare=False):
        ...
        self.backed_up_signal_histo_suffix = backed_up_signal_histo_suffix
        ...
        if self.load_daily_histo:
            ...
            self.reload_daily_histo(from_cryptocompare = self.from_cryptocompare)
    ...
    def recompute_daily_histo(self, from_cryptocompare = False):
        if from_cryptocompare:
            ...
            print('not enough ohlc binance history')


    def recreate_daily_histo(self, daily_df, up_to_date):
        cumulated_signals = None
        print('Number of signals to aggregate '+str(len(self.day_calib_signals_list)))
        ...
        backed_up_signal_histo_name =  up_to_date.strftime('%d_%b_%Y')+'_'+self.underlying+'_'+self.backed_up_signal_histo_suffix
    ...
    def reload_minute_histo(self, from_cryptocompare = True, nb_last_hours=24):
    ...
    def reload_hour_histo(self, from_cryptocompare=False, nb_last_days = 1):
        ...
        freqly_pkl_file_name_suffix = '_day_tmp.pkl'
        hours_df = napoleon_connector.fetch_crypto_minutely_data(ssj=self.underlying,
    ...
    def reload_daily_histo(self, from_cryptocompare = False):
        upto_date = datetime.now() - timedelta(days=1)
        if from_cryptocompare :
            backed_up_signal_histo_name =  upto_date.strftime('%d_%b_%Y')+'_'+self.underlying+'_'+self.backed_up_signal_histo_suffix
            saving_path = self.local_root_directory+backed_up_signal_histo_name
            ...
            updated_df = updated_df[updated_df.index <= upto_date]
            updated_df=updated_df.sort_index()
            signal_columns = [self.day_signal_mapping[me_sig] for me_sig in self.day_calib_signals_list]
            me_columns = self.generic_columns + signal_columns
            updated_df = updated_df[me_columns].copy()
    ...
    def launchDaySequential(self):
        ...
        if len(self.day_signal_np.shape)>1 and len(self.day_signal_np)>3:
            ...
            for meArg in self.day_calib_signals_list:
                ...
                counter = counter + 1
            last_signals = self.day_signal_np[-1, len(self.generic_columns) :]
            return self.day_signals_ia_mixing(last_signals)
        return np.nan
    ...
    def launchMinuteSequential(self):
        ...
        if len(self.minute_signal_np.shape)>1 and len(self.minute_signal_np)>3:
            ...
            for meArg in self.min_calib_signals_list:
                ...
                counter = counter + 1
            last_signals = self.minute_signal_np[-1, len(self.generic_columns) :]
            return self.minute_signals_ia_mixing(last_signals)
        return np.nan

    def day_signals_ia_mixing(self, signals):
        nb_nan_signals = np.isnan(signals).sum()
        print('still '+str(nb_nan_signals)+' not triggered day signals')
        return np.nanmean(signals)

    def minute_signals_ia_mixing(self, signals):
        nb_nan_signals = np.isnan(signals).sum()
