...
def validate_dataframe(pandas_dataframe,format):
    ...
    if not(isinstance(pandas_dataframe,pd.core.frame.DataFrame)):
        ...
        raise ValueError
    if not(pandas_dataframe.index.name == 'Date' or pandas_dataframe.index.name == 'DateTime'):
...
def validate_recarray(numpy_recarray,format):
    ...
    if not(isinstance(numpy_recarray,np.recarray)):
        ...
        raise ValueError
    if not(numpy_recarray.dtype.names[0] == 'Date' or numpy_recarray.dtype.names[0] == 'DateTime'):
...
def validate_ndarray(numpy_ndarray,format):
    ...
    if not(isinstance(numpy_ndarray,np.ndarray)):
        ...
        raise ValueError
    if not(numpy_ndarray.dtype.names[0] == 'Date'or numpy_ndarray.dtype.names[0] == 'DateTime'):
...
def validate_api_response(r,symbol):
    ...
    if r.status_code == 404:
        ...
        logger.error("There are no active data subscriptions in Norgate Data.  Perhaps you need to renew your subscription or need to run an update in the Norgate Data application?")
        raise NoValidSubscriptions
    elif not (r.status_code == 200 or r.status_code == 204) :
        logger.error(inspect.currentframe().f_back.f_code.co_name + ": Error in receiving Norgate Data - check parameters are correctly formatted/have valid parameter values." + errormsg)
        raise ValueError
...
def version_checker(currentversion, package):
    ...
    if newversion > currentversion:
        logger.warn("**PACKAGE VERSION WARNING*** You have version (" + currentversion + ") of the " + package + " package installed.  A newer version " + newversion + " is available and is a recommended upgrade.")
    ...
    try:
        ...
        if format == 'date':
            if (len(dateval) == 10):
                ...
                tempdatetime = datetime.datetime.strptime(dateval,'%Y%m%d')
            return datetime.date(tempdatetime.year,tempdatetime.month,tempdatetime.day)
        if format == 'pandas-timestamp':
            return pd.Timestamp(dateval, tz='UTC')
        if format == 'numpy-datetime64ns' or format == 'numpy-datetime64':
            return np.datetime64(dateval)
        if format == 'numpy-datetime64nsutc' or format == 'numpy-datetime64utc':
...
def encode_date_to_iso(date):
    ...
    if isinstance(date,str):
        ...
        try:
            if not valid:
                validate_date = datetime.datetime.strptime(date,'%Y%m%d') # Also try without dashes
            valid = True
        except:
            pass
        if not valid:
