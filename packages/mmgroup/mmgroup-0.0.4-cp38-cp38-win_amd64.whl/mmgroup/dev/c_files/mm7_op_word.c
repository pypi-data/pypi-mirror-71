...
static uint32_t mm_op7_group_n_swap(uint_mmv_t *v, uint32_t *g, uint_mmv_t *work)
...
// result is stored in v and 1 if it is stored in 'work'.
{
    ...
    if (g[1] | g[2]) {
        mm_op7_xy(p0, g[1], g[2], g[3], p1);
        d = 0;
        pt = p0; p0 = p1; p1 = pt;
    }
    if (g[4]) {
        mm_op7_pi(p0, d, g[4], p1);
        pt = p0; p0 = p1; p1 = pt;
    } else if (d) {
        mm_op7_delta(p0, d, p1);
        pt = p0; p0 = p1; p1 = pt;
    }
    return (p0 != v);
}
...
{
    if (mm_op7_group_n_swap(v, g, work)) mm_op7_copy(work, v);
}
...
void mm_op7_word(uint_mmv_t *v, uint32_t *g, int32_t len_g, int32_t e, uint_mmv_t *work)
{
    ...
    for (round = 0; round < e; ++round) {
        for (i = i_start; i != i_stop; i += i_step) {
            ...
            if (pending) {
                if (mm_op7_group_n_swap(p0, gn, p1)) {
                    pt = p0; p0 = p1; p1 = pt;
                }
                mm_group_n_clear(gn);
