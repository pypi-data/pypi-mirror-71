...
class Predictor(object):
    ...
    def _get_trial_metrics(self):
        # type: () -> Optional[Tuple[Optional[int], Optional[float]]]
        count = 0
    ...
    def _get_trial_state(self, data):
        # type: (Dict[str, Any]) -> Optional[Dict[str, float]]
        """
        ...
        experiment_count, best_metric = trial_metrics
    ...
    def stop_early(self, **data):
        ...
        try:
            # Check for cases where model is not stopped after _global_stop_early returns True
            if self.wait >= self.patience:
                LOGGER.debug(
