"""
This is a much stripped-down version of core.py in the "uncertainties" package
https://pythonhosted.org/uncertainties. That package is *excellent*, very well
documented, and incredibly useful! Go check it out. 

The goal here is to hack only the representation part (as in repr, str) of a
number with uncertainty to allow for asymetrical uncertainties, something that
the uncertanties package does not implement (AFAIK).

This module provides two main functions: uformat and uformatul

 - uformat is a complete copy and behaves in the same way as
   uncertainties.core.Variable.format

 - uformatul takes upper and lower uncertainties instead of the usual std_dev
   and represents the number in more or less the same way as uformat, but also
   showing the two uncertainty measures.

Note that a lot of the documentation and comments in uncertainties.core have
