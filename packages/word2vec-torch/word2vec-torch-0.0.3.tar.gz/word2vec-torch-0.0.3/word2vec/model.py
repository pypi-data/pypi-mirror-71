...
class SkipGramModel(nn.Module):
    ...
    def __init__(self, emb_size, emb_dimension):
        ...
        self.u_embeddings = []
        self.v_embeddings = []
        print('len(emb_size):',len(emb_size))
        initrange = 1.0 / self.emb_dimension / len(emb_size)
        for i in range(len(emb_size)):
            self.u_embeddings.append(nn.Embedding(emb_size[i], emb_dimension, sparse=True))
            self.v_embeddings.append(nn.Embedding(emb_size[i], emb_dimension, sparse=True))
    ...
    def forward(self, pos_u, pos_v, neg_v):
        ...
        for i in range(len(self.emb_size)):
            emb_u.append(torch.reshape(self.u_embeddings[i](pos_u[i]),(-1,1,self.emb_dimension)))
    ...
    def save_embedding(self, file_name):
        ...
        with open(file_name, 'w') as f:
            for i in range(len(embedding)):
                f.write('%s %s\n' % (i, ' '.join(list(map(str,embedding[i])))))
