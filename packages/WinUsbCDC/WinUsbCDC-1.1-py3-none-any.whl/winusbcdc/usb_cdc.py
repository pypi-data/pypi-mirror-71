...
class ComPort:
    ...
    def __init__(self, name=None, vid=0, pid=0, start=True):
        ...
        if not name and not (vid and pid):
            raise AttributeError("Must provide friendly name or vid & pid of device to connect to")
    ...
    def readinto(self, buf):
        if not self.is_open:
            return None
        orig_size = remaining = len(buf)
        remaining -= len(self._rxremaining)
        buf[:] = self._rxremaining
        self._rxremaining = b''
    ...
    def read(self, size=None):
        ...
        if size:
            ...
            self.device.set_timeout(self._ep_in, 2)
            while True:
                c = self.device.read(self._ep_in, 0)
                if c is not None and len(c):
                    end_timeout += 0.2
                    rx.append(c)
                    length += len(c)
