...
class PSenseAWSInterface(object):
    def __init__(self, profile_name='pshield', debugmode=False, testmode=False):
        ...
        try:
            ...
            log.debug('failed to load profile: {}. Loading defaults'.format(profile_name))
    ...
    def check_experiment_data(self, experiment_name):
        ...
        if len(experiment_name) > 0:  # skip if there is no data
            ...
            if response['Count'] > 0:
                ...
                last_enable = [value for key, value in sorted(response['Items'][0].items()) if 'enable' in key]
                last_vout = [value for key, value in sorted(response['Items'][0].items()) if 'vout' in key]
                if len(last_enable): # are there enable flags in the data?
                    last_vout = ' / '.join(['{:.3f}'.format(last_vout[i]) if enabled else '(off)' for i, enabled in enumerate(last_enable)])
                else:
    ...
    def add_sensordata(self, timestamp=[], vout=None, signal=[0], enable=True):
        ...
        for i in range(len(signal)):
            item['signal{}'.format(
                i + 1)] = Decimal(str(round(signal[i], 2))) # floats -> Decimal for ddb
            item['vout{}'.format(
                i + 1)] = Decimal(str(round(vout[i], 3))) # floats -> Decimal for ddb
    ...
    def generate_put_request(self, row):
        ...
        for i in range(int((len(row) - 1) / 3)):
            request['signal{}'.format(
                i + 1)] = Decimal(str(round(row[3 * i + 2], 2)))
            request['vout{}'.format(
                i + 1)] = Decimal(str(round(row[3 * i + 1], 3)))
    ...
    def add_bulk_sensordata(self, data):
        ...
        for sample in data:
            ...
            if len(requests) >= self.batch_req_limit:
                log.info('batch of [{}] items. First item: {}'.format(
                    len(requests),
                    requests[0]))
        ...
        try:
            log.info('batch of [{}] items. Last item: {}'.format(
                len(requests),
                requests[-1]))
    ...
    def get_sensor_data(self):
        ...
        log.info('[{}] initial request found {} records'.format(
            expid, count))
    ...
    def add_event(self, set, timestamp, type, reference, sensor):
        ...
        response = self.table_eventdata.put_item(
            Item=newitem
        )
        log.debug('response: {}'.format(response))
